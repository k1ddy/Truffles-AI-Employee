...
| **Top Architect** | Курс, инварианты, DoD, DEC, архитектура | Делать задачи руками |
...
**Git-власть:** Brain — единственный, кто мержит в main и катит релиз. Hands могут коммитить/пушить только в свою ветку (если это разрешено в Task Package) и обязаны приложить status/diff/evidence.
...
2) Проверить факты в среде: docker/БД/CI/логи (если есть доступ — проверяй сам).
3) Если ответ найден — **не спрашивать**.
4) Если ответа нет — задать вопрос и зафиксировать GAP.
...
6) **Checks**: тесты/команды.
7) **Evidence**: что сохраняем (CI run URL + логи/SQL/trace) и кто фиксирует запись в `STATE.md` (Brain, до merge при изменениях поведения/core).
...
**Кто обновляет `STATE.md`:** только **Brain**; для изменений поведения/core — **до merge в рамках PR**, плюс финальная запись в конце сессии.
