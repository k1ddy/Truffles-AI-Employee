...
3. Перед вопросом — проверить доступы и факты в среде (docker/БД/CI/логи); если доступ есть — проверять самому, не перекладывать на пользователя.
4. Вопросы могут быть “простыми/идиотскими” — это нормально. Важно, чтобы они были по фактам.
5. Если ответ найден в проекте — **не спрашивать**, а продолжать.
6. Если ответа нет — спросить Жанбола и зафиксировать GAP.
7. **STATE.md не является доказательством:** любой пункт требует подтверждения через логи/БД/trace до фикса.
...
### 5. Подгон под тесты и хардкод
Вижу failing EVAL → вшиваю списки/кейсы в код → ломаю масштабируемость.
**Правило:** Логика единая, факты/словари только в Client Pack/Domain Pack; EVAL = доказательство, не цель.
...
## ЧЕКЛИСТ ЗАДАЧИ BRAIN (перед выдачей Hands)

- Цель/ожидаемый результат
- Scope: файлы + ограничения (read‑only/можно править)
- DoD/критерии приёмки
- Формат ответа/доказательства (file:line + цитата/лог/SQL)
- Тесты/проверки (или явный “no tests”)
- Если есть CI fail → требовать пакет: run URL + failed job + failed step + 5–15 строк ошибки + точная команда + матрица
- Если чего‑то нет → стоп и вопрос/Gap

### ШАБЛОН TASK PACKAGE (ОБЯЗАТЕЛЕН)
...
- План/шаги (1..n)
- DoD/критерии приёмки
- Проверки/тесты (или явный "no tests")
- Evidence (какие логи/trace/CI)
- Зависимости/блокеры (если есть)
- Риски/stop-line (если применимо)

**Без заполненного шаблона — Hands не начинают работу.**
