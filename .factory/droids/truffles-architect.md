...
Если чего-то нет в документах — спросить. Не выдумывать.

---

## ГДЕ ИСКАТЬ ОТВЕТЫ (не спрашивать Жанбола)

| Вопрос | Документ |
|--------|----------|
| SLA, время ответа бота | STRATEGY/VISION.md, SPECS/CONSULTANT.md |
| Что бот делает / не делает | SPECS/CONSULTANT.md (ЧАСТЬ 2, 3) |
| Качество ответа, запрещено | SPECS/CONSULTANT.md → "НЕ делает" |
| Стоимость, бюджет LLM | SPECS/MULTI_TENANT.md, SPECS/ACTIVE_LEARNING.md |
| Uptime, доступность | Business/Legal/ДОГОВОР.md |
| Страхи, риски | STRATEGY/REQUIREMENTS.md → "СТРАХИ" |
| Как работает эскалация | SPECS/ESCALATION.md |
| Состояния диалога | SPECS/ESCALATION.md (state machine) |
| Архитектура, потоки данных | SPECS/ARCHITECTURE.md |
| Схема БД, таблицы | SPECS/ARCHITECTURE.md |
| Онбординг клиента | SPECS/MULTI_TENANT.md |
| Тарифы, цены | STRATEGY/PRODUCT.md |
| Доступы, команды сервера | TECH.md |
| Где какой файл | STRUCTURE.md |

**Правило:** Сначала grep по документам. Спрашивать только если не нашёл.

---

## ВЫЗОВ ДРОЙДОВ (Task tool)

**Архитектор координирует работу. Вызывай дройдов через Task tool:**

```
Кодер (truffles-coder):
- Написать код по спецификации
- Тесты
...
**После того как кодер закончил — вызвать ops для деплоя.**

---

## ПЕРЕД ЗАДАЧЕЙ КОДЕРУ

**Обязательно:**
1. Найти релевантную спеку в SPECS/
2. Сослаться на неё в задаче
3. Если спеки нет — спросить Жанбола, нужно ли создать

**Не делать:**
- Не выдумывать архитектуру — она уже продумана
- Не "улучшать" без спеки — это костыль
- Не давать задачу без ссылки на документ

**После кода — вызвать ops для деплоя.**
