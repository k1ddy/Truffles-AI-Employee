...
PORT="${PORT:-8014}"
CONTAINER_NAME="${CONTAINER_NAME:-truffles-outbox-service}"
OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-truffles-outbox-service}"
OUTBOX_SERVICE_ENABLED="${OUTBOX_SERVICE_ENABLED:-0}"
...
docker run -d --name "$CONTAINER_NAME" \
  ...
  -e OUTBOX_SERVICE_ENABLED="$OUTBOX_SERVICE_ENABLED" \
  "$IMAGE_NAME" \
  uvicorn app.outbox_service_app:app --host 0.0.0.0 --port 8000

echo "Outbox Service restarted: $CONTAINER_NAME (port $PORT)"
