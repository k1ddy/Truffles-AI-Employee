...
PORT="${PORT:-8010}"
CONTAINER_NAME="${CONTAINER_NAME:-truffles-knowledge-gateway}"
KNOWLEDGE_SNAPSHOT_ENABLED="${KNOWLEDGE_SNAPSHOT_ENABLED:-1}"
OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-truffles-knowledge-gateway}"
...
docker run -d --name "$CONTAINER_NAME" \
  ...
  -e KNOWLEDGE_SNAPSHOT_ENABLED="$KNOWLEDGE_SNAPSHOT_ENABLED" \
  -e OTEL_SERVICE_NAME="$OTEL_SERVICE_NAME" \
  "$IMAGE_NAME" \
  uvicorn app.knowledge_gateway_app:app --host 0.0.0.0 --port 8000

echo "Knowledge Gateway restarted: $CONTAINER_NAME (port $PORT)"
