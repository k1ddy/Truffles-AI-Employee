...
PORT="${PORT:-8012}"
CONTAINER_NAME="${CONTAINER_NAME:-truffles-inbox-service}"
OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-truffles-inbox-service}"
INBOX_SERVICE_ENABLED="${INBOX_SERVICE_ENABLED:-0}"
...
docker run -d --name "$CONTAINER_NAME" \
  ...
  -e INBOX_SERVICE_ENABLED="$INBOX_SERVICE_ENABLED" \
  "$IMAGE_NAME" \
  uvicorn app.inbox_service_app:app --host 0.0.0.0 --port 8000

echo "Inbox Service restarted: $CONTAINER_NAME (port $PORT)"
