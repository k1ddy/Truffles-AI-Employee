#!/usr/bin/env python3
"""Update Escalation Handler to use dynamic Telegram token via HTTP Request"""
import json
...
# 1. Update Load Status to include telegram_bot_token
...
WHERE c.id = '{{ $json.conversation_id }}';"""
        print("Updated: Load Status (added telegram_bot_token)")

# 2. Update Decide Action to pass telegram_bot_token
for node in workflow['nodes']:
    if node['name'] == 'Decide Action':
        # Update jsCode to include telegram_bot_token
...
// Настройки клиента (дефолты если не заданы)
...
return [{
  json: {
    ...
    instance_id: status.instance_id
        ...
        print("Updated: Decide Action (added telegram_bot_token)")

# 3. Replace "Send to Telegram" node with HTTP Request
for i, node in enumerate(workflow['nodes']):
    if node['name'] == 'Send to Telegram':
            ...
            "parameters": {
                ...
                "bodyParameters": {
                    "parameters": [
                        {
                            "name": "chat_id",
                            "value": "={{ $('Decide Action').first().json.telegram_chat_id }}"
                        },
                        {
                            "name": "text",
        ...
        print("Replaced: Send to Telegram (now HTTP Request with dynamic token)")

# 4. Replace "Notify Admin" with HTTP Request using TrufflesChatBot token (admin notifications always to me)
for i, node in enumerate(workflow['nodes']):
    if node['name'] == 'Notify Admin':
        workflow['nodes'][i] = {
            "parameters": {
                "method": "POST",
        ...
        print("Replaced: Notify Admin (now HTTP Request)")
