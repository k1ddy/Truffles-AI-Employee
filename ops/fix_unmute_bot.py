#!/usr/bin/env python3
"""Fix Unmute Bot - get conversation_id from handover directly"""
import json
...
for node in workflow['nodes']:
    # Fix Unmute Bot - get conversation_id from handover by handover_id
    if node['name'] == 'Unmute Bot':
        node['parameters']['query'] = """UPDATE conversations 
SET bot_status = 'active', bot_muted_until = NULL 
WHERE id = (
  SELECT conversation_id FROM handovers 
  WHERE id = '{{ $('Merge Token').first().json.handover_id }}'
);"""
        print("Fixed: Unmute Bot (gets conversation_id from handover)")
