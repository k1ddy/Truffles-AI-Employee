...
|----------|----------------|
| `STATE.md` | Brain (до merge для core/поведенческих изменений + финальный шаг сессии) |
...
1) **Старт:** открыть `STRUCTURE.md` (карта) и `STATE.md` (факты). Проверить, что в `STATE.md` есть краткий **NOW (1 экран)**: фокус, активные CA‑ID/Task Packages, следующие 3 шага, блокеры, последняя evidence‑дата. Если нет — стоп и запросить у Brain обновление.
