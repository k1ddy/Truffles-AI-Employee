...
def get_reminders(db: Session = Depends(get_db)):
    ...
    return RemindersResponse(count=len(reminders), reminders=reminders)


@router.post("/reminders/{handover_id}/sent", response_model=ReminderSentResponse)
def reminder_sent(handover_id: UUID, request: ReminderSentRequest, db: Session = Depends(get_db)):
