...
class ConsoleSavedView(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_console_saved_views_scope_surface_updated_at", "scope", "surface", "updated_at"),
    )
    ...
    scope = Column(Text, nullable=False, default="personal")
    name = Column(Text, nullable=False)
    version = Column(Integer, nullable=False, default=1)
    query_state = Column(JSONB, nullable=False, default=dict)
    is_default = Column(Boolean, nullable=False, default=False)
    target_branch_id = Column(UUID(as_uuid=True), ForeignKey("branches.id"), nullable=True)
