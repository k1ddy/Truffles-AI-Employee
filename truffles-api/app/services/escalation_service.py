...
from app.models import Handover, Conversation, User, ClientSettings, Branch
...
def get_or_create_topic(
    ...
    if topic_id:
        ...
        db.flush()
        print(f"Created topic {topic_id} for conversation {conversation.id}")
    else:
        print(f"Failed to create topic for conversation {conversation.id}")
...
def send_telegram_notification(
    ...
    if not bot_token or not chat_id:
        print(f"No Telegram credentials for client {handover.client_id}")
        return False
    ...
    if result.get("ok"):
        ...
        print(f"Sent to Telegram: topic={topic_id}, message_id={message_id}")
        return True
    else:
