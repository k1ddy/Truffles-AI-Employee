...
def send_whatsapp_message(
    ...
    if not instance_id or not message:
        print(f"send_whatsapp_message: missing instance_id={instance_id} or message")
        return False
    
    try:
        print(f"Sending to ChatFlow: jid={remote_jid}, instance_id={instance_id[:20]}...")
        with httpx.Client(timeout=30.0) as client:
            ...
            print(f"ChatFlow response: {response.status_code} - {response.text[:200]}")
...
def send_bot_response(
    ...
    if not instance_id:
        print(f"No instance_id found for client {client_id}")
        return False
