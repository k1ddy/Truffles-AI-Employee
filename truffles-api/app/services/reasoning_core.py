...
async def handle_webhook_payload(
    ...
    *,
    ...
    normalized_payload = _normalize_payload_for_delegation(payload)
    secret_preflight_payload: dict[str, object] | None = (
        preflight_payload if not enforce_secret and isinstance(preflight_payload, dict) else None
    )
    if enforce_secret:
        preflight_response, preflight_payload = _run_secret_enforced_preflight(
            normalized_payload,
            db,
            provided_secret=provided_secret,
            conversation_id=conversation_id,
        ...
        secret_preflight_payload = preflight_payload
    metadata = normalized_payload.body.metadata if normalized_payload and normalized_payload.body else None
    remote_jid = getattr(metadata, "remoteJid", None)
    sender_branch = _lookup_active_sender_branch(db, remote_jid)
    if sender_branch is not None:
        artifact = _build_sender_branch_ignore_artifact()
        logger.info(
            "Reasoning core ignored active branch sender",
            extra={
                "context": {
                    "reason_code": REASONING_CORE_SENDER_BRANCH_IGNORE_REASON,
                    "contract_status": artifact.turn_result.contract_status,
                    "reply_kind": artifact.turn_result.reply.reply_kind,
                    "branch_id": str(getattr(sender_branch, "id", "")) or None,
                    "branch_client_id": str(getattr(sender_branch, "client_id", "")) or None,
                    "remote_jid": remote_jid,
                }
            },
        )
    ...
    if not enforce_secret and not remote_jid:
        artifact = _build_missing_remote_jid_artifact()
        logger.info(
            "Reasoning core blocked inbound without metadata.remoteJid",
            extra={
                "context": {
                    "reason_code": REASONING_CORE_MISSING_REMOTE_JID_REASON,
                    "contract_status": artifact.turn_result.contract_status,
    ...
    if not enforce_secret and not _has_usable_tenant_context(normalized_payload):
        artifact = _build_missing_tenant_context_artifact()
        logger.info(
            "Reasoning core blocked inbound without usable tenant_context",
            extra={
                "context": {
                    "reason_code": REASONING_CORE_MISSING_TENANT_CONTEXT_REASON,
                    "contract_status": artifact.turn_result.contract_status,
    ...
    client_id = _resolve_preflight_client_id(db, normalized_payload)
    if not enforce_secret:
        tenant_context_rejection = _get_preflight_tenant_context_rejection(
            normalized_payload,
            client_id=client_id,
        )
        if tenant_context_rejection is not None:
            artifact = _build_tenant_context_reject_artifact(
                rejection=tenant_context_rejection,
            )
            logger.info(
                "Reasoning core rejected tenant_context before delegate",
                extra={
                    "context": {
                        "reason_code": tenant_context_rejection.reason_code,
                        "contract_status": artifact.turn_result.contract_status,
                        "reply_kind": artifact.turn_result.reply.reply_kind,
                        "client_id": str(client_id) if client_id is not None else None,
                        "meta": tenant_context_rejection.meta,
                    }
                },
            )
            return WebhookResponse(success=False, message=tenant_context_rejection.message)

    conversation_snapshot = _resolve_active_conversation_snapshot(
        db,
    ...
    inbound = TurnPlanner().coerce_inbound(
        {
            "message_text": normalized_payload.body.message if normalized_payload and normalized_payload.body else None,
            "message_type": normalized_payload.body.messageType if normalized_payload and normalized_payload.body else None,
            "has_media": (
                bool(normalized_payload.body.mediaData)
                or bool(
                    isinstance(normalized_payload.body.messageType, str)
                    and normalized_payload.body.messageType.strip().lower() != "text"
                )
            )
            if normalized_payload and normalized_payload.body
            else False,
        }
    )
    if inbound.is_empty_non_media():
        artifact = _build_empty_message_artifact()
        logger.info(
            "Reasoning core blocked empty non-media inbound",
            extra={
                "context": {
                    "reason_code": REASONING_CORE_PREFLIGHT_REASON,
                    "contract_status": artifact.turn_result.contract_status,
    ...
    if not enforce_secret:
        matched_branch_phone = _lookup_client_branch_phone(
            db,
            client_id=client_id,
            remote_jid=remote_jid,
        )
        if matched_branch_phone is not None:
            artifact = _build_remote_branch_phone_ignore_artifact(
                matched_phone=matched_branch_phone,
            )
            logger.info(
                "Reasoning core ignored same-client branch phone sender",
                extra={
                    "context": {
                        "reason_code": REASONING_CORE_REMOTE_BRANCH_PHONE_REASON,
                        "contract_status": artifact.turn_result.contract_status,
    ...
    duplicate_probe_enabled = not enforce_secret and not skip_persist
    if duplicate_probe_enabled:
        message_id = getattr(metadata, "messageId", None)
        duplicate_probe = _lookup_preexisting_duplicate_message(
            db,
            client_id=client_id,
            message_id=message_id,
        )
        if duplicate_probe.duplicate:
            artifact = _build_duplicate_message_artifact(
                dedup_backend=duplicate_probe.backend,
                dedup_fallback_reason=duplicate_probe.fallback_reason,
            )
            logger.info(
                "Reasoning core skipped preexisting duplicate message_id",
                extra={
                    "context": {
                        "reason_code": REASONING_CORE_DUPLICATE_REASON,
                        "contract_status": artifact.turn_result.contract_status,
            ...
            return WebhookResponse(success=True, message="Duplicate message_id")
    ...
    try:
        with ExitStack() as stack:
            if secret_preflight_payload is not None:
                ...
                stack.enter_context(
                    http_helpers._use_preflight_bridge_cache(
                        normalized_payload,
                        db,
                        conversation_id=conversation_id,
                        preflight_payload=secret_preflight_payload,
            ...
            pending_booking_resume_boundary_payload = (
            ...
            direct_owner_response = await _try_handle_turn_planner_safe_session_reset_only_delegate(
                payload=normalized_payload,
                db=db,
                conversation_id=conversation_id,
                enqueue_only=enqueue_only,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_greeting_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_booking_prompt_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_info_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
                conversation_id=conversation_id,
                conversation_snapshot=conversation_snapshot,
                pending_booking_resume_boundary_payload=pending_booking_resume_boundary_payload,
                enqueue_only=enqueue_only,
                skip_persist=skip_persist,
                policy_core_route_snapshot=policy_core_route_snapshot,
            )
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_initial_booking_prompt_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            early_explicit_handoff_allowed = (
                pending_booking_resume_boundary_payload is None
                or (
                    (
                        getattr(policy_core_route_snapshot, "intent", None),
                        getattr(policy_core_route_snapshot, "reason", None),
                    )
                    in {
                        ("human_request", "ingress_explicit_human_request"),
                        ("frustration", "ingress_explicit_frustration_handoff"),
            ...
            if early_explicit_handoff_allowed:
                direct_owner_response = await _try_handle_turn_planner_safe_explicit_handoff_owner_cutover(
                    payload=normalized_payload,
                    db=db,
                    client_id=client_id,
                    preflight_payload=secret_preflight_payload,
                ...
                if direct_owner_response is not None:
                    return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_pending_ack_continuity_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_catalog_fact_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_service_query_fact_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_pricing_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_duration_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_bookability_time_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_active_name_time_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_specialist_name_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_specialist_datetime_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_service_choice_specialist_time_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_service_choice_specialist_daypart_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_master_query_fact_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_master_query_collect_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_booking_verification_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_check_booking_prompt_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_specialist_followup_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if direct_owner_response is not None:
                return direct_owner_response
            direct_owner_response = await _try_handle_turn_planner_safe_semantic_arbitration_owner_cutover(
                payload=normalized_payload,
                db=db,
                client_id=client_id,
                preflight_payload=secret_preflight_payload,
            ...
            if pending_booking_resume_boundary_payload is None:
                terminal_handoff_intent = getattr(policy_core_route_snapshot, "intent", None)
                if terminal_handoff_intent not in REASONING_CORE_TURN_PLANNER_EXPLICIT_HANDOFF_INTENTS:
                    terminal_handoff_intent = "policy_core_guard"
                terminal_handoff_snapshot = PolicyCoreRouteSnapshot(
                    normalized_text=decision_router.normalize_for_matching(
                        body.message if body is not None and isinstance(body.message, str) else ""
                    ),
                    intent=terminal_handoff_intent,
                    action="handoff",
                    tool_action="handoff",
                    confidence=0.0,
                    reason=REASONING_CORE_TERMINAL_UNRESOLVED_REASON,
                    needs_manager=True,
                    goal="handoff",
                )
                direct_owner_response = await _try_handle_turn_planner_safe_explicit_handoff_owner_cutover(
                    ...
                    enqueue_only=enqueue_only,
                    skip_persist=skip_persist,
                    policy_core_route_snapshot=terminal_handoff_snapshot,
                )
                if direct_owner_response is not None:
                    return direct_owner_response

            terminal_bot_response = decision_router.MSG_AI_ERROR
            terminal_transport_status = "skipped"
            terminal_transport_reason = "terminal_handoff_unavailable"
            if not skip_persist and not outbox_ids:
                metadata = body.metadata if body is not None else None
                remote_jid = getattr(metadata, "remoteJid", None)
                if isinstance(remote_jid, str) and remote_jid.strip():
                    terminal_client = _resolve_turn_planner_owner_client(
                        db,
                        payload=normalized_payload,
                        client_id=client_id,
                        preflight_payload=secret_preflight_payload,
                    )
                    if isinstance(terminal_client, Client):
                        instance_id = get_instance_id(
                            db,
                            terminal_client.id,
                            branch_id=_resolve_snapshot_branch_id(secret_preflight_payload),
                            remote_jid=remote_jid.strip(),
                        )
                        if instance_id:
                            send_result = send_message_safe(
                                instance_id,
                                remote_jid.strip(),
                                terminal_bot_response,
                                getattr(metadata, "messageId", None),
                                notify_on_failure=True,
                                record_metrics=True,
                            )
                            sent = bool(getattr(send_result, "is_ok", lambda: False)())
                            terminal_transport_status = "delivered" if sent else "failed"
                            terminal_transport_reason = (
                                None
                                if sent
                                else _resolve_turn_planner_transport_reason(
                                    send_result,
                                    instance_id=instance_id,
                                )
                            )
                        else:
                            terminal_transport_status = "failed"
                            terminal_transport_reason = "transport_target_missing"
            terminal_artifact = _build_terminal_unresolved_artifact(
                bot_response=terminal_bot_response,
                transport_status=terminal_transport_status,
                transport_reason=terminal_transport_reason,
            )
            logger.warning(
                "Reasoning core terminated unresolved owner path without frozen delegate",
                extra={
                    "context": {
                        "reason_code": REASONING_CORE_TERMINAL_UNRESOLVED_REASON,
                        "contract_status": terminal_artifact.turn_result.contract_status,
                        "reply_kind": terminal_artifact.turn_result.reply.reply_kind,
            ...
            return WebhookResponse(
                success=True,
                message="Reasoning core terminal unresolved response sent"
                if terminal_transport_status == "delivered"
                else "Reasoning core terminal unresolved response skipped",
                bot_response=terminal_bot_response,
            )
        ...
        degrade_artifact = _build_runtime_exception_artifact(
        ...
        error_meta_recorded = _record_error_decision_meta(
            ...
            degrade_artifact=degrade_artifact,
        )
        if not error_meta_recorded and should_fallback and message_id:
            for _ in range(15):
