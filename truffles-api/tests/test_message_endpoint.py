...
def test_booking_verification_creates_handover_when_none_active(monkeypatch):
    ...
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Все еще жду подтверждения, соедините с менеджером.",
            messageType="text",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
...
def test_calendar_list_slots_ok_does_not_prepend_redundant_booking_prompt(monkeypatch):
    monkeypatch.setenv("LLM_POLICY_CORE_ENABLED", "1")
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    ...
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Есть ли свободные слоты на маникюр завтра у Айгерим?",
            messageType="text",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-list-slots-ok-no-redundant-prompt",
    ...
    policy_payload = {
        ...
        "action": "fact",
        "tool_action": "calendar.list_slots",
        "tool_args": {"service_query": "Маникюр", "start_at": "2026-02-18T10:00:00"},
        "pack_refs": [],
        "language": "ru",
        "confidence": 0.95,
        "reason": "slots_lookup",
        "goal": "booking",
        "slots": {"service": "Маникюр", "datetime": "2026-02-18T10:00:00"},
        "next_question": None,
        "open_questions": [],
        "needs_manager": False,
        "risk_signals": [],
    ...
    assert "На какую дату и время вам удобно?" not in (response.bot_response or "")
    meta = saved_message.message_metadata.get("decision_meta", {})
    assert meta.get("action") == "reply"
    assert meta.get("intent") == "calendar.list_slots"


def test_calendar_book_slot_conflict_does_not_prepend_redundant_time_prompt(monkeypatch):
    monkeypatch.setenv("LLM_POLICY_CORE_ENABLED", "1")
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    ...
    payload = WebhookRequest(
        ...
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-book-slot-conflict-no-redundant-prompt",
    ...
    policy_payload = {
        ...
        "reason": "book_slot",
        "goal": "booking",
        "slots": {"service": "Маникюр", "datetime": "2026-02-18T10:00:00", "name": "Лена"},
        "next_question": None,
        "open_questions": [],
        "needs_manager": False,
        "risk_signals": [],
    ...
    assert response.success is True
    assert response.bot_response == "Этот слот уже занят. Могу предложить другое время."
    assert "На какую дату и время вам удобно?" not in (response.bot_response or "")
    meta = saved_message.message_metadata.get("decision_meta", {})
    assert meta.get("action") == "reply"
    assert meta.get("intent") == "calendar.book_slot"
    assert meta.get("tool_decision") == "conflict"


def test_calendar_get_booking_ok_does_not_escalate_without_explicit_manager_request(monkeypatch):
    monkeypatch.setenv("LLM_POLICY_CORE_ENABLED", "1")
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    ...
    payload = WebhookRequest(
        ...
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-check-booking-ok-no-auto-handoff",
    ...
    policy_payload = {
        ...
        "action": "fact",
        "tool_action": "calendar.get_booking",
        "tool_args": {"appointment_id": appointment_id},
        "pack_refs": [],
        "language": "ru",
        "confidence": 0.95,
    ...
    with patch(
        ...
        return_value=SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Запись подтверждена: 18.02 в 10:00, мастер Айгерим.",
            error_code=None,
            decision_meta={
                "tool_action": "calendar.get_booking",
                "tool_decision": "ok",
            ...
            trace={"stage": "tool_registry", "decision": "ok", "tool_action": "calendar.get_booking"},
            expected_reply_type=None,
        ),
    ), patch(
        "app.routers.webhook.decision._reuse_active_handover",
    ...
    assert response.success is True
    assert response.bot_response == "Запись подтверждена: 18.02 в 10:00, мастер Айгерим."
    assert response.bot_response != webhook_router.MSG_ESCALATED
    assert reuse_handover_mock.called is False
    assert escalate_mock.called is False
    meta = saved_message.message_metadata.get("decision_meta", {})
    assert meta.get("action") == "reply"
    assert meta.get("intent") == "calendar.get_booking"
    assert meta.get("tool_decision") == "ok"
...
def test_llm_policy_core_degraded_booking_guard_skips_collect_when_slots_complete(monkeypatch):
    monkeypatch.setenv("LLM_POLICY_CORE_ENABLED", "1")
    ...
    payload = WebhookRequest(
        ...
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-llm-policy-core-degraded-slots-complete",
                timestamp=1234567898,
            ),
        ),
    )
    ...
    intent_decomp_state = IntentDecompositionState(
        ...
        intent_decomp_primary="booking",
        intent_decomp_secondary=[],
        intent_decomp_service_query="Маникюр",
        intent_decomp_multi=False,
        intent_decomp_used=True,
        intent_decomp_set={"booking"},
        consult_intent=False,
    ...
    ai_response = WebhookResponse(
        ...
        message="ai_response",
        conversation_id=conversation_id,
        bot_response="SENTINEL_AI_RESPONSE",
    )

    booking_flow_result = SimpleNamespace(
    ...
    with patch(
        ...
        "app.routers.webhook.decision._handle_booking_flow",
        return_value=booking_flow_result,
    ) as booking_flow_mock, patch(
    ...
    assert response.success is True
    assert response.bot_response == "SENTINEL_AI_RESPONSE"
    assert booking_flow_mock.called is True
    meta = saved_message.message_metadata.get("decision_meta", {})
    assert meta.get("policy_core_mode") == "degraded_fallback"
    assert meta.get("policy_core_degrade_reason") == "policy_error:deadline_exceeded"
