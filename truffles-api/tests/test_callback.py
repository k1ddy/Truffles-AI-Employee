...
class TestCallbackSchemas:
    def test_callback_request_valid(self):
        req = CallbackRequest(conversation_id=uuid4(), action="take", manager_id="mgr123", manager_name="John")
        assert req.action == "take"

    def test_callback_request_invalid_action(self):
        with pytest.raises(ValueError):
            CallbackRequest(conversation_id=uuid4(), action="invalid_action", manager_id="mgr123")

    def test_callback_response_valid(self):
        resp = CallbackResponse(
            success=True, conversation_id=uuid4(), action="take", old_state="pending", new_state="manager_active"
        )
