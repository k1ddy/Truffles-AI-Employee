...
def test_send_telegram_notification_retries_media_as_local_upload(
    mock_telegram_class,
    mock_get_or_create_topic,
    mock_download_remote_media,
    mock_refresh_contract,
    tmp_path,
):
    mock_refresh_contract.return_value = None
    mock_get_or_create_topic.return_value = 4321
    local_media_path = tmp_path / "relay.jpg"
    local_media_path.write_bytes(b"fake-image")
    mock_download_remote_media.return_value = (str(local_media_path), None)

    mock_telegram = Mock()
    mock_telegram.send_message.return_value = {"ok": True, "result": {"message_id": 901}}
    mock_telegram.pin_message.return_value = {"ok": True}
    mock_telegram.send_photo.side_effect = [
    ...
    conversation = SimpleNamespace(
        id="conv-3",
        client_id="client-3",
        telegram_topic_id=4321,
        context={},
    )
    user = SimpleNamespace(name="Assel", phone="77015707777", telegram_topic_id=4321)
    ...
    ok = send_telegram_notification(
        ...
        conversation=conversation,
        user=user,
        message="Передайте фото менеджеру",
        routing_meta={"bot_token": "token", "chat_id": "chat-id"},
    )

    assert ok is True
    assert mock_telegram.send_photo.call_count == 2
    first_photo = mock_telegram.send_photo.call_args_list[0].kwargs["photo"]
    second_photo = mock_telegram.send_photo.call_args_list[1].kwargs["photo"]
    assert first_photo == "https://example.com/reference.jpg"
    assert second_photo == str(local_media_path)
    assert local_media_path.exists() is False
    delivery = (handover.meta or {}).get("media_handoff_delivery") or {}
    telegram_delivery = delivery.get("telegram") or {}
    assert telegram_delivery.get("status") == "sent"
    assert telegram_delivery.get("sent_count") == 1
