...
@pytest.mark.asyncio
async def test_reasoning_core_primes_intent_routing_overrides_for_non_secret_delegate(monkeypatch):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Привет",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-intent-bridge-1",
            ),
        ),
    )

    async def _delegate(*args, **kwargs):
        signal_override = ai_service.get_intent_signal_override()
        semantic_override = intent_service.get_intent_semantic_override()
        assert signal_override is not None
        assert semantic_override is not None
        assert signal_override["normalized_text"] == "привет"
        assert signal_override["is_greeting"] is True
        assert semantic_override["intent"] == intent_service.Intent.GREETING.value
        return WebhookResponse(success=True, message="delegated")
    ...
    assert response.success is True
    assert response.message == "delegated"
    assert ai_service.get_intent_signal_override() is None
    assert intent_service.get_intent_semantic_override() is None


@pytest.mark.asyncio
async def test_reasoning_core_primes_protective_lexical_overrides_for_delegate(monkeypatch):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Отпишись",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-protective-bridge-1",
            ),
        ),
    )
    ...
    async def _delegate(*args, **kwargs):
        semantic_override = intent_service.get_intent_semantic_override()
        assert semantic_override is not None
        assert semantic_override["normalized_text"] == "отпишись"
        assert semantic_override["is_opt_out"] is True
        assert semantic_override["is_frustration"] is False
        assert intent_service.is_opt_out_message("Отпишись") is True
        assert intent_service.classify_intent("Отпишись") == intent_service.Intent.REJECTION
        return WebhookResponse(success=True, message="delegated")
...
@pytest.mark.asyncio
async def test_reasoning_core_primes_confirmation_and_refusal_signal_overrides(monkeypatch):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Не хочу оставлять телефон",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-confirm-refusal-bridge-1",
            ),
        ),
    )
    ...
    async def _delegate(*args, **kwargs):
        signal_override = ai_service.get_intent_signal_override()
        assert signal_override is not None
        assert signal_override["normalized_text"] == "не хочу оставлять телефон"
        assert signal_override["confirmation_decision"] == "no"
        assert signal_override["refusal_flags"] == {"name": False, "phone": True}
        assert ai_service.classify_confirmation("Не хочу оставлять телефон") == "no"
        assert ai_service.detect_refusal_flags("Не хочу оставлять телефон") == {
            "name": False,
            "phone": True,
        }
        return WebhookResponse(success=True, message="delegated")
...
@pytest.mark.asyncio
async def test_reasoning_core_primes_domain_routing_override_for_delegate(monkeypatch):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Маникюр",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-domain-bridge-1",
            ),
        ),
    )
    client_id = UUID("00000000-0000-0000-0000-000000000132")
    client = Mock(
        id=client_id,
        config={
            "domain_router": {
    ...
    async def _delegate(*args, **kwargs):
        override = intent_service.get_domain_routing_override()
        assert override is not None
        assert override["normalized_text"] == "маникюр"
        domain_result = intent_service.classify_domain_with_scores(
            "Маникюр",
            {"domain_router": {}},
        )
        assert domain_result[0] == intent_service.DomainIntent.IN_DOMAIN
        assert domain_result[1] > domain_result[2]
        return WebhookResponse(success=True, message="delegated")
    ...
    assert response.success is True
    assert response.message == "delegated"
    assert intent_service.get_domain_routing_override() is None


@pytest.mark.asyncio
async def test_reasoning_core_primes_controller_route_override_for_out_of_domain_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Налоговая отчетность",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-controller-bridge-ood-1",
            ),
        ),
    )
    client_id = UUID("00000000-0000-0000-0000-000000000133")
    client = Mock(
        id=client_id,
        config={
            "domain_router": {
    ...
    async def _delegate(*args, **kwargs):
        controller_override = intent_service.get_dialogue_controller_override()
        assert controller_override is not None
        assert controller_override["normalized_text"] == "налоговая отчетность"
        assert controller_override["class"] == "out_of_domain"
        assert controller_override["goal"] == "out_of_domain"
        return WebhookResponse(success=True, message="delegated")
...
@pytest.mark.asyncio
async def test_reasoning_core_handle_webhook_payload_delegates_session_reset_only_message_past_explicit_handoff_owner(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-session-reset-delegate",
            ),
        ),
    )
    mock_db = Mock()
    ...
    async def _delegate(*args, **kwargs):
        delegate_calls.append(payload.body.message)
        return WebhookResponse(success=True, message="delegated-reset")
    ...
    monkeypatch.setattr(reasoning_core, "_should_prime_ingress_semantic_overrides", lambda *args, **kwargs: True)
    monkeypatch.setattr(
        reasoning_core,
        "_detect_ingress_intent_routing_primitives",
        lambda *args, **kwargs: SimpleNamespace(
            to_ai_signal_override=lambda: None,
            to_intent_override=lambda: None,
        ),
    )
    monkeypatch.setattr(reasoning_core, "_detect_ingress_domain_routing_snapshot", lambda *args, **kwargs: None)
    monkeypatch.setattr(reasoning_core, "detect_controller_route_snapshot", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        reasoning_core,
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: handoff_calls.append(True) or SimpleNamespace(ok=True),
    )
    monkeypatch.setattr(decision_router, "_handle_webhook_payload", _delegate)
    ...
    assert response.success is True
    assert response.message == "delegated-reset"
    assert delegate_calls == ["начнем сначала"]
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_needs_manager_handoff_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Свяжите меня с человеком по этому вопросу",
            metadata=WebhookMetadata(
                remoteJid="77000000061@s.whatsapp.net",
                messageId="msg-semantic-needs-manager-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000931"),
        client_id=UUID("00000000-0000-0000-0000-000000000932"),
        user_id=UUID("00000000-0000-0000-0000-000000000933"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    user = User(
        id=conversation.user_id,
        client_id=conversation.client_id,
        remote_jid="77000000061@s.whatsapp.net",
        created_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
    )
    handover = Mock(id=UUID("00000000-0000-0000-0000-000000000934"), _reopened=False)
    saved_messages: list[Message] = []
    delegate_calls: list[bool] = []
    escalation_metrics: list[tuple[str | None, str]] = []
    ...
    monkeypatch.setattr(reasoning_core, "detect_policy_core_route_snapshot", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        reasoning_core,
        "_ensure_turn_planner_owner_conversation",
        lambda *args, **kwargs: conversation,
    ...
    for owner_name in (
        ...
        "_try_handle_turn_planner_safe_catalog_fact_owner_cutover",
        "_try_handle_turn_planner_safe_service_query_fact_owner_cutover",
        "_try_handle_turn_planner_safe_pricing_collect_owner_cutover",
        "_try_handle_turn_planner_safe_duration_collect_owner_cutover",
        "_try_handle_turn_planner_safe_bookability_time_collect_owner_cutover",
        "_try_handle_turn_planner_safe_active_name_time_collect_owner_cutover",
        "_try_handle_turn_planner_safe_specialist_name_collect_owner_cutover",
        "_try_handle_turn_planner_safe_specialist_datetime_collect_owner_cutover",
        "_try_handle_turn_planner_safe_service_choice_specialist_time_collect_owner_cutover",
        "_try_handle_turn_planner_safe_service_choice_specialist_daypart_collect_owner_cutover",
        "_try_handle_turn_planner_safe_master_query_fact_owner_cutover",
        "_try_handle_turn_planner_safe_master_query_collect_owner_cutover",
        ...
        "_try_handle_turn_planner_safe_initial_booking_prompt_owner_cutover",
    ):
        monkeypatch.setattr(reasoning_core, owner_name, _return_none)
    ...
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "escalate"
    assert user_metadata.get("intent") == "policy_core_guard"
    assert user_metadata.get("source") == "consultant_core_runtime"
    assert user_metadata.get("tool_action") == "handoff"
    assert user_metadata.get("needs_manager") is True
    assert (
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == reasoning_core.REASONING_CORE_TURN_PLANNER_EXPLICIT_HANDOFF_STAGE
        and entry.get("intent") == "policy_core_guard"
        and entry.get("reason") == "semantic_arbitration_needs_manager"
        for entry in trace
    )
    assert conversation.state == "pending"
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_info_owner_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-contact-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000602"),
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (
            ...
            {"info_sections": ["contact"], "resolver_id": "webhook.info_intent"},
        ),
    )
    ...
    assert response.success is True
    assert response.message == "Turn planner safe info fact sent"
    assert response.bot_response == "Телефон: +7 700 000 00 00."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "contact"
    assert user_metadata.get("consultant_core_runtime", {}).get("outcome") == "FACT"
    assert (
        user_metadata.get("consultant_core_runtime", {}).get("owner_replacement_cutover")
        is True
    )
    assert user_metadata.get("turn_outcome", {}).get("tool_decision") == "planner_owner_cutover"
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_catalog_owner_bypasses_frozen_delegate_for_location(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                ...
                messageId="msg-location-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    assert response.bot_response == "Мы находимся на Абая 10."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("intent") == "info"
    assert user_metadata.get("tool_action") == "catalog.location"
    assert (
        user_metadata.get("consultant_core_runtime", {}).get("owner_cutover")
        == reasoning_core.REASONING_CORE_TURN_PLANNER_CATALOG_FACT_OWNER
    )
    assert (
        user_metadata.get("consultant_core_runtime", {}).get("downstream_tool_decision")
        == "ok"
...
@pytest.mark.parametrize(
    "message_text",
    (
        "А какова цена на маникюр?",
        "А какие у вас цены на маникюр?",
    ),
)
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_active_time_pricing_interrupt(
    monkeypatch,
    message_text,
):
    payload = WebhookRequest(
        ...
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                ...
                messageId="msg-booking-prompt-owner-pricing-interrupt-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000725"),
        user_id=UUID("00000000-0000-0000-0000-000000000726"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        assert kwargs.get("service_query") == "Маникюр"
        assert kwargs.get("info_sections_hint") == ["pricing"]
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Маникюр стоит 10 000 тг.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "truth_fallback",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking interrupt tool reply sent"
    assert response.bot_response == "Маникюр стоит 10 000 тг."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert user_metadata.get("action_source") == "tool_registry"
    assert user_metadata.get("tool_action") == "catalog.service_query"
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_info_intents") == ["pricing"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == "active_time_pricing_interrupt"
    assert user_metadata.get("service_query") == "Маникюр"
    assert user_metadata.get("expected_reply_type") == "time"
    ...
    assert runtime_meta.get("owner_cutover") == "turn_executor.tool_reply_turn_outcome.v1"
    assert runtime_meta.get("downstream_tool_decision") == "truth_fallback"
    assert conversation.context.get("expected_reply_type") == "time"
    assert conversation.context.get("expected_reply_reason") == "booking_interrupt"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_prompt_interrupt_contract"
        and entry.get("decision") == "collect_service_info_interrupt"
        and entry.get("service_query") == "Маникюр"
        and entry.get("info_sections") == ["pricing"]
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_interrupt"
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "llm_policy_core_tool"
        and entry.get("source_route") == "booking_prompt_owner"
        and entry.get("tool_action") == "catalog.service_query"
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_ignores_pochemu_weekend_pricing_false_positive(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Почему я не могу записаться на выходные?",
            metadata=WebhookMetadata(
                remoteJid="77000000022@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-pochemu-weekend-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: reasoning_core.ReasoningCoreConversationSnapshot(
            ...
            booking_time_token=None,
            booking_datetime_value=None,
            service_referent="Дизайн ногтей",
        ),
    )
    monkeypatch.setattr(
        reasoning_core,
    ...
    def _execute_tool_action(*_args, **kwargs):
        ...
        raise AssertionError("weekend booking followup must not route through catalog.service_query")
    ...
    assert response.bot_response != "Дизайн ногтей — от 300 ₸."
    assert "₸" not in (response.bot_response or "")
    assert saved_messages
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("tool_action") != "catalog.service_query"
    assert user_metadata.get("booking_prompt_interrupt_recovery") is None
    assert delegate_calls in ([], [True])


@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_recovers_branch_missing_pricing_interrupt(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Какой у вас прайс на стрижку?",
            metadata=WebhookMetadata(
                remoteJid="77000000023@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-pricing-branch-missing-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000728"),
        user_id=UUID("00000000-0000-0000-0000-000000000729"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        assert kwargs.get("service_query") == "Стрижка"
        return SimpleNamespace(
            handled=True,
            ok=False,
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking interrupt tool reply sent"
    assert response.bot_response == "Женская стрижка — 5 000 ₸."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert user_metadata.get("booking_info_intents") == ["pricing"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == "active_time_pricing_interrupt"
    assert user_metadata.get("catalog_service_query_branch_missing_recovered") is True
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "booking_interrupt"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("downstream_tool_decision") == "truth_fallback"
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_prompt_interrupt_branch_recovery"
        and entry.get("decision") == "truth_fallback"
        and entry.get("service_query") == "Стрижка"
        and entry.get("info_sections") == ["pricing"]
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_active_service_pricing_interrupt_and_advances_followup(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="А какая цена на эту стрижку?",
            metadata=WebhookMetadata(
                remoteJid="77000000024@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-pricing-service-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000731"),
    ...
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_info_intents") == ["pricing"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == "active_service_pricing_interrupt"
    assert user_metadata.get("catalog_service_query_branch_missing_recovered") is True
    assert user_metadata.get("service_query") == "Стрижка"
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "booking_interrupt"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("downstream_tool_decision") == "truth_fallback"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_service_grounded_promotions_interrupt_and_advances_to_time(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Есть ли какие-то акции на маникюр в следующем месяце?",
            metadata=WebhookMetadata(
                remoteJid="77000000024@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-promotions-service-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    saved_messages: list[Message] = []
    delegate_calls: list[bool] = []
    continuity_writer_calls: list[dict[str, object]] = []
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        service_query = kwargs.get("service_query")
        assert isinstance(service_query, str)
        assert service_query.strip().casefold() == "маникюр"
        assert kwargs.get("info_sections_hint") == ["promotions"]
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Да, в следующем месяце будет акция на маникюр.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
    ...
    real_set_expected_reply_context = context_manager_router._set_expected_reply_context

    def _track_expected_reply_context(**kwargs):
        continuity_writer_calls.append(
            {
                "expected_reply_type": kwargs.get("expected_reply_type"),
                "reason": kwargs.get("reason"),
            }
        )
        return real_set_expected_reply_context(**kwargs)

    monkeypatch.setattr(
        context_manager_router,
        "_set_expected_reply_context",
        _track_expected_reply_context,
    )

    real_sync = reasoning_core.DialogStateService.build_expected_reply_context_sync_result

    def _track_sync(self, context, **kwargs):
        sync_calls.append(
            {
                "expected_reply_type": kwargs.get("expected_reply_type"),
                "reason": kwargs.get("reason"),
        ...
        return real_sync(self, context, **kwargs)

    monkeypatch.setattr(
        reasoning_core.DialogStateService,
        "build_expected_reply_context_sync_result",
        _track_sync,
    )
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking interrupt tool reply sent"
    assert response.bot_response == "Да, в следующем месяце будет акция на маникюр."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert continuity_writer_calls == []
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "info"
    assert user_metadata.get("tool_action") == "catalog.service_query"
    assert user_metadata.get("action_source") == "tool_registry"
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_info_intents") == ["promotions"]
    assert user_metadata.get("service_query").casefold() == "маникюр"
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "catalog_service_booking_progress"
    assert user_metadata.get("expected_reply_contract_reason") == "catalog_service_booking_progress"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == "turn_executor.tool_reply_turn_outcome.v1"
    assert conversation.context.get("expected_reply_type") == "time"
    assert conversation.context.get("expected_reply_reason") == "catalog_service_booking_progress"
    booking = conversation.context.get("booking") or {}
    assert isinstance(booking.get("service"), str)
    assert booking.get("service").casefold() == "маникюр"
    assert booking.get("last_question") == "datetime"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_prompt_interrupt_contract"
        and entry.get("decision") == "collect_service_info_interrupt"
        and entry.get("service_query").casefold() == "маникюр"
        and entry.get("info_sections") == ["promotions"]
        and entry.get("expected_reply_type") == "time"
        for entry in trace
    )
    assert any(
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_active_time_hours_interrupt_without_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Какое время работы вашего салона?",
            metadata=WebhookMetadata(
                remoteJid="77000000025@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-hours-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000733"),
        client_id=UUID("00000000-0000-0000-0000-000000000734"),
        user_id=UUID("00000000-0000-0000-0000-000000000735"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking interrupt info fact sent"
    assert response.bot_response == "Мы работаем ежедневно с 10:00 до 20:00."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "hours"
    assert user_metadata.get("source") == "consultant_core_runtime"
    assert user_metadata.get("action_source") == "booking_prompt_owner"
    assert user_metadata.get("tool_action") == "info"
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_interrupt_info") is True
    assert user_metadata.get("booking_info_intents") == ["hours"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == "active_time_hours_interrupt"
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "booking_interrupt"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == reasoning_core.REASONING_CORE_TURN_PLANNER_INFO_OWNER
    assert runtime_meta.get("contract_status") == "ok"
    assert conversation.context.get("expected_reply_type") == "time"
    assert conversation.context.get("expected_reply_reason") == "booking_interrupt"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_prompt_interrupt_contract"
        and entry.get("decision") == "generic_info_interrupt"
        and entry.get("info_sections") == ["hours"]
        and entry.get("expected_reply_type") == "time"
        for entry in trace
    )
    assert any(
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == reasoning_core.REASONING_CORE_TURN_PLANNER_INFO_STAGE
        and entry.get("source_route") == "booking_prompt_owner"
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_promotions_interrupt_and_resumes_time_collect(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Есть ли у вас акции на маникюр?",
            metadata=WebhookMetadata(
                remoteJid="77000000025@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-promotions-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000743"),
        client_id=UUID("00000000-0000-0000-0000-000000000744"),
        user_id=UUID("00000000-0000-0000-0000-000000000745"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        "_build_info_intent_reply",
        lambda *args, **kwargs: (
            "Да, сейчас действует скидка 10% на маникюр для новых гостей.",
            {
                "fact_source": "truth",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking interrupt tool reply sent"
    assert response.bot_response == "Да, сейчас действует скидка 10% на маникюр для новых гостей."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_info_intents") == ["promotions"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == (
        "active_time_promotions_interrupt"
    )
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "catalog_service_booking_progress"
    assert user_metadata.get("expected_reply_contract_reason") == "catalog_service_booking_progress"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_answers_explicit_hours_interrupt(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Каковы часы работы салона?",
            metadata=WebhookMetadata(
                remoteJid="77000000025@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-explicit-hours-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000746"),
        client_id=UUID("00000000-0000-0000-0000-000000000747"),
        user_id=UUID("00000000-0000-0000-0000-000000000748"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    assert not any(
        isinstance(entry, dict)
        and entry.get("stage") == "booking_interrupt"
        and entry.get("info_sections") == ["duration"]
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_info_owner_defers_active_service_hours_interrupt_to_booking_prompt_owner(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="А по выходным вы работаете?",
            metadata=WebhookMetadata(
                remoteJid="77000000026@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-service-hours-followup-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    monkeypatch.setattr(
        reasoning_core,
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="а по выходным вы работаете",
            intent="hours",
            action="fact",
            tool_action="info",
            confidence=0.95,
    ...
    assert user_metadata.get("booking_interrupt_info") is True
    assert user_metadata.get("booking_info_intents") == ["hours"]
    assert user_metadata.get("booking_prompt_interrupt_recovery") == "active_service_hours_interrupt"
    ...
    assert any(
        ...
        and entry.get("decision") == "generic_info_interrupt"
        and entry.get("info_sections") == ["hours"]
        and entry.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_pending_boundary_promotions_interrupt_defers_to_booking_prompt_owner(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                ...
                messageId="msg-booking-prompt-owner-pending-boundary-promotions-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-00000000073a"),
        user_id=UUID("00000000-0000-0000-0000-00000000073b"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
        context={
            "pending_resume": {
                ...
                "session_memory": {
                    "active_goal": "booking",
                    "last_question_type": decision_router.EXPECTED_REPLY_TIME,
                },
            },
        },
    )
    ...
    monkeypatch.setattr(
        reasoning_core,
        "_resolve_turn_planner_pending_booking_resume_boundary_payload",
        lambda *args, **kwargs: reasoning_core.DialogStateService().derive_pending_booking_resume_boundary_payload(
    ...
    monkeypatch.setattr(
        ...
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="есть ли какие то акции на маникюр в следующем месяце",
            intent="promotions",
            action="fact",
            tool_action="info",
            confidence=0.95,
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=AssertionError(
                "pending boundary promotions interrupt must not fall through explicit handoff"
            )
        ),
    )
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_direct_catalog_fact_defers_active_booking_interrupt(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Я слышал, что у вас есть акция на маникюр.",
            metadata=WebhookMetadata(
                remoteJid="77000000026@s.whatsapp.net",
                messageId="msg-direct-catalog-fact-defers-active-booking-interrupt-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000749"),
        client_id=UUID("00000000-0000-0000-0000-000000000750"),
    ...
    monkeypatch.setattr(
        ...
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="я слышал что у вас есть акция на маникюр",
    ...
    def _execute_tool_action(*_args, **_kwargs):
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Мы салон красоты: маникюр, педикюр и брови.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "services_overview",
    ...
    async def _stub_booking_prompt_owner(*args, **kwargs):
        return WebhookResponse(
            success=True,
            message="booking prompt owner took over",
            conversation_id=conversation.id,
            bot_response="promo-owner",
        )
    ...
    monkeypatch.setattr(reasoning_core, "save_message", lambda *args, **kwargs: Message())
    monkeypatch.setattr(reasoning_core, "get_instance_id", lambda *args, **kwargs: "inst-1")
    monkeypatch.setattr(reasoning_core, "send_message_safe", lambda *args, **kwargs: Ok("ok"))
    monkeypatch.setattr(decision_router, "_handle_webhook_payload", _return_none)
    ...
    assert response.success is True
    assert response.message == "booking prompt owner took over"
    assert response.bot_response == "promo-owner"


@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_direct_service_query_fact_defers_active_booking_interrupt(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000026@s.whatsapp.net",
                messageId="msg-direct-service-query-fact-defers-active-booking-interrupt-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000753"),
    ...
    monkeypatch.setattr(
        ...
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="почему я не могу записаться на выходные",
            intent="info",
            action="fact",
            tool_action="catalog.service_query",
            confidence=0.95,
            ...
            needs_manager=False,
            goal="info",
            tool_args={"service_query": "Дизайн ногтей"},
            pack_refs=("pricing",),
            slots={"service": "Дизайн ногтей"},
        ),
    )
    ...
    def _execute_tool_action(*_args, **_kwargs):
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Дизайн ногтей стоит от 3000 тг.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "truth_fallback",
            ...
            trace={"stage": "tool_registry", "decision": "truth_fallback"},
            expected_reply_type=None,
        )
    ...
    assert response.success is True
    assert response.message == "booking prompt owner took over"
    assert response.bot_response == "weekend-owner"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_booking_completion_owner_preserves_specialist_missing_name_followup(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Я бы хотел специалиста Асем.",
            metadata=WebhookMetadata(
                remoteJid="77000000034@s.whatsapp.net",
                messageId="msg-booking-completion-owner-cutover-specialist-missing",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000784"),
        client_id=UUID("00000000-0000-0000-0000-000000000785"),
        user_id=UUID("00000000-0000-0000-0000-000000000786"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: reasoning_core.ReasoningCoreConversationSnapshot(
            ...
            allow_bot_reply=True,
            resume_reason="booking_prompt",
            booking_time_token="2026-03-10T15:00:00+05:00",
            booking_datetime_value="2026-03-10T15:00:00+05:00",
            service_referent="Маникюр",
        ),
    )
    monkeypatch.setattr(
    ...
    def _execute_tool_action(*_args, **kwargs):
        ...
        return ToolExecutionResult(
            handled=True,
            ok=False,
            response_text="Не нашла такого мастера. Уточните, пожалуйста.",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe booking completion owner sent"
    assert response.bot_response == "Не нашла такого мастера. Уточните, пожалуйста."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert captured_tool_call.get("tool_action") == "calendar.book_slot"
    tool_args = captured_tool_call.get("tool_args") or {}
    assert tool_args.get("service_query") == "Маникюр"
    assert tool_args.get("start_at") == "2026-03-10T15:00:00+05:00"
    assert tool_args.get("customer_name") == "Асем"
    assert tool_args.get("specialist_name") == "Асем"
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("source") == "tool_registry"
    assert user_metadata.get("tool_action") == "calendar.book_slot"
    assert user_metadata.get("tool_decision") == "specialist_missing"
    ...
    assert user_metadata.get("active_question_relation") == "tool_result_followup_specialist_missing"
    assert user_metadata.get("expected_reply_type") == "name"
    assert user_metadata.get("expected_reply_reason") == "calendar_book_slot_specialist_followup"
    assert user_metadata.get("specialist_name") == "Асем"
    booking = conversation.context.get("booking") or {}
    assert booking.get("service") == "Маникюр"
    assert booking.get("datetime") == "2026-03-10T15:00:00+05:00"
    assert booking.get("specialist_name") == "Асем"
    assert booking.get("last_question") == "name"
    assert not booking.get("name")
    assert conversation.context.get("expected_reply_type") == "name"
    assert conversation.context.get("expected_reply_reason") == "calendar_book_slot_specialist_followup"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "pending_question_interaction"
        ...
        and entry.get("tool_action") == "calendar.book_slot"
        and entry.get("tool_decision") == "specialist_missing"
        and entry.get("pending_question_target") == "specialist"
        and entry.get("active_question_relation") == "tool_result_followup_specialist_missing"
        and entry.get("expected_reply_type") == "name"
        and entry.get("specialist_name") == "Асем"
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_tool_reply_pending_question_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Есть ли свободные слоты на этой неделе?",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-semantic-tool-reply-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000552"),
        user_id=UUID("00000000-0000-0000-0000-000000000553"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "payload": {
                ...
                "tool_args": {"service_query": "Маникюр"},
                "pack_refs": [],
                "reason": "slot_question_live_calendar",
                "goal": "booking",
                "slots": {},
                "next_question": "datetime",
                "open_questions": ["datetime"],
    ...
    assert response.success is True
    assert response.message == "Turn planner safe semantic tool reply sent"
    ...
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "calendar.list_slots"
    assert user_metadata.get("source") == "tool_registry"
    assert user_metadata.get("action_source") == "tool_registry"
    assert user_metadata.get("tool_action") == "calendar.list_slots"
    assert user_metadata.get("tool_decision") == "missing_slot"
    assert user_metadata.get("pending_question_act") == "ask_about_requested_slot"
    assert user_metadata.get("pending_question_target") == "time"
    assert user_metadata.get("pending_question_interaction") == "ask_about_requested_slot"
    assert user_metadata.get("pending_question_owner") == "booking_slot_guidance"
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "booking_slot_guidance"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == "turn_executor.tool_reply_turn_outcome.v1"
    assert runtime_meta.get("downstream_tool_decision") == "missing_slot"
    assert runtime_meta.get("contract_status") == "ok"
    turn_outcome = user_metadata.get("turn_outcome") or {}
    assert turn_outcome.get("tool_decision") == "missing_slot"
    assert turn_outcome.get("expected_reply_type") == "time"
    assert turn_outcome.get("expected_reply_reason") == "booking_slot_guidance"
    assert conversation.context.get("expected_reply_type") == "time"
    assert conversation.context.get("expected_reply_reason") == "booking_slot_guidance"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "pending_question_interaction"
        and entry.get("decision") == "booking_slot_guidance"
        and entry.get("source") == "tool_registry"
        and entry.get("tool_action") == "calendar.list_slots"
        and entry.get("tool_decision") == "missing_slot"
    ...
    assert any(
        ...
        and entry.get("decision") == "set"
        and entry.get("expected_reply_type") == "time"
        and entry.get("reason") == "booking_slot_guidance"
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_service_info_interrupt_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Какие услуги по стрижке волос вы предлагаете?",
            metadata=WebhookMetadata(
                remoteJid="77000000010@s.whatsapp.net",
                messageId="msg-semantic-service-info-interrupt-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000562"),
        user_id=UUID("00000000-0000-0000-0000-000000000563"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "payload": {
                ...
                "action": "collect",
                "tool_action": "collect",
                "tool_args": {"service_query": "стрижка волос"},
                "pack_refs": [],
                "reason": "active_time_service_info_interrupt",
                "goal": "booking",
                "slots": {"service": "", "datetime": ""},
                "next_question": "service",
                "open_questions": ["service"],
                "needs_manager": False,
                "risk_signals": [],
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        service_query = kwargs.get("service_query") or ""
        assert isinstance(service_query, str)
        assert service_query.casefold().startswith("стриж")
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="По стрижке у нас есть женская, мужская и детская стрижка.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "services_overview",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe semantic tool reply sent"
    assert response.bot_response == "По стрижке у нас есть женская, мужская и детская стрижка."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "catalog.service_query"
    assert user_metadata.get("source") == "tool_registry"
    assert user_metadata.get("action_source") == "tool_registry"
    assert user_metadata.get("tool_action") == "catalog.service_query"
    assert user_metadata.get("tool_decision") == "services_overview"
    assert user_metadata.get("booking_info_interrupt") is True
    assert user_metadata.get("booking_interrupt_info") is True
    assert user_metadata.get("booking_info_intents") == ["services_overview"]
    assert user_metadata.get("expected_reply_type") == "time"
    assert user_metadata.get("expected_reply_reason") == "booking_interrupt"
    assert user_metadata.get("policy_collect_guard_recovery") == "active_time_service_info_interrupt"
    assert (
        user_metadata.get("policy_semantic_override_block_reason")
        == "policy_collect_service_info_interrupt_owner"
    )
    assert user_metadata.get("service_query") == "стрижка волос"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == "turn_executor.tool_reply_turn_outcome.v1"
    assert runtime_meta.get("downstream_tool_decision") == "services_overview"
    assert runtime_meta.get("contract_status") == "ok"
    turn_outcome = user_metadata.get("turn_outcome") or {}
    assert turn_outcome.get("tool_action") == "catalog.service_query"
    assert turn_outcome.get("tool_decision") == "services_overview"
    assert turn_outcome.get("expected_reply_type") == "time"
    assert turn_outcome.get("expected_reply_reason") == "booking_interrupt"
    assert conversation.context.get("expected_reply_type") == "time"
    assert conversation.context.get("expected_reply_reason") == "booking_interrupt"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "llm_policy_plan_delta"
        and entry.get("decision") == "override_event"
        and entry.get("from_tool_action") == "collect"
        and entry.get("to_tool_action") == "catalog.service_query"
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_interrupt_contract"
        and entry.get("decision") == "collect_service_info_interrupt"
        and isinstance(entry.get("service_query"), str)
        and entry.get("service_query").casefold().startswith("стриж")
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        ...
        and entry.get("decision") == "info_reply"
        and entry.get("booking_interrupt_info") is True
        and entry.get("info_sections") == ["services_overview"]
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_master_override_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000011@s.whatsapp.net",
                messageId="msg-semantic-master-override-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    monkeypatch.setattr(
        reasoning_core,
        "resolve_master_intent",
        lambda *args, **kwargs: SimpleNamespace(explicit=True),
    )
    monkeypatch.setattr(
        decision_router,
    ...
    monkeypatch.setattr(
        ...
        "_build_info_intent_reply",
        lambda *args, **kwargs: (
            "По мастерам: Айгерим делает маникюр.",
            {"info_sections": ["master"]},
        ),
    )
    ...
    assert response.success is True
    assert response.message == "Turn planner safe semantic tool reply sent"
    assert response.bot_response == "По мастерам: Айгерим делает маникюр."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "reply"
    assert user_metadata.get("intent") == "master"
    assert user_metadata.get("source") == "policy_core_guard"
    assert user_metadata.get("tool_action") == "catalog.service_query"
    assert "master" in (user_metadata.get("info_sections") or [])
    assert user_metadata.get("policy_semantic_override_blocked") is True
    assert user_metadata.get("policy_semantic_override_block_reason") == "master_signal_override_blocked"
    assert user_metadata.get("policy_semantic_override_enforced") is True
    assert (
        user_metadata.get("policy_semantic_override_enforced_reason")
        == "master_signal_override_blocked"
    )
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == "turn_executor.tool_reply_turn_outcome.v1"
    assert runtime_meta.get("downstream_tool_decision") == "master_signal_override_blocked"
    turn_outcome = user_metadata.get("turn_outcome") or {}
    assert turn_outcome.get("tool_action") == "catalog.service_query"
    assert turn_outcome.get("tool_decision") == "master_signal_override_blocked"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "llm_policy_semantic_delta"
        and entry.get("decision") == "semantic_override_blocked"
        and entry.get("reason") == "master_signal_override_blocked"
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_guard"
        and entry.get("decision") == "master_pack_enforced"
        and entry.get("reason_code") == "master_signal_override_blocked"
        and entry.get("service_query") == "маникюр"
        for entry in trace
    )
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_duration_service_query_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Я хочу понять, сколько по времени занимает маникюр у вас.",
            metadata=WebhookMetadata(
                remoteJid="77000000012@s.whatsapp.net",
                messageId="msg-semantic-duration-service-query-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000581"),
        client_id=UUID("00000000-0000-0000-0000-000000000582"),
        user_id=UUID("00000000-0000-0000-0000-000000000583"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    def _execute_tool_action(*_args, **kwargs):
        ...
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="Маникюр занимает около 90 минут.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "duration",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe semantic service-query fact sent"
    assert response.bot_response == "Маникюр занимает около 90 минут."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert user_metadata.get("action_source") == "semantic_arbitration"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == reasoning_core.REASONING_CORE_TURN_PLANNER_SERVICE_QUERY_FACT_OWNER
    assert runtime_meta.get("downstream_tool_decision") == "duration"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == reasoning_core.REASONING_CORE_TURN_PLANNER_SERVICE_QUERY_FACT_STAGE
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_pricing_service_query_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Сколько стоит маникюр?",
            metadata=WebhookMetadata(
                remoteJid="77000000013@s.whatsapp.net",
                messageId="msg-semantic-pricing-service-query-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000591"),
        client_id=UUID("00000000-0000-0000-0000-000000000592"),
        user_id=UUID("00000000-0000-0000-0000-000000000593"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        reasoning_core,
        "resolve_master_intent",
        lambda *args, **kwargs: SimpleNamespace(explicit=False),
    )
    monkeypatch.setattr(
        decision_router,
        "_has_explicit_location_or_hours_request",
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_service_query_fact_rescue_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Сколько стоит маникюр?",
            metadata=WebhookMetadata(
                remoteJid="77000000015@s.whatsapp.net",
                messageId="msg-semantic-service-query-fact-rescue-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000595"),
        client_id=UUID("00000000-0000-0000-0000-000000000596"),
        user_id=UUID("00000000-0000-0000-0000-000000000597"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="pending",
    ...
    saved_messages: list[Message] = []
    route_policy_calls: list[dict[str, object]] = []
    delegate_calls: list[bool] = []
    ...
    monkeypatch.setattr(decision_router, "POLICY_CORE_RESCUE_MATRIX_ENABLED", True)
    monkeypatch.setattr(decision_router, "POLICY_CORE_RESCUE_TIMEOUT_SECONDS", 1.5)

    def _route_policy_core(*_args, **kwargs):
        route_policy_calls.append(dict(kwargs))
    ...
    monkeypatch.setattr(reasoning_core, "route_llm_policy_core", _route_policy_core)
    monkeypatch.setattr(
        reasoning_core,
        "resolve_master_intent",
        lambda *args, **kwargs: SimpleNamespace(explicit=False),
    ...
    rescue_timing_context = next(
        (
            call.get("timing_context")
            for call in route_policy_calls
            if isinstance(call.get("timing_context"), dict)
        ),
        None,
    )
    ...
    assert user_metadata.get("source") == "tool_registry"
    assert user_metadata.get("action_source") == "semantic_arbitration"
    llm_policy_meta = user_metadata.get("llm_policy_core") or {}
    assert llm_policy_meta.get("rescue_attempted") is True
    assert llm_policy_meta.get("rescue_applied") is True
    assert llm_policy_meta.get("rescue_trigger_error") == "invalid_schema"
    runtime_meta = user_metadata.get("consultant_core_runtime") or {}
    assert runtime_meta.get("owner_cutover") == reasoning_core.REASONING_CORE_TURN_PLANNER_SERVICE_QUERY_FACT_OWNER
    assert runtime_meta.get("downstream_tool_decision") == "truth_fallback"
    assert mock_db.commit.call_count == 1
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_semantic_services_overview_service_query_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Какие стрижки у вас есть?",
            metadata=WebhookMetadata(
                remoteJid="77000000014@s.whatsapp.net",
                messageId="msg-semantic-services-overview-service-query-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000594"),
        client_id=UUID("00000000-0000-0000-0000-000000000595"),
        user_id=UUID("00000000-0000-0000-0000-000000000596"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "payload": {
                ...
                "tool_args": {"service_query": "стрижка волос"},
                "pack_refs": [],
                "reason": "services_overview_query",
                "goal": "info",
                "slots": {"service": "стрижка волос"},
                "next_question": None,
                "open_questions": [],
                "needs_manager": False,
                "risk_signals": [],
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        assert kwargs.get("service_query") == "стрижка волос"
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="По стрижкам у нас есть женская, мужская и детская стрижка.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "services_overview",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe semantic tool reply sent"
    assert response.bot_response == "По стрижкам у нас есть женская, мужская и детская стрижка."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert turn_outcome.get("tool_action") == "catalog.service_query"
    assert turn_outcome.get("tool_decision") == "services_overview"
    assert turn_outcome.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
    assert turn_outcome.get("expected_reply_reason") == "services_overview"
    assert turn_outcome.get("meta", {}).get("services_overview_followup") is True
    assert conversation.context.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
    assert conversation.context.get("expected_reply_reason") == "services_overview"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "llm_policy_core_tool"
        and entry.get("source_route") == "llm_policy_core_semantic_arbitration"
        and entry.get("tool_action") == "catalog.service_query"
        and entry.get("tool_decision") == "services_overview"
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        ...
        and entry.get("decision") == "set"
        and entry.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
        and entry.get("reason") == "services_overview"
        for entry in trace
    )


async def test_reasoning_core_turn_planner_safe_timeout_services_overview_fallback_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000016@s.whatsapp.net",
                messageId="msg-timeout-services-overview-fallback-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000598"),
        client_id=UUID("00000000-0000-0000-0000-000000000599"),
        user_id=UUID("00000000-0000-0000-0000-000000000600"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    def _execute_tool_action(*_args, **kwargs):
        assert kwargs.get("tool_action") == "catalog.service_query"
        assert kwargs.get("service_query") is None
        assert kwargs.get("message_text") == "Какие услуги у вас есть?"
        return SimpleNamespace(
            handled=True,
            ok=True,
            response_text="У нас есть маникюр, педикюр и стрижки.",
            error_code=None,
            decision_meta={
                "tool_action": "catalog.service_query",
                "tool_decision": "services_overview",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe timeout services overview fallback sent"
    assert response.bot_response == "У нас есть маникюр, педикюр и стрижки."
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    assert user_metadata.get("tool_action") == "catalog.service_query"
    assert user_metadata.get("tool_decision") == "services_overview"
    assert user_metadata.get("policy_core_guard_recovery") == "services_overview"
    assert user_metadata.get("policy_core_mode") == "degraded_fallback"
    assert user_metadata.get("policy_core_degrade_reason") == "policy_error:timeout"
    assert conversation.context.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
    assert conversation.context.get("expected_reply_reason") == "services_overview"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_core_guard"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_timeout_active_name_time_collect_owner_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000017@s.whatsapp.net",
                messageId="msg-timeout-active-name-time-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    ...
    assert user_metadata.get("turn_outcome", {}).get("expected_reply_type") == decision_router.EXPECTED_REPLY_NAME
    assert (
        user_metadata.get("turn_outcome", {}).get("expected_reply_reason")
    ...
    assert any(
        ...
        and entry.get("reason") == "booking_time_availability_followup"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_timeout_specialist_followup_owner_bypasses_frozen_delegate_for_time_collect(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Я хочу записаться к Айгерим.",
            metadata=WebhookMetadata(
                remoteJid="77000000057@s.whatsapp.net",
                messageId="msg-timeout-specialist-followup-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000912"),
        user_id=UUID("00000000-0000-0000-0000-000000000913"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "specialist_name": "Айгерим",
            "confidence": 0.99,
            "reason": "explicit specialist name",
            "language": "ru",
            "error": None,
            "raw": "{\"specialist_name\":\"Айгерим\"}",
            "attempted": True,
            "elapsed_ms": 12.0,
        },
    )
    ...
    assert response.success is True
    assert response.message == "Turn planner safe timeout specialist followup owner sent"
    assert response.bot_response == decision_router.MSG_BOOKING_ASK_DATETIME
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    ...
    assert user_metadata.get("pending_question_target") == "specialist"
    assert user_metadata.get("pending_question_interaction") == "specialist_followup"
    assert user_metadata.get("pending_question_owner") == "policy_core_timeout_specialist_followup"
    assert user_metadata.get("specialist_name") == "Айгерим"
    assert user_metadata.get("expected_reply_type") == decision_router.EXPECTED_REPLY_TIME
    assert user_metadata.get("expected_reply_reason") == "policy_core_timeout_specialist_followup"
    assert user_metadata.get("policy_core_guard_recovery") == "timeout_specialist_followup"
    assert user_metadata.get("policy_core_timeout_retry_path") == "booking_collect_specialist_followup"
    assert conversation.context.get("expected_reply_type") == decision_router.EXPECTED_REPLY_TIME
    assert conversation.context.get("expected_reply_reason") == "policy_core_timeout_specialist_followup"
    booking = conversation.context.get("booking") or {}
    assert booking.get("service") == "Маникюр"
    assert booking.get("specialist_name") == "Айгерим"
    assert booking.get("last_question") == "datetime"
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "specialist_hint"
        and entry.get("decision") == "ok"
        and entry.get("tool_action") == "collect"
        for entry in trace
    )
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_core_guard"
        and entry.get("decision") == "timeout_specialist_followup"
        and entry.get("reason") == "policy_error:timeout"
        and entry.get("missing_slot") == "datetime"
        for entry in trace
    )
    assert any(
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_timeout_active_name_specialist_followup_owner_bypasses_frozen_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Могу выбрать Айгерим?",
            metadata=WebhookMetadata(
                remoteJid="77000000058@s.whatsapp.net",
                messageId="msg-timeout-active-name-specialist-followup-owner-cutover-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        ...
        client_id=UUID("00000000-0000-0000-0000-000000000922"),
        user_id=UUID("00000000-0000-0000-0000-000000000923"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: reasoning_core.ReasoningCoreConversationSnapshot(
            ...
            allow_bot_reply=True,
            resume_reason="booking_prompt",
            booking_datetime_value="2026-03-13T15:00:00",
            service_referent="Маникюр",
        ),
    )
    monkeypatch.setattr(
    ...
    assert response.success is True
    assert response.message == "Turn planner safe timeout active-name specialist followup owner sent"
    assert "Айгерим" in (response.bot_response or "")
    assert decision_router.MSG_BOOKING_ASK_NAME in (response.bot_response or "")
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    ...
    booking = conversation.context.get("booking") or {}
    assert booking.get("service") == "Маникюр"
    assert booking.get("datetime") == "2026-03-13T15:00:00"
    assert booking.get("specialist_name") == "Айгерим"
    assert booking.get("last_question") == "name"
    trace = conversation.context.get("decision_trace", [])
    assert any(
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_booking_prompt_owner_reactivates_pending_collect_without_active_booking_snapshot(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="На какое время лучше записаться?",
            metadata=WebhookMetadata(
                remoteJid="77000000041@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-pending-reactivation",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-0000000006c1"),
        client_id=UUID("00000000-0000-0000-0000-0000000006c2"),
        user_id=UUID("00000000-0000-0000-0000-0000000006c3"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="pending",
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=AssertionError(
                "touched family must not fall through explicit handoff fallback authority"
            )
        ),
    )
    real_set_expected_reply_context = context_manager_router._set_expected_reply_context
    ...
    assert len(policy_core_calls) == 1
    assert policy_core_calls[0]["expected_reply_type"] == decision_router.EXPECTED_REPLY_SERVICE
    assert policy_core_calls[0]["current_goal"] is None
    assert isinstance(policy_core_calls[0]["slot_state"], dict)
    assert policy_core_calls[0]["info_refs"] == sorted(decision_router.INFO_INTENTS)
    assert policy_core_calls[0]["client_slug"] == "demo_salon"
    assert policy_core_calls[0]["max_tokens_override"] is None
    assert sync_calls == [
    ...
    assert continuity_writer_calls == []
    assert conversation.context.get("booking", {}).get("datetime") == "20:00"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "collect_owner_reactivation"
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_pending_booking_reentry_preempts_explicit_handoff_without_boundary_payload(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000042@s.whatsapp.net",
                messageId="msg-booking-prompt-owner-pending-reentry-before-handoff",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-0000000006c4"),
        client_id=UUID("00000000-0000-0000-0000-0000000006c5"),
        user_id=UUID("00000000-0000-0000-0000-0000000006c6"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="pending",
    ...
    continuity_writer_calls: list[dict[str, object]] = []
    handover = SimpleNamespace(
        id=UUID("00000000-0000-0000-0000-0000000006c7"),
        status="pending",
    )
    ...
    monkeypatch.setattr(
        ...
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="на какое время лучше записаться",
            intent="policy_core_guard",
            action="handoff",
            tool_action="handoff",
            confidence=0.0,
            reason="terminal_owner_unresolved",
            needs_manager=True,
            goal="handoff",
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=AssertionError(
                "pending booking reentry must reach booking_prompt_owner before explicit handoff"
            )
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=WebhookResponse(success=True, message="delegated")),
    )
    ...
    assert any(
        ...
        and entry.get("stage") == "question_contract"
        and entry.get("expected_reply_type") == decision_router.EXPECTED_REPLY_SERVICE
        and entry.get("reason") == "booking_prompt"
        for entry in trace
    )


@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_initial_booking_owner_recovers_invalid_schema_before_terminal_handoff(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000033@s.whatsapp.net",
                messageId="msg-initial-booking-invalid-schema-recovery-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-0000000006e4"),
        client_id=UUID("00000000-0000-0000-0000-0000000006e5"),
        user_id=UUID("00000000-0000-0000-0000-0000000006e6"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    monkeypatch.setattr(
        ...
        "detect_policy_core_route_snapshot",
        lambda *args, **kwargs: reasoning_core.PolicyCoreRouteSnapshot(
            normalized_text="я хочу записаться на маникюр",
            intent="policy_core_guard",
            action="handoff",
            tool_action="handoff",
            confidence=0.0,
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "payload": {
                ...
                "subject_kind": "booking",
                "temporal_scope": "general",
                "resolution_mode": "clarify_missing_datetime",
                "pending_question_act": None,
                "pending_question_target": None,
                "active_question_relation": None,
                "needs_manager": False,
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=AssertionError(
                "initial booking owner must recover invalid_schema before explicit handoff"
            )
        ),
    )
    ...
    assert user_metadata.get("expected_reply_reason") == "booking_prompt"
    assert user_metadata.get("policy_core_mode") == "degraded_fallback"
    assert user_metadata.get("policy_core_degrade_reason") == "policy_error:invalid_schema"
    assert user_metadata.get("policy_core_guard_recovery") == "invalid_schema_collect_contract"
    assert conversation.context.get("booking", {}).get("service") == "Маникюр"
    assert conversation.context.get("booking", {}).get("last_question") == "datetime"
    assert conversation.context.get("expected_reply_type") == decision_router.EXPECTED_REPLY_TIME
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_core_guard"
        and entry.get("decision") == "invalid_schema_initial_booking_collect"
        and entry.get("recovery") == "invalid_schema_collect_contract"
        and entry.get("reason") == "policy_error:invalid_schema"
        and entry.get("missing_slot") == "datetime"
        for entry in trace
    )
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_initial_booking_owner_recovers_timeout_before_terminal_handoff(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000031@s.whatsapp.net",
                messageId="msg-initial-booking-timeout-recovery-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-0000000006d1"),
        client_id=UUID("00000000-0000-0000-0000-0000000006d2"),
        user_id=UUID("00000000-0000-0000-0000-0000000006d3"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    assert user_metadata.get("policy_core_mode") == "degraded_fallback"
    assert user_metadata.get("policy_core_degrade_reason") == "policy_error:timeout"
    assert user_metadata.get("policy_core_guard_recovery") == "initial_booking_parser"
    assert conversation.context.get("booking", {}).get("service") == "Маникюр"
    ...
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == "policy_core_guard"
        and entry.get("decision") == "timeout_initial_booking_collect"
        and entry.get("recovery") == "initial_booking_parser"
        and entry.get("reason") == "policy_error:timeout"
        and entry.get("missing_slot") == "datetime"
        for entry in trace
    )
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_initial_booking_owner_recovers_service_only_timeout_before_terminal_handoff(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000032@s.whatsapp.net",
                messageId="msg-initial-booking-timeout-recovery-service-only-1",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-0000000006e1"),
        client_id=UUID("00000000-0000-0000-0000-0000000006e2"),
        user_id=UUID("00000000-0000-0000-0000-0000000006e3"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
...
@pytest.mark.asyncio
async def test_reasoning_core_manager_active_snapshot_skips_ingress_semantic_overrides(monkeypatch):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            message="Привет",
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-snapshot-bridge-1",
            ),
        ),
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("message_text", "expected_intent", "expected_reply"),
    [
        ("Привет", "greeting", ai_service.GREETING_RESPONSE),
        ("Спасибо", "thanks", ai_service.THANKS_RESPONSE),
        ("ок", "ack", ai_service.ACKNOWLEDGEMENT_RESPONSE),
    ],
)
async def test_reasoning_core_turn_planner_safe_greeting_owner_family_bypasses_frozen_delegate(
    monkeypatch,
    message_text,
    expected_intent,
    expected_reply,
    ...
    payload = WebhookRequest(
        ...
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId=f"msg-greeting-owner-{expected_intent}",
            ),
        ),
    )
    mock_db = Mock()
    conversation = Conversation(
        id=UUID("00000000-0000-0000-0000-000000000145"),
        client_id=UUID("00000000-0000-0000-0000-000000000245"),
        user_id=UUID("00000000-0000-0000-0000-000000000345"),
        channel="whatsapp",
        status="active",
        started_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
        state="bot_active",
    ...
    assert response.success is True
    assert response.message == "Turn planner safe greeting owner sent"
    assert response.bot_response == expected_reply
    assert response.conversation_id == conversation.id
    assert delegate_calls == []
    assert [message.role for message in saved_messages] == ["user", "assistant"]
    user_metadata = saved_messages[0].message_metadata.get("decision_meta") or {}
    assert user_metadata.get("action") == "smalltalk"
    assert user_metadata.get("intent") == expected_intent
    assert user_metadata.get("source") == "consultant_core_runtime"
    assert (
        user_metadata.get("consultant_core_runtime", {}).get("owner_cutover")
        == reasoning_core.REASONING_CORE_TURN_PLANNER_GREETING_OWNER
    )
    assert (
        user_metadata.get("consultant_core_runtime", {}).get("downstream_tool_decision")
        == expected_intent
    ...
    turn_outcome = user_metadata.get("turn_outcome") or {}
    assert turn_outcome.get("action") == "smalltalk"
    assert turn_outcome.get("intent") == expected_intent
    assert turn_outcome.get("tool_action") == "smalltalk"
    assert turn_outcome.get("tool_decision") == "planner_owner_cutover"
    assert turn_outcome.get("observability", {}).get("transport_status") == "delivered"
    trace = conversation.context.get("decision_trace", [])
    assert any(
        isinstance(entry, dict)
        and entry.get("stage") == reasoning_core.REASONING_CORE_TURN_PLANNER_GREETING_STAGE
        and entry.get("smalltalk_intent") == expected_intent
        and entry.get("controller_class") == "greeting"
        for entry in trace
    )
    assert ai_service.get_intent_signal_override() is None
    assert intent_service.get_intent_semantic_override() is None
...
@pytest.mark.asyncio
async def test_reasoning_core_turn_planner_safe_greeting_owner_family_skips_out_of_domain_delegate(
    monkeypatch,
):
    payload = WebhookRequest(
        client_slug="demo_salon",
        body=WebhookBody(
            ...
            metadata=WebhookMetadata(
                remoteJid="77000000000@s.whatsapp.net",
                messageId="msg-greeting-owner-ood-delegate",
            ),
        ),
    )
