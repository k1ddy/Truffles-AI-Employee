...
| Компонент | Статус |
|-----------|--------|
| Сохранение ответа менеджера | ⚠️ ЧАСТИЧНО (поле есть, не заполняется) |
| Модерация | 📋 ПЛАН |
| Добавление в Qdrant | 📋 ПЛАН |
...
- `handover.manager_response` — поле для ответа менеджера (пока не заполняется)
...
manager_message_service.process_manager_message():
  ...
  - ⚠️ НУЖНО: сохранить manager_response в handover
    ↓
Ответ доставлен клиенту
```

**Что нужно добавить в `manager_message_service.py`:**
```python
# После отправки в WhatsApp
handover.manager_response = message_text
handover.moderation_status = 'pending'  # или 'approved' если owner
```
...
> Это оптимизация стоимости. Не критично для MVP.
