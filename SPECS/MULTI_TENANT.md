...
|--------|----------|--------|
| **Заказчик** | Покупатель услуги Truffles (бизнес) | Салон "Мира", ТОО "Truffles" |
| **Клиент** | Клиент заказчика (человек в WhatsApp) | Анна, +7 701 570 5555 |
| **client_id** | UUID заказчика в системе | 499e4744-5e7f-4a97-8466-56ff2cdcf587 |
| **client_slug** | Короткое имя заказчика | truffles, demo_salon |
| **user** | Запись клиента в БД (по телефону) | users.phone = +77015705555 |
| **conversation** | Диалог между клиентом и ботом | conversations.id = UUID |
...
Заказчик (clients)
    ...
    ├── Промпт (prompts)
    ├── База знаний (Qdrant, client_slug)
    ├── Telegram группа
    │
    └── Клиенты заказчика (users)
            │
...
Входящее сообщение (WhatsApp)
        ↓
n8n определяет client_slug (из webhook URL)
        ↓
POST /message { client_id, remote_jid, content }
        ↓
Python API:
...
**Решение:** webhook URL содержит client_slug.

```
https://n8n.truffles.kz/webhook/.../salon_elita
                                    ↑
                              client_slug
```

n8n извлекает slug → ищет в БД → получает client_id.

**Реализация:** n8n 1_Webhook → Python API.
...
**Решение:** Пока — отдельный client на каждый номер.

В будущем (если нужно):
- Таблица `branches` — филиалы
- branch_id в conversations
