...
- `docs/NORTHSTAR.md` — NorthStar и ценность продукта
- `SPECS/ARCHITECTURE.md` — Decision Graph и контракты между слоями
- `SPECS/ESCALATION.md` — процесс эскалации и SLA
...
0. **State Gate**: `manager_active/opt_out` → статус/ожидание; `pending` → статусные команды + обычный ответный пайплайн (без booking и новых handover).
1. **Hard‑LAW** (оплата: подтверждение/проверка/возвраты, медицинка, жалобы, переносы, тех. сбои) → только эскалация.
2. **Policy‑gates (скидки/оплата info)** → ответ **только** по правилам из client_pack; если правил нет — эскалация.
...
## LLM как семантика + жёсткое enforcement [P0]

- **LLM‑контроллер** выбирает класс/цель/слоты (structured JSON) — арбитр **смысла**, но не безопасности и не фактов.
- **Safety/Policy Gate** имеет приоритет над семантикой: риск/LAW → эскалация без попытки “договорить”.
- Router — **единственный источник класса**; fast/anchor/heuristic — только fallback при низкой уверенности/ошибке.
- LLM **не создаёт факты** и не изменяет правила: факты приходят из data pack (truth/RAG), иначе уточнение/эскалация.
- Валидатор ответа:
  - оплата/медицинка/жалобы/переносы → **override на эскалацию**;
...
## Контракт ответа (живость без лжи) [P0]

Формула ответа: **эмпатия → компетентность → факт → шаг**.
- 2–3 предложения, без воды.
- Факты только из data pack; если факта нет — уточнение или эскалация.
- В `manager_active` — статус/ожидание; в `pending` — статусные команды + обычный ответный пайплайн (без booking и новых handover).

## Правило 1: Нет значит нет [РЕАЛИЗОВАНО]

Клиент просит менеджера → эскалация + статус без обещаний:
> "Передал администратору. Он подключится в ближайшее время. Если нужно узнать статус — напишите."

**Факт реализации:** `MSG_ESCALATED` сейчас = “Передал менеджеру. Могу чем-то помочь пока ждёте?” в
`truffles-api/app/routers/webhook/_legacy.py` → **GAP** (текст не совпадает с pending‑режимом).
...
- Session Memory v1.1: `last_question_type`, `pending_slots`, `active_goal`, `goal_stack` (до 3 целей)
 ...
 - **Context Manager (P0):**
   ...
   - `context_manager.language` — язык клиента (RU/KZ) для устойчивого тона
...
- Применяется только в `bot_active` (в `pending/manager_active` правило quiet-hours не применяется).
...
**Реализация:** факты только из data pack клиента; RAG/Qdrant — механизм поиска по этим фактам.
...
**Реализация:** Risk Gate (Hard‑LAW/policy) + отсутствие фактов после 1–2 уточнений → эскалация.  
`should_escalate(intent)` используется только для `human_request`/`frustration`, не является единственным триггером.
...
# ЧАСТЬ 4: ЭСКАЛАЦИЯ (СЕРВИС)

> Полная спецификация: `SPECS/ESCALATION.md`

## 4.1 Ценность и конечная цель
- **Ценность клиенту:** гарантированный ответ человека в срок, без риска и выдумок.
- **Ценность бизнесу:** защита LAW‑зон + сохранение доверия на сложных кейсах.
- **Ценность системе:** превращает неопределённость в управляемый процесс + источник данных для улучшений.

**Конечная цель:** устойчивый SLA по эскалациям и снижение повторных эскалаций за счёт обновления data pack.

**DoD эскалации (минимум):**
- клиент всегда получает понятный статус и SLA;
- у менеджера есть полная карточка (summary + причина + следующий шаг);
- time‑to‑take и time‑to‑resolve измеримы и улучшаются.

## 4.2 Канон триггеров (приоритет)
1) **Hard‑LAW / Policy‑gates** → немедленная эскалация.  
...
**Принцип:** риск выше смысла; семантика не может “договорить” риск‑зону.

## 4.3 Поведение консультанта вокруг эскалации
- **До эскалации:** собрать минимум безопасных слотов (филиал/услуга/контакт/время) и не уходить в длинные уточнения.
- **Сообщение клиенту:** статус + SLA + следующий шаг, без “извините”, без обещаний решения.
- **Pending:** бот отвечает только статусами (`pending_wait`, `pending_status`, `pending_ack`, `pending_close`).
- **Manager Active:** бот молчит, сообщения клиента форвардятся менеджеру.
- **После resolve:** бот возвращается в active, контекст сбрасывается (кроме pending‑resume).

**Факт реализации (опорные места):**
- Текст подтверждения: `MSG_ESCALATED` в `truffles-api/app/routers/webhook/_legacy.py`.
- Pending‑логика и статусы: `truffles-api/app/routers/webhook/_legacy.py`.
- Snapshot/restore контекста: `truffles-api/app/services/state_service.py`,
  `truffles-api/app/routers/webhook/pending.py`.

## 4.4 Контракт ответа клиенту (эскалация)
- 2–3 предложения: забота → компетентность → статус → шаг.
- **Must include:** “передал администратору”, “ожидайте ответ”, “что делать дальше”.
- **Must not include:** обещания решения, “извините”, догадки.

## 4.5 Карточка менеджера (контракт)
**Сейчас:** имя, телефон, причина, последнее сообщение.  
**Должно быть:** summary, причина/риск, контекстные слоты, предложенный следующий шаг.  
**Ценность:** сокращает время до первого ответа и снижает ошибочные решения.

## 4.6 Реализация (ключевые файлы)
- `truffles-api/app/services/state_service.py` — `escalate_to_pending`, `manager_take`, `manager_resolve`.
- `truffles-api/app/services/escalation_service.py` — handover + уведомление.
- `truffles-api/app/services/telegram_service.py` — карточка + кнопки.
- `truffles-api/app/services/reminder_service.py` — pending‑ping, auto‑close, reminders.
- `truffles-api/app/routers/telegram_webhook.py` — take/resolve/return/skip.
- `truffles-api/app/services/manager_message_service.py` — ответы менеджера клиенту.

## 4.7 GAP/задачи (с ценностью)
1) **Карточка менеджера = summary + next step** → быстрее ответы, меньше ошибок.  
2) **Согласовать текст `MSG_ESCALATED` с pending‑поведением** → меньше ложных ожиданий.  
3) **Единый реестр причин эскалации** → меньше false‑escalation, лучше обучение.  
...
Вопрос → Бот не знает → Эскалация
                    ...
                    Менеджер отвечает
                            ↓
                Backlog кандидатов (review)
                            ↓
             Обновление client/branch/policy pack
                            ↓
                Compile → Sync → RAG
                            ↓
                    Бот выучил (через данные)
```
...
| Data packs клиента | ✅ Канон |
| Ответы менеджеров | 📋 Кандидаты в backlog |
| Исправления от владельца | 📋 Через pack‑апдейт |
...
| SLA эскалации | Время до TAKE/RESOLVE | ⚠️ Метрики есть, контроль не настроен |
| Карточка менеджера полная | summary + причина + шаг | 📋 План |
...
3. Если не знаешь — эскалируй и скажи "Передал администратору"
...
RAG score 0.5-0.85 →  AUTO + FLAG FOR REVIEW (бот отвечает, но флаг на проверку)
RAG score < 0.5    →  ESCALATION (передать менеджеру)
RAG score = 0      →  ESCALATION + "пробел в базе"
```

**Текущая реализация (v1):**
- HIGH=0.85, MID=0.5 (`ai_service.py`)
- max_score >= 0.85 → ответ по фактам
- 0.5 ≤ max_score < 0.85 → ответ по фактам + метка в trace/meta
- max_score < 0.5 → `low_confidence` → handover (`pending`) + уведомление
- Никаких “уточню у коллег” без реальной эскалации
