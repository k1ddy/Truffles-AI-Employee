...
| 2. Анти-амнезия | ⚠️ ЧАСТИЧНО (история есть, логика приветствия — в промпте) |
...
**Пример из промпта:**
```
Если клиент отправил что-то непонятное (точку, эмодзи, фото) — 
не паникуй, вежливо уточни или продолжи тему.
```
...
**Не умеет:**
- ❌ Долгосрочная память (P2-P3)
- ❌ Работа с возражениями (P3)
- ❌ Детекция тупика (P2)
- ❌ Автообучение (P2)
...
|------|-----------|----------|
| Детекция тупика | P1 | Если клиент застрял → эскалация |
| Автообучение | P2 | Ответ менеджера → в базу |
| Анти-амнезия в коде | P2 | Логика приветствия по времени |
| Долгосрочная память | P2-P3 | Summary в user.metadata |
...
| Фича | Описание |
|------|----------|
| Умные продажи | Детекция готовности, застревания |
| Воронка 5 этапов | State machine продаж |
| Задержка ответа | Имитация печатания |
| Свой классификатор | Вместо LLM |
