...
1. **Запрещённые/рисковые темы** (жалобы, возвраты денег, индивидуальные договорённости, тех. сбои) → только эскалация.
2. **Opt-out / rejection** → мьют. Если в сообщении есть запись/цена/адрес/время — запросить re-engage и не выполнять действие до ответа.
3. **Human request / frustration** → эскалация.
4. **Booking flow** → сбор слотов записи (важнее цены/общих вопросов).
5. **Question (in-domain)** → ответ через truth/RAG, при низкой уверенности — уточнение/эскалация.
6. **Greeting / thanks** → короткий ответ, без заявки.
7. **Out-of-domain / other** → вежливый отказ/перенаправление.
...
- Валидатор ответа: если LLM упоминает оплату/медицину/жалобы/скидки/возвраты → **override на эскалацию**.
...
- Окно: 22:00–09:00 по локальному времени салона.
- Поведение: 1–2 коротких предложения, спокойный тон, без лишних вопросов; CTA допустим, но мягко и коротко.
...
- Мягко отвечает на возражение по цене без скидок и без выдуманных фактов
...
## Правило 9: Бот отвечает только текстом [РЕАЛИЗОВАНО]
...
**Реализация:** API отправляет только текст.
...
| Возврат после долгого перерыва (>24ч) | Представиться снова |
