...
**Behavioral Shield (P1)**
- Спам/машинная скорость: планируется silent‑ban по `remote_jid` (для WhatsApp нет IP).
- Токсичность/агрессия: intent `frustration` → эскалация, без попытки “отработать”.
- Rate‑limit: для медиа уже есть лимиты; для текста — план.
...
- LLM — основной ответчик после жёстких гейтов (pending/opt‑out/Hard‑LAW) и policy‑gates.
