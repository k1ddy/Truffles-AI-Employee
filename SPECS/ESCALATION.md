...
- В первом сообщении при эскалации обязательно: статус заявки + предупреждение, что пока заявка активна бот не отвечает; клиент может дополнять, всё будет передано.
