...
| `pending` | Ждём менеджера | In‑domain ответы разрешены, новые handover запрещены; статус/подтверждение приоритетно | Менеджер [Беру] или клиент отменил |
...
- В первом сообщении при эскалации обязательно: статус заявки + уведомление, что сообщения передаются менеджеру; бот может помочь с услугами/ценами/записью пока ждём ответ.
- Создан handover в БД
- Уведомление в Telegram с кнопками [Беру] [Вернуть боту] [Не могу]
- In‑domain ответы (info/booking/consult) разрешены, **без** создания новых handover.
...
- если запрос “как на фото/референс” без фото → `style_reference_pending` (TTL) и просьба прислать фото.
- если фото пришло раньше текста → сохраняем ссылку/путь (TTL) и используем при явном стиле/референсе в следующем сообщении.
- В `pending`: медиа форвардится в Telegram; бот может отвечать по фактам/записи, но без новых handover.
- В `manager_active`: бот полностью молчит.
- Голосовые: транскрибируем только короткие PTT; длинные аудио/видео — просим текст.
- Несколько аудио подряд: одна активная транскрибация на диалог, очередь по времени; “расшифровываю” не чаще 1 раза в 10 минут.
