...
- top-level `expected_reply_type` — это shorthand projection активного `pending_question_contract.expected_reply_type`, а не отдельный semantic owner. Если `memory.profile.pending_question_contract` присутствует, ориентируйся на него как на primary source.
