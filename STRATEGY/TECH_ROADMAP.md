...
| CA-06 | Consult pack-only + short-circuit при явной услуге | CONSULTANT | `ca06-consult` / live | trace `consult_flow/consult` + `consult_playbook_id` + `source=pack` + CI consult cases | verified (STATE.md:1142) |
| CA-07 | OOD + low-signal guard + smalltalk redirect | CONSULTANT + ARCHITECTURE | `ca07-ood` / live | trace `out_of_domain/fast_intent/smalltalk` + `livecheck-ca07-ood.jsonl` + `livecheck-evidence.md` | pending |
