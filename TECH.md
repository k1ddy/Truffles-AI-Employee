...
- `WEBHOOK_PIPELINE_BUDGET_MS` — бюджет (мс) для /webhook пайплайна (LLM/RAG gating) (default: 12000).
- `ROUTER_TIMEOUT_SECONDS` — таймаут LLM‑router (intent/controller) (default: 3.0).
- `LLM_TIMEOUT_SECONDS` — таймаут общего LLM‑ответа (default: 8).
- `CONSULT_LLM_TIMEOUT_SECONDS` — таймаут LLM для consult (default: 8).
