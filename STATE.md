...
- Soft price defense (why_price_from / objection_price) реализована в demo_salon knowledge + покрыта EVAL; доки не синкнуты, тесты не запускались (doc-only sync). Evidence: `truffles-api/app/services/demo_salon_knowledge.py:1969-1977`, `truffles-api/app/knowledge/demo_salon/EVAL.yaml` (кейсы objection_price/why_price_from).
- Night tone/timezone: `salon.timezone` добавлен (Asia/Almaty) + quiet-hours notice в `webhook.py` по локальному времени/часам; исключения pending/manager_active + LAW/opt-out/OOD; EVAL `E014e/E014f`; `docker run --rm -v /home/zhan/truffles-main/truffles-api:/app -w /app ghcr.io/k1ddy/truffles-ai-employee:main pytest /app/tests/test_demo_salon_eval.py -q` PASS.
