...
**NOW (1 экран)**
- NOW (Wave38 local closeout complete; PR #960 ready for push update): Calendar now closes the remaining operator-grade gaps that stayed open after merged `Wave37`. Filters already use one explicit `draft -> applied` boundary, the phone field now preserves raw typing/deletion/paste, and existing bookings now support bounded operator `edit/reschedule/cancel` lifecycle with status-based guardrails instead of dead ends. The Calendar action panel exposes `Изменить запись` / `Отменить запись`, the booking composer reopens in edit mode with current data prefilled, specialist/date changes reset slot choice deterministically, backend now serves `PATCH /calendar/bookings/{booking_id}` plus body-based cancel, OpenAPI/types are synced, and deterministic proof now covers filter semantics, phone ergonomics, edit, cancel, blocked non-active statuses, no-show follow-up safety, and medium-width layout. Visual inspection is clean for edit, cancel, blocked no-show, phone, filter, and `1024px` states; remaining work is only canon sync/push/PR review, not product implementation. Evidence: `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/calendar-bookings.ts`, `console-web/src/types/api.generated.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `contracts/console_api/openapi.v1.yaml`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `truffles-api/tests/test_console_openapi_calendar_contract.py`, `/tmp/wave38-operator-captures/wave38-filters-applied-1280.png`, `/tmp/wave38-operator-captures/wave38-phone-partial-1280.png`, `/tmp/wave38-operator-captures/wave38-phone-valid-1280.png`, `/tmp/wave38-operator-captures/wave38-edit-open-1280.png`, `/tmp/wave38-operator-captures/wave38-cancel-panel-1280.png`, `/tmp/wave38-operator-captures/wave38-no-show-disabled-1280.png`, `/tmp/wave38-operator-captures/wave38-medium-width-1024.png`, `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file src/lib/calendar-bookings.ts --file e2e/calendar-operator.spec.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/calendar-operator.spec.ts --project chromium` (`10 passed`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium` (`14 passed, 1 skipped`), `cd truffles-api && pytest -q tests/test_calendar_bookings_router.py tests/test_console_openapi_calendar_contract.py` (`38 passed`), `cd truffles-api && ruff check app/routers/calendar.py app/services/appointment_service.py tests/test_calendar_bookings_router.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
