...
**NOW (1 экран)**
- NOW (Wave39 Part E local evidence): `Wave39` remains the only valid active block on top of merged `Wave38`, and Parts A through E are now implemented locally. Calendar now has a bounded action-safety envelope plus failure-family visibility: backend metrics track denied/version-conflict/double-submit/filter/follow-up families, router-owned audit observations record those families through `POST /calendar/operator-events` and lifecycle routes, frontend emits apply/reset/double-submit observations from the real operator surfaces, and deterministic replay proof now verifies both observability and the fail-closed action contract. `Wave39` PR `#961` is now open. The next active step is to merge it and then run the bounded post-merge replay on `main`. Evidence: `console-web/src/app/calendar/page.tsx`, `console-web/src/app/calendar/_lib/useCalendarFiltersMachine.ts`, `console-web/src/app/calendar/_lib/useBookingComposerMachine.ts`, `console-web/src/app/calendar/_lib/useBookingActionPanelMachine.ts`, `console-web/src/app/calendar/_lib/useBookingFollowUpMachine.ts`, `console-web/src/components/CaseBookingsPanel.tsx`, `console-web/src/lib/calendar-action-registry.ts`, `console-web/src/lib/calendar-bookings.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/logging_config.py`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `truffles-api/app/services/calendar_action_contract.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `truffles-api/tests/test_calendar_noshow_followup_router.py`, `truffles-api/tests/test_console_openapi_calendar_contract.py`, `contracts/console_api/openapi.v1.yaml`, `cd truffles-api && pytest -q tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py` (`37 passed`), `cd truffles-api && pytest -q tests/test_console_openapi_calendar_contract.py` (`26 passed`), `cd truffles-api && ruff check app/routers/calendar.py app/logging_config.py app/services/calendar_action_contract.py tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file src/lib/calendar-bookings.ts --file e2e/calendar-operator.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 CALENDAR_OPERATOR_CAPTURE_DIR=/tmp/wave39-part-e-operator-captures npx playwright test e2e/calendar-operator.spec.ts --project chromium` (`18 passed`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 npx playwright test e2e/inspect_case.spec.ts --project chromium --workers=1` (`14 passed, 1 skipped`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`), `/tmp/wave39-part-e-operator-captures/wave39-queue-default-1280.png`, `/tmp/wave39-part-e-operator-captures/wave39-cancel-panel-1280.png`, `/tmp/wave39-part-e-operator-captures/wave39-medium-width-1024.png`.
