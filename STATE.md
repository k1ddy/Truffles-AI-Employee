...
- GAP (runtime): For the same inbound reply flow, `decision_trace` in `conversations.context` does not include stage `marketing_reply_context` (`marketing_reply_context_count=0`, trace length `40`) even though meta/context linkage is present; runtime trace stage-retention parity for Wave `3` remains open — evidence: `/tmp/marketing-live-20260219-131840/sql_reply2_trace_full.json`, `/tmp/marketing-live-20260219-131840/reply2_context_summary.json`.
