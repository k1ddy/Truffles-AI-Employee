...
- GAP (CI/live-auth): PR run `https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/22278409035` has failed job `console-e2e-live` (`64444293270`) at step `Playwright login smoke (live)` with command `npm run test:e2e:smoke -- e2e/login.spec.ts`; error snippet: `Error: Keycloak sign-in not reachable (status 404)` from `console-web/e2e/global-setup.ts:67`.
