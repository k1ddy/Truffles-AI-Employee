...
**NOW (1 экран)**
- NOW (local worktree, activation-service GHCR digest fix P11): merge run `23127522792` failed only in `deploy` because `restart_release.sh` correctly promoted the image to immutable GHCR digest form, but `scripts/restart_knowledge_activation_service.sh` still rejected `ghcr.io/k1ddy/truffles-ai-employee@sha256:...` under `REQUIRE_GHCR=1`. The fix makes the activation-service restart script use the same digest-aware GHCR ref contract as `restart_release.sh`/`restart_api.sh`; targeted regression proof now covers a digest ref directly, so deploy keeps immutable parity instead of silently downgrading back to mutable tags. Evidence: `https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/23127522792`, `scripts/restart_knowledge_activation_service.sh`, `truffles-api/tests/test_restart_release_scripts.py`, `docs/TASK_PACKAGES/TP-2026-03-16-activation-service-ghcr-digest-fix-p11-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && pytest -q truffles-api/tests/test_restart_release_scripts.py` (`3 passed`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && ruff check truffles-api/tests/test_restart_release_scripts.py` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && bash -n scripts/restart_knowledge_activation_service.sh` (`pass`).
- NOW (local worktree, knowledge activation closeout hardening P10): `main` deploy proof no longer depends on owner consultant-verification rollout and no longer permits `closeout.status=skipped`. `ops/knowledge_activation_closeout.py` now treats owner rollout as evidence only (`owner_surface_enabled`) while gating on actual release invariants (`release_preview_ready`, live pointer separation, activation consistency); local proof against the real canary tenant `demo_salon/main` now returns `decision=go` even though `owner_surface_enabled=false`. `.github/workflows/ci.yml` now passes canonical default closeout target `demo_salon/main` into `scripts/knowledge_activation_postdeploy.sh`, marks closeout hard-required on deploy, and still allows full explicit overrides through workflow-dispatch inputs or repo vars without silently accepting partial overrides. Evidence: `ops/knowledge_activation_closeout.py`, `scripts/knowledge_activation_postdeploy.sh`, `.github/workflows/ci.yml`, `truffles-api/tests/test_knowledge_activation_closeout.py`, `truffles-api/tests/test_knowledge_activation_postdeploy_script.py`, `docs/runbooks/KNOWLEDGE_ACTIVATION_RELEASE.md`, `TECH.md`, `SPECS/SYSTEM_REFERENCE.md`, `docs/TASK_PACKAGES/TP-2026-03-16-knowledge-activation-closeout-hardening-p10-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && pytest -q truffles-api/tests/test_knowledge_activation_closeout.py truffles-api/tests/test_knowledge_activation_postdeploy_script.py` (`11 passed`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && ruff check ops/knowledge_activation_closeout.py truffles-api/tests/test_knowledge_activation_closeout.py truffles-api/tests/test_knowledge_activation_postdeploy_script.py` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && bash -n scripts/knowledge_activation_postdeploy.sh` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 - <<'PY'\nimport pathlib, yaml\npath = pathlib.Path('.github/workflows/ci.yml')\nyaml.safe_load(path.read_text())\nprint('YAML_PARSE_OK')\nPY` (`YAML_PARSE_OK`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 - <<'PY'\nimport pathlib, re, subprocess, tempfile, yaml\nworkflow = yaml.safe_load(pathlib.Path('.github/workflows/ci.yml').read_text())\nfor step_name in ('Deploy to VPS', 'Summarize knowledge activation proof'):\n    step = next(item for item in workflow['jobs']['deploy']['steps'] if item.get('name') == step_name)\n    script = re.sub(r\"\\$\\{\\{[^}]+\\}\\}\", 'DUMMY', step['run'])\n    with tempfile.NamedTemporaryFile('w', suffix='.sh', delete=False) as tmp:\n        tmp.write(script)\n        path = tmp.name\n    subprocess.check_call(['bash', '-n', path])\n    print(f'BASH_PARSE_OK {step_name}')\nPY` (`BASH_PARSE_OK Deploy to VPS`, `BASH_PARSE_OK Summarize knowledge activation proof`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && tmp_guard=$(mktemp) && printf '{\"decision\":\"go\"}\\n' > \"$tmp_guard\" && python3 ops/knowledge_activation_closeout.py --client-slug demo_salon --branch-slug main --guard-json \"$tmp_guard\" --pretty` (`decision=go`, `owner_surface_enabled=false`, `release_preview_ready=true`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && SESSION_AGENT=a30 bash scripts/session_check.sh` (`Session OK`).
- NOW (local worktree, deploy SSH auth unification P7): post-merge `main` failure is isolated to CI deploy transport, not product/runtime code. Run `23112703782` failed only in `deploy`/`Deploy to VPS` with `ssh: unable to authenticate`; all upstream gates (`lint`, `unit-tests`, `console-contract-predeploy`, `console-e2e`, `build-push`) were green. The workflow fix now moves deploy onto the same runner-side SSH bootstrap already used by livecheck: `.github/workflows/ci.yml` normalizes/validates `SSH_PRIVATE_KEY` with `ssh-keygen -yf`, populates `known_hosts`, performs an explicit `ssh ... echo deploy-ready` preflight, and then runs the deploy script over standard `ssh` instead of raw `appleboy/ssh-action` key parsing. This keeps deploy mandatory on `main`, preserves parity checks, and turns future auth drift into an earlier/clearer preflight failure. Evidence: `https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/23112703782`, `.github/workflows/ci.yml`, `docs/TASK_PACKAGES/TP-2026-03-15-ci-deploy-ssh-auth-unification-p7-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 - <<'PY'\nimport pathlib, yaml\nyaml.safe_load(pathlib.Path('.github/workflows/ci.yml').read_text())\nprint('YAML_PARSE_OK')\nPY` (`YAML_PARSE_OK`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 - <<'PY'\nimport pathlib, re, subprocess, tempfile, yaml\nworkflow = yaml.safe_load(pathlib.Path('.github/workflows/ci.yml').read_text())\nfor step_name in ('Configure SSH for deploy', 'Deploy to VPS'):\n    step = next(item for item in workflow['jobs']['deploy']['steps'] if item.get('name') == step_name)\n    script = re.sub(r\"\\$\\{\\{[^}]+\\}\\}\", 'DUMMY', step['run'])\n    with tempfile.NamedTemporaryFile('w', suffix='.sh', delete=False) as tmp:\n        tmp.write(script)\n        path = tmp.name\n    subprocess.check_call(['bash', '-n', path])\n    print(f'BASH_PARSE_OK {step_name}')\nPY` (`BASH_PARSE_OK Configure SSH for deploy`, `BASH_PARSE_OK Deploy to VPS`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && SESSION_AGENT=a30 bash scripts/session_check.sh` (`pass`).
- NOW (local worktree, knowledge activation closeout P6): final release-model proof now has one machine-readable tenant-level artifact instead of manual stitching. `ops/knowledge_activation_closeout.py` reuses the P5 release guard, snapshots one `client_slug + branch_slug` from Postgres truth, derives preview/live invariants (`preview_available`, `can_verify_now`, `active vs published candidate`, activation job consistency), and emits a single `go|no_go` verdict for post-deploy closeout. P0/P1/P2/P3/P4/P5 contracts remain unchanged: preview still uses pinned snapshots, live still switches only through `active_knowledge_version_id`, activation still runs through `knowledge_activation_jobs`, and release parity/canary still flows through `restart_release.sh` + `knowledge_activation_release_guard.py`. Residual debt is no longer missing automation; the only remaining step is to capture one real closeout artifact on the rollout target. Evidence: `ops/knowledge_activation_closeout.py`, `truffles-api/tests/test_knowledge_activation_closeout.py`, `docs/runbooks/KNOWLEDGE_ACTIVATION_RELEASE.md`, `docs/TASK_PACKAGES/TP-2026-03-15-knowledge-activation-closeout-p6-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30/truffles-api && pytest -q tests/test_knowledge_activation_closeout.py tests/test_knowledge_activation_release_guard.py tests/test_restart_release_scripts.py` (`9 passed`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 -m py_compile ops/knowledge_activation_closeout.py` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && SESSION_AGENT=a30 bash scripts/session_check.sh` (`pass`).
- NOW (local worktree, owner surface decomposition slice A8): `Knowledge` and `Business -> Проверка консультанта` no longer keep their page-level query/mutation orchestration inside the route shells. `console-web/src/app/knowledge/page.tsx` is now a `211`-line route shell (down from `2530`) that delegates authoring/fleet/readiness orchestration to `console-web/src/app/knowledge/_hooks/useKnowledgeStudioState.ts` plus extracted panels; `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx` is now a `58`-line composition shell (down from `630`) that delegates session/findings/compare orchestration to `console-web/src/app/business/consultant-verification/_hooks/useConsultantVerificationWorkspaceState.ts`. Deterministic proof is green and the owner path remains unchanged, but maintainability debt is now isolated inside the new hooks (`2146` and `599` LOC) instead of the pages; the next structural follow-up is tracked under `UX-54`. Evidence: `console-web/src/app/knowledge/page.tsx`, `console-web/src/app/knowledge/_hooks/useKnowledgeStudioState.ts`, `console-web/src/app/knowledge/_components/KnowledgePlatformAdminFleetPanel.tsx`, `console-web/src/app/knowledge/_components/KnowledgeBranchReadinessPanel.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx`, `console-web/src/app/business/consultant-verification/_hooks/useConsultantVerificationWorkspaceState.ts`, `console-web/e2e/owner-admin-business.spec.ts`, `docs/TASK_PACKAGES/TP-2026-03-15-console-owner-surface-decomposition-statehooks-a8.md`; checks: `cd /home/zhan/worktrees/2026-03-15-console-owner-surface-decomposition-statehooks-a8/console-web && npm run lint -- --file src/app/knowledge/page.tsx --file src/app/knowledge/_hooks/useKnowledgeStudioState.ts --file src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx --file src/app/business/consultant-verification/_hooks/useConsultantVerificationWorkspaceState.ts --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-owner-surface-decomposition-statehooks-a8/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-owner-surface-decomposition-statehooks-a8/console-web && npx playwright test e2e/owner-admin-business.spec.ts --project chromium --workers 1 --grep 'knowledge owner flow decomposition|consultant verification decomposed lanes'` (`2 passed`).
- NOW (local worktree, owner scope-gate unification): `Knowledge` and `Business -> Проверка консультанта` now reuse one shared owner branch-scope gate and one shared context-apply helper instead of carrying separate page-level branch repair flows. The new `ConsoleOwnerScopeGate` component keeps branch-selection UI aligned, while `console-scope-gate.ts` centralizes `console-me` refetch and dependent query invalidation so both pages repair context with the same semantics. Evidence: `console-web/src/components/ConsoleOwnerScopeGate.tsx`, `console-web/src/lib/console-scope-gate.ts`, `console-web/src/app/knowledge/page.tsx`, `console-web/src/app/business/consultant-verification/page.tsx`, `console-web/e2e/owner-admin-business.spec.ts`, `docs/TASK_PACKAGES/TP-2026-03-15-console-owner-scope-gate-unification-a5.md`; checks: `cd /home/zhan/worktrees/2026-03-15-console-owner-scope-gate-unification-a5/console-web && npm run lint -- --file src/app/knowledge/page.tsx --file src/app/business/consultant-verification/page.tsx --file src/components/ConsoleOwnerScopeGate.tsx --file src/lib/console-scope-gate.ts --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-owner-scope-gate-unification-a5/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-owner-scope-gate-unification-a5/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --workers 1 --grep 'consultant verification branch gate|knowledge scope gate'` (`2 passed`).
- NOW (local worktree, sync-state truth unification): owner-facing `Knowledge` sync state no longer mixes fresh `knowledge/current` with stale `console-me` branch safe-mode fields after async `publish/retry-sync/rollback`. The page now derives `sync_status`, `safe_mode`, and failure detail from `GET /console/v1/knowledge/current`, suppresses old failure detail while sync is `pending`, and refetches all dependent server-state (`console-me`, `knowledge/current`, `knowledge/history`, consultant-verification readiness) after each async mutation so owner UI cannot render `pending + safe_mode + timed out` together. Evidence: `console-web/src/app/knowledge/page.tsx`, `console-web/e2e/owner-admin-business.spec.ts`, `docs/TASK_PACKAGES/TP-2026-03-15-console-knowledge-sync-state-unification-a4.md`; checks: `cd /home/zhan/worktrees/2026-03-15-console-knowledge-sync-state-unification-a4/console-web && npm run lint -- --file src/app/knowledge/page.tsx --file src/app/business/consultant-verification/page.tsx --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-knowledge-sync-state-unification-a4/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-15-console-knowledge-sync-state-unification-a4/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --workers 1 --grep 'knowledge sync contradiction|consultant verification sync state'` (`1 passed`).
- NOW (local worktree, owner knowledge stabilization reset): owner/admin `Knowledge` no longer runs heavy sync inline on publish/retry/rollback. That stabilization block established the async owner contract (`sync_status=pending`, bounded owner copy, no inline live switch); P3 now completes it by replacing the old generic `knowledge.sync` carrier with direct `knowledge_activation_jobs` ownership while keeping the same user-facing contract. The preview-vs-live split from the release-model stop-loss remains intact: preview proof comes from a pinned snapshot, while live activation stays a separate operational status. Evidence: `truffles-api/app/services/knowledge_registry_service.py`, `truffles-api/app/routers/console.py`, `truffles-api/app/routers/webhook/outbox.py`, `truffles-api/app/services/console_consultant_verification.py`, `console-web/src/app/knowledge/page.tsx`, `console-web/src/app/business/consultant-verification/page.tsx`, `docs/TASK_PACKAGES/TP-2026-03-15-owner-knowledge-stabilization-reset-a4.md`, `docs/TASK_PACKAGES/TP-2026-03-15-knowledge-release-model-stoploss-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/truffles-api && pytest -q tests/test_console_owner_business.py tests/test_console_consultant_verification_api.py tests/test_knowledge_registry_sync_backfill.py -k 'knowledge or consultant_verification or sync'` (`67 passed, 32 deselected`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/truffles-api && ruff check app/routers/console.py app/services/knowledge_registry_service.py app/services/console_consultant_verification.py app/routers/webhook/outbox.py tests/test_console_owner_business.py tests/test_console_consultant_verification_api.py tests/test_knowledge_registry_sync_backfill.py` (`pass`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/console-web && npm run generate:api` (`pass`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/console-web && npm run lint -- --file src/app/knowledge/page.tsx --file src/app/business/consultant-verification/page.tsx --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-15-owner-knowledge-stabilization-reset-a4/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --workers 1 --grep 'knowledge sync|consultant verification readiness'` (`2 passed`).
- NOW (local worktree, owner consultant verification closeout): local closeout proof is green and the remaining gap is release-only. A local clone DB (`truffles-local-postgres-a920`) plus a local branch API (`truffles-api-local-a920`) serve the branch-only `/console/v1/business/consultant-verification/*` endpoints, and the simulation runtime path is fixed end-to-end: consultant-verification webhook payloads use tenancy-safe `source="system"` + `origin_source="console_consultant_verification"`, resolve `instanceId`, and preserve `origin_source` through webhook preflight so simulation mode survives into the real runtime without real outbound side effects. Local deterministic proof is green (`111 passed`, OpenAPI check, `npm run generate:api`, lint, build), the owner/admin compare lane is green against the local stack when the local console server is started with the same auth env as Playwright (`NEXTAUTH_URL=http://127.0.0.1:3102`, `NEXT_PUBLIC_E2E_BYPASS_AUTH=1`), and closeout screenshots were captured at `390/1024/1280/1440` widths under `/tmp/consultant_verification_closeout_a920/`. One-client canary and post-merge monitoring remain blocked on merge/deploy because the rollout flag and endpoints are currently proven only on the branch-local stack, not on merged main. Evidence: `console-web/e2e/support/keycloak-auth.ts`, `console-web/e2e/owner-admin-business.spec.ts`, `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/app/routers/webhook/http.py`, `truffles-api/app/routers/webhook/decision.py`, `truffles-api/app/schemas/webhook.py`, `contracts/tenancy/tenant_context.v1.jsonschema`, `truffles-api/tests/test_console_consultant_verification_api.py`, `truffles-api/tests/test_branch_routing_instance.py`, `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && pytest -q tests/test_console_consultant_verification_api.py tests/test_console_owner_business.py tests/test_console_knowledge_preflight.py tests/test_state_service_simulation.py tests/test_branch_routing_instance.py` (`111 passed`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run generate:api` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run lint -- --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && NEXT_PUBLIC_API_URL=http://127.0.0.1:8001/console/v1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 E2E_CONSULTANT_VERIFICATION_SCREENSHOT_DIR=/tmp/consultant_verification_closeout_a920 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --workers 1 --grep "consultant verification compare"` (`1 passed (56.2s)`), `/tmp/consultant_verification_closeout_a920/consultant-verification-390.png`, `/tmp/consultant_verification_closeout_a920/consultant-verification-1024.png`, `/tmp/consultant_verification_closeout_a920/consultant-verification-1280.png`, `/tmp/consultant_verification_closeout_a920/consultant-verification-1440.png`.
- NOW (Wave39 merged + post-merge replay complete): `Wave39` is now merged to `main` via `PR #961`, and the bounded post-merge replay on `/home/zhan/truffles-main` at `origin/main@710f8faa` is green. Calendar action safety is now closed on merged main: the action registry, server-owned `allowed_actions` / blocked reasons, version-safe lifecycle, extracted local machines, and failure-family observability all hold under replay without route-mock drift. `UX-38` is closed. The next valid backlog block is `UX-08` (`runtime health / outbox pressure`), followed by `UX-20` and `UX-26`. Evidence: `PR #961`, `710f8faa`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/calendar-bookings.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/logging_config.py`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/calendar_action_contract.py`, `cd /home/zhan/truffles-main/truffles-api && pytest -q tests/test_appointment_service_status_transitions.py tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py tests/test_console_openapi_calendar_contract.py` (`68 passed`), `cd /home/zhan/truffles-main/truffles-api && ruff check app/routers/calendar.py app/logging_config.py app/services/calendar_action_contract.py app/services/appointment_service.py tests/test_appointment_service_status_transitions.py tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd /home/zhan/truffles-main/truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd /home/zhan/truffles-main/console-web && npm run generate:api` (`pass`), `cd /home/zhan/truffles-main/console-web && npm run lint -- --file src/app/calendar/page.tsx --file src/lib/calendar-bookings.ts --file e2e/calendar-operator.spec.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd /home/zhan/truffles-main/console-web && npm run build` (`pass`), `cd /home/zhan/truffles-main/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 CALENDAR_OPERATOR_CAPTURE_DIR=/tmp/wave39-postmerge-operator-captures npx playwright test e2e/calendar-operator.spec.ts --project chromium` (`18 passed`), `cd /home/zhan/truffles-main/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 npx playwright test e2e/inspect_case.spec.ts --project chromium --workers=1` (`14 passed, 1 skipped`), `/tmp/wave39-postmerge-operator-captures/wave39-queue-default-1280.png`, `/tmp/wave39-postmerge-operator-captures/wave39-cancel-panel-1280.png`, `/tmp/wave39-postmerge-operator-captures/wave39-medium-width-1024.png`.
- DONE (docs-only, program planning): owner-facing consultant verification is now canonized as a bounded Console initiative with one master TP, six execution waves, and one closeout block covering trust entrypoint, safe simulation, owner-readable verdicts, stress scenarios, weak-spot remediation loop, and `live vs draft` compare before publish. Evidence: `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-program-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave1-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave2-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave3-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave4-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave5-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-wave6-a920.md`, `docs/TASK_PACKAGES/TP-2026-03-13-owner-consultant-verification-closeout-a920.md`, `docs/CONSOLE_AUDIT/UX_BACKLOG.md`.
- DONE (local+checks, Wave1 foundation): owner/admin now have a dedicated `Business -> Проверка консультанта` entrypoint with a thin backend overview contract, extracted service boundary, overview cards explaining readiness/gaps/next-wave promise, business-home shortcut, and updated owner/admin smoke lane coverage for the new route. Backend/OpenAPI/frontend targeted checks are green; the only missing proof is Playwright auth execution on local `127.0.0.1:3102`, which currently times out before Keycloak transition in the shared auth helper and therefore remains infra/auth-env evidence rather than product failure. Evidence: `console-web/src/app/business/consultant-verification/page.tsx`, `console-web/src/app/business/page.tsx`, `console-web/src/components/ConsoleShell.tsx`, `console-web/src/lib/api-client.ts`, `console-web/src/types/api.generated.ts`, `console-web/e2e/owner-admin-business.spec.ts`, `truffles-api/app/routers/console.py`, `truffles-api/app/schemas/console.py`, `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/tests/test_console_owner_business.py`, `contracts/console_api/openapi.v1.yaml`, `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && pytest -q tests/test_console_owner_business.py -k consultant_verification` (`4 passed, 59 deselected`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && ruff check app/routers/console.py app/schemas/console.py app/services/console_consultant_verification.py tests/test_console_owner_business.py` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run generate:api` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run lint -- --file src/components/ConsoleShell.tsx --file src/app/business/consultant-verification/page.tsx --file src/app/business/page.tsx --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run build` (`pass`), `curl http://127.0.0.1:3102/business/consultant-verification` (`200`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --grep "consultant verification"` (`failed: Keycloak auth transition timeout in e2e/support/keycloak-auth.ts:55`).
- DONE (local+checks, Wave2 safe simulation kernel): owner/admin consultant verification now has first-class session APIs plus rollback-only runtime execution on the real consultant path. `/business/consultant-verification/sessions` can create/list/get sessions and append owner turns; each simulated turn persists transcript/evidence (`decision_meta`, `decision_trace`, `source_refs`, `would_handoff`, `would_book`, `gap_detected`) while internal simulation source wiring allows runtime suppression without leaking real outbound traffic, bookings, or handovers into committed production state. Evidence: `truffles-api/app/models/console_consultant_verification_session.py`, `truffles-api/app/models/console_consultant_verification_turn.py`, `truffles-api/migrations/057_add_console_consultant_verification.sql`, `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/app/services/state_service.py`, `truffles-api/app/routers/webhook/decision.py`, `truffles-api/app/routers/console.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_consultant_verification_api.py`, `truffles-api/tests/test_state_service_simulation.py`, `contracts/console_api/openapi.v1.yaml`, `console-web/src/lib/api-client.ts`, `console-web/src/types/api.generated.ts`, `cd truffles-api && pytest -q tests/test_console_consultant_verification_api.py tests/test_state_service_simulation.py tests/test_console_owner_business.py -k 'consultant_verification or simulation'` (`13 passed`), `cd truffles-api && ruff check app/models/__init__.py app/models/console_consultant_verification_session.py app/models/console_consultant_verification_turn.py app/services/console_consultant_verification.py app/schemas/console.py app/routers/console.py app/services/state_service.py app/routers/webhook/decision.py tests/test_console_consultant_verification_api.py tests/test_state_service_simulation.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`).
- DONE (local+checks, Wave3 owner-readable workspace): `Business -> Проверка консультанта` now renders a real owner/admin chat workspace whenever rollout is enabled: explicit `как клиент` / `найти слабые места` modes, automatic or manual session start, recent-session list, transcript bubbles, plain-language verdict chips, explanation cards, source refs, preview-only side-effect badges, and bounded advanced details for the team. Frontend targeted lint/build are green; the only missing proof is the local Playwright lane because nothing was listening on `127.0.0.1:3102` during the run, so `page.goto` failed before auth/product checks. Evidence: `console-web/src/app/business/consultant-verification/page.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx`, `console-web/src/app/business/consultant-verification/_lib/presentation.ts`, `console-web/e2e/owner-admin-business.spec.ts`, `docs/CONSOLE_GUIDE.md`, `docs/CONSOLE_AUDIT/UX_BACKLOG.md`, `cd console-web && npm run lint -- --file src/app/business/consultant-verification/page.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx --file src/app/business/consultant-verification/_lib/presentation.ts --file e2e/owner-admin-business.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --grep "consultant verification chat"` (`failed: ERR_CONNECTION_REFUSED before auth because local console server was not listening on 127.0.0.1:3102`).
- DONE (local+checks, Wave4 scenario library + summary): owner/admin consultant verification now exposes a data-driven scenario catalog built from onboarding blueprints, capabilities, and reference packs; owners can launch any scenario as a fresh replay session, fill prompts into the composer, and read a bounded session summary with honest counts for `answered / clarification / handoff / gap` plus weak-turn replay without mutating prior evidence. OpenAPI/types/docs are synced; the remaining gap is still the owner/admin Playwright auth lane: even with a local console server listening on `127.0.0.1:3102`, the shared Keycloak helper times out waiting for auth transition in `e2e/support/keycloak-auth.ts:55`, so the missing proof remains infra/auth-env rather than product behavior. Evidence: `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_consultant_verification_api.py`, `contracts/console_api/openapi.v1.yaml`, `console-web/src/lib/api-client.ts`, `console-web/src/types/api.generated.ts`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationScenarioLibrary.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationSessionSummaryPanel.tsx`, `console-web/src/app/business/consultant-verification/_lib/presentation.ts`, `console-web/e2e/owner-admin-business.spec.ts`, `docs/CONSOLE_GUIDE.md`, `docs/CONSOLE_AUDIT/UX_BACKLOG.md`, `cd truffles-api && pytest -q tests/test_console_consultant_verification_api.py -k 'scenario or replay or summary'` (`2 passed, 5 deselected`), `cd truffles-api && ruff check app/schemas/console.py app/services/console_consultant_verification.py tests/test_console_consultant_verification_api.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/app/business/consultant-verification/page.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationScenarioLibrary.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationSessionSummaryPanel.tsx --file src/app/business/consultant-verification/_lib/presentation.ts --file src/lib/api-client.ts --file e2e/owner-admin-business.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && npm run start -- --hostname 127.0.0.1 --port 3102` + `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --grep "consultant verification scenarios"` (`failed: Keycloak auth transition timeout in e2e/support/keycloak-auth.ts:55`).
- DONE (local+checks, Wave5 findings/remediation loop): owner/admin consultant verification now turns weak answers into persistent findings with family clustering, repeat counts, backlog/learning linkage, and owner-visible statuses. Findings can be reopened automatically when the same failure family appears again, and the UI now exposes manual status updates plus draft retest entrypoints instead of losing the signal inside chat history. Evidence: `truffles-api/app/models/console_consultant_verification_finding.py`, `truffles-api/migrations/058_add_console_consultant_verification_findings.sql`, `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/app/routers/console.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_consultant_verification_api.py`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationFindingsPanel.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx`, `docs/CONSOLE_GUIDE.md`, `docs/CONSOLE_AUDIT/UX_BACKLOG.md`, `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && pytest -q tests/test_console_consultant_verification_api.py -k 'finding or scenario or summary'` (`7 passed, 5 deselected`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && ruff check app/models/__init__.py app/models/console_consultant_verification_finding.py app/schemas/console.py app/services/console_consultant_verification.py app/routers/console.py tests/test_console_consultant_verification_api.py` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run lint -- --file src/app/business/consultant-verification/_components/ConsultantVerificationFindingsPanel.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx --file src/lib/api-client.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run build` (`pass`).
- DONE (local+checks, Wave6 live-vs-draft compare + publish readiness): owner/admin can now compare the same prompt or flagged finding across `live` and the latest saved `draft`, see explicit regression/improvement markers, and retest findings directly from the remediation list. Knowledge Publish is now fail-closed on both recent Validate and recent compare readiness for the current canonical draft hash, and the Knowledge page surfaces the compare status with a direct link back to `Business -> Проверка консультанта`. Evidence: `truffles-api/app/services/knowledge_runtime.py`, `truffles-api/app/services/console_knowledge_preflight.py`, `truffles-api/app/services/console_consultant_verification.py`, `truffles-api/app/routers/console.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_consultant_verification_api.py`, `truffles-api/tests/test_console_knowledge_preflight.py`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationComparePanel.tsx`, `console-web/src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx`, `console-web/src/app/knowledge/page.tsx`, `console-web/e2e/owner-admin-business.spec.ts`, `contracts/console_api/openapi.v1.yaml`, `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && pytest -q tests/test_console_consultant_verification_api.py tests/test_console_knowledge_preflight.py tests/test_console_owner_business.py tests/test_state_service_simulation.py -k 'consultant_verification or knowledge_preflight or simulation or compare or readiness'` (`33 passed, 57 deselected`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run generate:api` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run lint -- --file src/app/business/consultant-verification/page.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationWorkspace.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationScenarioLibrary.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationSessionSummaryPanel.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationFindingsPanel.tsx --file src/app/business/consultant-verification/_components/ConsultantVerificationComparePanel.tsx --file src/app/business/consultant-verification/_lib/presentation.ts --file src/app/knowledge/page.tsx --file src/lib/api-client.ts --file e2e/owner-admin-business.spec.ts` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run build` (`pass`), `cd /home/zhan/worktrees/2026-03-13-owner-consultant-verification-program-a920/console-web && npm run start -- --hostname 127.0.0.1 --port 3102` + `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/owner-admin-business.spec.ts --project chromium --grep "consultant verification compare"` (`failed: Keycloak auth transition timeout in e2e/support/keycloak-auth.ts:55`).
- DONE (merged, Wave38 Calendar post-merge hardening): Calendar filters now use one explicit `draft -> applied` contract, phone input preserves raw typing/deletion/paste with normalized preview, and existing bookings expose bounded operator `edit/reschedule/cancel` lifecycle with deterministic proof and synced OpenAPI/types. `Wave38` is merged to `main` via `PR #960`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/calendar-bookings.ts`, `console-web/src/types/api.generated.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `truffles-api/tests/test_console_openapi_calendar_contract.py`, `contracts/console_api/openapi.v1.yaml`, `PR #960`, `git fetch origin --prune && git log --oneline --decorate -5 origin/main` (`f1103dfd ...`).
- DONE (local+checks, Wave33 Inbox decomposition): `Заявки` first screen is now reduced to the five triage controls only — `mode`, `queue slice`, `search`, `owner scope`, `refresh` — while saved views/share, advanced filters, view settings, and bulk forms moved into a bounded secondary panel without changing Wave24-30 backend contracts. Targeted operator workflow lane was updated to the new secondary surfaces and current routing/calendar semantics and is now green (`5 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave33-a1.md`, `console-web/src/components/CaseList.tsx`, `console-web/e2e/inspect_case.spec.ts`, `cd console-web && npm run lint -- --file src/components/CaseList.tsx --file src/components/InboxView.tsx --file e2e/inspect_case.spec.ts --file e2e/smoke.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://localhost:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium --grep "inspect first case|manager history modes hide queue views and keep owner scope role-gated|manage and apply action macro|action feedback hides raw sync reason codes and keeps reopen internal-only|booking no-show reopens resolved case and preserves case-booking semantics"` (`5 passed`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local+checks, Wave34 Calendar decomposition): `Записи` first screen now keeps only queue triage inline (`mode`, `queue slice`) plus compact queue/scheduling summaries, while filters, saved views/share, scheduling, and booking governance/actions moved into bounded secondary panels. Follow-up governance no longer sits inline on booking cards and is only reachable from the booking action panel. Targeted calendar workflow lane was updated to open the new secondary surfaces and is green (`4 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave34-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/e2e/inspect_case.spec.ts`, `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium --grep "inspect first case|manager history modes hide queue views and keep owner scope role-gated|booking no-show reopens resolved case and preserves case-booking semantics|calendar secondary panels isolate filters and booking actions"` (`4 passed`).
- DONE (local+checks, Wave35 operator proof): the rebuilt Inbox/Calendar surfaces now have deterministic workflow/layout proof instead of backend-only confidence. The targeted operator lane now covers inbox saved views, team presets, and share URLs; calendar follow-up governance from the booking action panel; routing-profile-disabled reassignment states; and medium-width desktop layout assertions for both `Заявки` and `Записи`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave35-a1.md`, `console-web/e2e/inspect_case.spec.ts`, `cd console-web && npm run lint -- --file e2e/inspect_case.spec.ts --file src/components/CaseList.tsx --file src/app/calendar/page.tsx` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium --grep "inspect first case|saved views, team presets, and share urls stay reachable from inbox secondary surfaces|follow-up governance stays inside the calendar booking panel|routing profile restrictions stay visible in reassignment panel|medium-width inbox and calendar keep primary queue surfaces visible|calendar secondary panels isolate filters and booking actions|booking no-show reopens resolved case and preserves case-booking semantics"` (`7 passed`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`). Wave35 is now merged to `main` through the stacked chain ending in `PR #956`.
- DONE (docs-only, Wave31 routing re-check): after Wave33-Wave35 closed locally and moved into `PR #952`/`PR #953`/`PR #954`, the routing contract still exposes only Wave29/Wave30 server-owned inputs: access eligibility, open-case load, booking follow-up continuity, SLA-sensitive scoring, and routing profiles (`available` / `paused` / `follow_up_only` + optional capacity). `ConsoleCaseRoutingPolicyType` remains limited to `least_open_cases` and `follow_up_sla_balance`, and unknown `skills_presence` stays rejected in contract tests, so routing v2 remains explicit `no-go / hold` until new server-owned assignee capability inputs exist. Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave31-a1.md`, `truffles-api/app/services/console_case_routing.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_cases_helpers.py`, `docs/CONSOLE_AUDIT/UX_BACKLOG.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-08-inbox-calendar-ux-logic-audit-a1.md`, `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local+checks, Wave36 Calendar operator-grade rebuild): `Записи` now read as a plain-language operator surface instead of a mixed `follow-up/governance` screen. Calendar hides technical/service follow-up owners from selectors, safely masks historical technical owners on cards and in the action panel, formats customer phones consistently, and rebuilds booking creation into a guided composer `мастер -> услуга -> день -> слот -> клиент` with case-prefill, inline validation, and disabled submit until the record is coherent. Backend now rejects weak booking payloads without `customer_name` / normalized `customer_phone` / `service_type` and rejects `result=rebooked` without a linked booking id. Final visual passes at `1280px` and `1440px` confirmed readable filters, wrapped overdue-only copy, non-overflow scheduling/action panels, safe owner labels, and no raw technical account noise. Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave36-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/routers/calendar.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `truffles-api/tests/test_calendar_noshow_followup_router.py`, `truffles-api/tests/test_console_openapi_calendar_contract.py`, `contracts/console_api/openapi.v1.yaml`, `/tmp/wave36-calendar-queue-1280.png`, `/tmp/wave36-calendar-filters-1280.png`, `/tmp/wave36-calendar-scheduling-1280.png`, `/tmp/wave36-calendar-actions-1280.png`, `/tmp/wave36-calendar-scheduling-1440.png`, `/tmp/wave36-calendar-actions-1440.png`, `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium --grep "calendar secondary panels isolate filters and booking actions|calendar hides technical follow-up owners and keeps operator-safe labels|guided booking composer blocks invalid submit until service name phone and slot are coherent, then creates booking|no-show rebook path requires linked booking selection before submit|medium-width inbox and calendar keep primary queue surfaces visible|booking no-show reopens resolved case and preserves case-booking semantics"` (`6 passed`), `cd truffles-api && pytest -q tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py tests/test_console_openapi_calendar_contract.py` (`42 passed`), `cd truffles-api && ruff check app/routers/calendar.py tests/test_calendar_bookings_router.py tests/test_calendar_noshow_followup_router.py tests/test_console_openapi_calendar_contract.py` (`pass`).

- DONE (local+checks, inbox Wave29 richer routing v1): case routing is no longer limited to `least_open_cases`. Backend now supports an opt-in explainable policy `follow_up_sla_balance` with server-owned score/explainability fields, explicit booking `NO_SHOW` follow-up continuity inputs, and SLA-weighted load balancing; frontend single-case and bulk reassignment surfaces now expose an explicit policy selector defaulting to the richer policy in UI while preserving `least_open_cases` as a fallback. Evidence: `truffles-api/app/services/console_case_routing.py`, `truffles-api/app/routers/console.py`, `truffles-api/app/schemas/console.py`, `truffles-api/tests/test_console_cases_helpers.py`, `truffles-api/tests/test_console_queue_state_api.py`, `truffles-api/tests/test_console_openapi_calendar_contract.py`, `contracts/console_api/openapi.v1.yaml`, `console-web/src/lib/api-client.ts`, `console-web/src/components/CaseConversation.tsx`, `console-web/src/components/CaseList.tsx`, `console-web/src/types/api.generated.ts`, `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave29-a1.md`, `cd truffles-api && pytest -q tests/test_console_cases_helpers.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`87 passed`), `cd truffles-api && ruff check app/routers/console.py app/schemas/console.py app/services/console_case_routing.py tests/test_console_cases_helpers.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/api-client.ts --file src/components/CaseConversation.tsx --file src/components/CaseList.tsx` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+checks, calendar Wave28 supervisor-grade governance): `Записи` now carry explicit booking follow-up ownership/due semantics and an explicit queue history mode on top of the Wave24-27 canon. Backend adds `appointments.follow_up_owner_id`/`follow_up_due_at`, `/calendar/bookings` filters for `follow_up_owner_id` and `follow_up_overdue`, deterministic `NO_SHOW` defaults, and bounded owner/admin mutation `POST /calendar/bookings/{booking_id}/follow-up-governance`; frontend calendar now persists/restores `queue_mode`, follow-up owner/overdue filters, surfaces owner/due/overdue chips, and exposes supervisor reassignment/due controls without opening richer routing. Evidence: `truffles-api/app/models/appointment.py`, `truffles-api/migrations/055_add_booking_follow_up_governance.sql`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `truffles-api/app/services/console_queue_state.py`, `truffles-api/tests/test_calendar_noshow_followup_router.py`, `truffles-api/tests/test_console_queue_state_api.py`, `console-web/src/lib/calendar-bookings.ts`, `console-web/src/lib/inbox-workspace.ts`, `console-web/src/lib/queue-state.ts`, `console-web/src/app/calendar/page.tsx`, `contracts/console_api/openapi.v1.yaml`, `console-web/src/types/api.generated.ts`, `cd truffles-api && pytest -q tests/test_calendar_noshow_followup_router.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`40 passed`), `cd truffles-api && ruff check app/models/appointment.py app/routers/calendar.py app/services/appointment_service.py app/services/console_queue_state.py tests/test_calendar_noshow_followup_router.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/calendar-bookings.ts --file src/lib/queue-state.ts --file src/lib/inbox-workspace.ts --file src/app/calendar/page.tsx` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+checks, inbox/calendar Wave27 shareable queue URLs): queue links are now reproducible for both `Заявки` and `Записи` on top of Wave24-26 canon. Backend adds read access to one saved view via `GET /console/v1/queue-state/views/{view_id}`; frontend inbox/calendar now restore by precedence `URL queue params -> URL view_id -> server current -> managed team default -> personal default -> local fallback`, keep the browser URL synchronized with the active queue slice, and expose one-click `Копировать ссылку` actions without leaking local-only presentation state. Evidence: `truffles-api/app/routers/console.py`, `truffles-api/tests/test_console_saved_views_api.py`, `console-web/src/lib/queue-state.ts`, `console-web/src/lib/api-client.ts`, `console-web/src/components/CaseList.tsx`, `console-web/src/app/calendar/page.tsx`, `contracts/console_api/openapi.v1.yaml`, `cd truffles-api && pytest -q tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`40 passed`), `cd truffles-api && ruff check app/routers/console.py app/schemas/console.py tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/api-client.ts --file src/lib/queue-state.ts --file src/components/CaseList.tsx --file src/app/calendar/page.tsx` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+checks, inbox/calendar Wave26 managed team presets): managed team presets are now implemented for both `Заявки` and `Записи` on top of the same Wave25 saved-view object. Backend extends `console_saved_views` with `scope`, `created_by_agent_id`, `target_branch_id`, and `target_role`, keeps one `/console/v1/queue-state/views` catalog/CRUD contract, and enforces owner/admin-only managed preset mutation with branch/role targeting. Frontend inbox/calendar now show personal views plus team presets in one catalog, restore defaults by precedence `URL override -> server current -> managed team default -> personal default -> local fallback`, and let owner/admin save/update/delete branch/role-targeted team defaults without forking a second preset model. Evidence: `truffles-api/app/models/console_saved_view.py`, `truffles-api/app/services/console_saved_views.py`, `truffles-api/migrations/054_extend_console_saved_views_for_team_presets.sql`, `truffles-api/tests/test_console_saved_views_api.py`, `console-web/src/lib/queue-state.ts`, `console-web/src/components/CaseList.tsx`, `console-web/src/app/calendar/page.tsx`, `contracts/console_api/openapi.v1.yaml`, `cd truffles-api && pytest -q tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`38 passed`), `cd truffles-api && ruff check app/models/__init__.py app/models/console_saved_view.py app/services/console_saved_views.py app/routers/console.py app/schemas/console.py tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/queue-state.ts --file src/lib/api-client.ts --file src/components/CaseList.tsx --file src/app/calendar/page.tsx` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+checks, inbox/calendar Wave25 personal saved views): personal named saved views are now implemented for both `Заявки` and `Записи` on top of Wave24 `Queue State Canon`. Backend added `console_saved_views` storage plus `/console/v1/queue-state/views` `GET/POST/PATCH/DELETE`; frontend inbox/calendar now list, save, apply, update, delete, and mark one default personal view per surface, while saved-view payload stays limited to canonical operational queue state and restore precedence becomes `URL override -> server current -> default saved view -> local fallback`. Evidence: `truffles-api/app/models/console_saved_view.py`, `truffles-api/app/services/console_saved_views.py`, `truffles-api/migrations/053_add_console_saved_views.sql`, `truffles-api/tests/test_console_saved_views_api.py`, `console-web/src/lib/queue-state.ts`, `console-web/src/lib/api-client.ts`, `console-web/src/components/CaseList.tsx`, `console-web/src/app/calendar/page.tsx`, `contracts/console_api/openapi.v1.yaml`, `cd truffles-api && pytest -q tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`31 passed`), `cd truffles-api && ruff check app/models/__init__.py app/models/console_saved_view.py app/services/console_saved_views.py app/routers/console.py app/schemas/console.py tests/test_console_saved_views_api.py tests/test_console_queue_state_api.py tests/test_console_openapi_calendar_contract.py` (`pass`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/queue-state.ts --file src/lib/api-client.ts --file src/components/CaseList.tsx --file src/app/calendar/page.tsx` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+checks, inbox/calendar Wave24 queue-state canon): server-owned current queue state is now implemented for both `Заявки` and `Записи`. Backend added `console_queue_states` storage plus `/console/v1/queue-state/current` `GET/PUT`; frontend inbox/calendar now restore operational queue state by precedence `URL override -> server current state -> local fallback`, while local-only workspace state remains limited to presentation/context (`selected case`, side panel, visible fields). Evidence: `truffles-api/app/models/console_queue_state.py`, `truffles-api/app/services/console_queue_state.py`, `truffles-api/migrations/052_add_console_queue_states.sql`, `truffles-api/tests/test_console_queue_state_api.py`, `console-web/src/lib/queue-state.ts`, `console-web/src/components/CaseList.tsx`, `console-web/src/app/calendar/page.tsx`, `contracts/console_api/openapi.v1.yaml`, `cd truffles-api && pytest -q tests/test_console_queue_state_api.py tests/test_console_cases_helpers.py tests/test_console_openapi_calendar_contract.py` (`78 passed`), `cd truffles-api && python3 scripts/generate_openapi.py --check` (`pass`), `cd console-web && npm run generate:api` (`pass`), `cd console-web && npm run lint -- --file src/lib/inbox-workspace.ts --file src/lib/inbox-case-filters.ts --file src/lib/queue-state.ts --file src/components/CaseList.tsx --file src/app/calendar/page.tsx --file src/lib/calendar-bookings.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`).
- DONE (local+docs+checks, inbox/calendar post-closeout maturity analysis): owner-approved follow-up analysis is now canonized in the current worktree. `Wave23` localizes the remaining maturity defects as `D1 local-only queue state`, `D2 no server-owned saved-view object`, `D3 non-shareable queue URLs`, `D4 calendar not yet supervisor-grade`, and `D5 routing inputs too narrow`; `Wave24` is now the first bounded execution block via server-owned `Queue State Canon` before any saved views, managed presets, shareable queue URLs, or richer routing work. Evidence: `docs/TASK_PACKAGES/TP-2026-03-07-inbox-calendar-ux-reconstruction-wave23-a1.md`, `docs/TASK_PACKAGES/TP-2026-03-07-inbox-calendar-ux-reconstruction-wave24-a1.md`, `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local+live+docs+merge, inbox/calendar semantic closure): `CONSOLE-INBOX-CALENDAR-UX-RECONSTRUCTION-WAVE22-A1` is closed end-to-end after `PR #944` (`dc8b720f`) shipped the deterministic manager/admin matrix and `PR #946` (`acc887dc`) shipped the live-proof helper hardening plus explicit safe-case evidence. Final live no-mocks proof passed on demo case `2e2de879-e4be-405e-83f6-c11dd95cad65`; user-provided case `01fdf4ed-ccdd-4752-9592-f44ee2614458` was confirmed accessible but unsuitable because the hydrated state was `В работе` with no visible reopen control. Evidence: `cd console-web && PLAYWRIGHT_BASE_URL=http://localhost:3100 npx playwright test e2e/inspect_case.spec.ts --project=chromium --reporter=line --workers=1` (`6 passed, 1 skipped`), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz INSPECT_CASE_USE_MOCKS=0 INSPECT_CASE_LIVE_CASE_ID=2e2de879-e4be-405e-83f6-c11dd95cad65 npx playwright test e2e/inspect_case.spec.ts --grep @wave22-live-proof --project=chromium --reporter=line --workers=1` (`1 passed`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local+checks+docs, pending PR merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW16-A705` executed after merged wave20 PR `#926` (`7b054918`) with binary DoD matrix pass on merged-main (`console.py=24353 <= 24396`, `ProvisioningWizard.tsx=4287 <= 4296`, deterministic lanes `35 passed` and `32 passed, 23 deselected`, delegation contract verified in service/router); fail-closed decision recorded as `UX-11 = Fixed`, `UX-12 = Fixed`, and wave21 is not opened. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review16-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review16-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/926`.
- DONE (local+docs+checks, auth helper unification): block `CONSOLE-E2E-AUTH-HELPER-UNIFICATION-A1` executed in current worktree: added shared helper `console-web/e2e/support/keycloak-auth.ts` and migrated `console-web/e2e/login.spec.ts`, `console-web/e2e/smoke.spec.ts`, `console-web/e2e/inspect_case.spec.ts` to unified Keycloak auth flow (`start/login/isAuthGate`) to remove local helper drift; started follow-up TP `docs/TASK_PACKAGES/TP-2026-03-06-console-e2e-auth-helper-unification-a1.md` and synced canon map/session pointer. Evidence: `cd console-web && rg -n "async function (startKeycloakLogin|loginThroughKeycloak)" e2e/login.spec.ts e2e/smoke.spec.ts e2e/inspect_case.spec.ts` (no matches), `cd console-web && npm run lint -- --file e2e/support/keycloak-auth.ts --file e2e/login.spec.ts --file e2e/smoke.spec.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://localhost:3100 npx playwright test e2e/inspect_case.spec.ts --project=chromium --reporter=line` (`pass`), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz npx playwright test e2e/login.spec.ts --project=chromium-login --reporter=line` (`3 passed`), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz INSPECT_CASE_USE_MOCKS=0 npx playwright test e2e/inspect_case.spec.ts --project=chromium --reporter=line` (`pass`, calendar fallback), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz npx playwright test e2e/smoke.spec.ts --project=chromium --reporter=line --grep "should display filter controls|should navigate to Settings"` (`2 passed`), `scripts/session_check.sh` (`Session OK`).
- DONE (local+docs+checks, inbox/calendar wave4 closeout): closeout block `CONSOLE-INBOX-CALENDAR-UX-RECONSTRUCTION-CLOSEOUT-A1` executed in current worktree with formal release discipline artifacts: added closeout TP (`docs/TASK_PACKAGES/TP-2026-03-05-inbox-calendar-ux-reconstruction-closeout-a1.md`), added canary/go-no-go/rollback runbook (`docs/runbooks/INBOX_CALENDAR_WAVE4_RELEASE.md`), added runtime rollback toggle `NEXT_PUBLIC_CASE_SSE_ENABLED=0` in `console-web/src/hooks/useCaseData.ts`, hardened live no-mocks lane in `console-web/e2e/inspect_case.spec.ts` to emit reasoned auth-gate skip with direct-case fallback, and synced master/wave4/session docs. Evidence: `cd console-web && npm run lint -- --file src/hooks/useCaseData.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://localhost:3100 npx playwright test e2e/inspect_case.spec.ts --reporter=line` (`pass`), `cd console-web && INSPECT_CASE_USE_MOCKS=0 PLAYWRIGHT_BASE_URL=http://localhost:3100 npx playwright test e2e/inspect_case.spec.ts --reporter=line` (`skip`, `auth gate` reasoned), `scripts/session_check.sh` (`Session OK`).
- DONE (local+docs+checks, live auth hardening follow-up): follow-up block `CONSOLE-E2E-LIVE-AUTH-HARDENING-A1` executed after PR #925 merge: created TP `docs/TASK_PACKAGES/TP-2026-03-05-console-e2e-live-auth-hardening-a1.md`, hardened `console-web/e2e/inspect_case.spec.ts` with inbox workspace storage cleanup, API-backed live case discovery, and deterministic calendar-surface fallback (with re-login recovery) instead of flaky auth-gate skip/fail paths on empty queue; closeout TP residual link updated to concrete follow-up TP and canon map synced (`STRUCTURE.md`). Evidence: `cd console-web && npm run lint -- --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://localhost:3100 npx playwright test e2e/inspect_case.spec.ts --project=chromium --reporter=line` (`pass`), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz npx playwright test e2e/login.spec.ts --project=chromium-login --reporter=line` (`3 passed`), `cd console-web && set -a && source /home/zhan/secrets/console-e2e.env && set +a && E2E_USE_STORAGE_STATE=1 E2E_DETERMINISTIC_AUTH=0 PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz INSPECT_CASE_USE_MOCKS=0 npx playwright test e2e/inspect_case.spec.ts --project=chromium --reporter=line` (`pass`, calendar fallback on empty queue), `scripts/session_check.sh` (`Session OK`).
- DONE (merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW12-A705` merged via PR `#918` (`1a899203`) with fail-closed merged-main decision preserved: `UX-11/UX-12` remain `Open (Mitigated wave16; wave17 required)`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review12-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review12-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/918`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE17-A705` merged via PR `#919` (`1a11ddeb`) as next bounded structural split for `UX-11/UX-12`: extracted rollback normalization orchestration from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`prepare_branch_change_rollback_payload`), extracted go-live decision/waiver submit orchestration from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-branch-actions.ts` (`submitGoLiveDecisionMutation`, `submitGoLiveWaiverMutation`), and revalidated deterministic lane (`py_compile`, `pytest 35 passed`, `pytest branch_change 25 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`). Follow-up closure contract locked via `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review13-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave17-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave17-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/919`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW13-A705` merged via PR `#920` (`9fc46c94`) on merged-main wave17 evidence (`console.py=24381`, `ProvisioningWizard.tsx=4325`, deterministic lane retained: `35 passed` + `branch_change 25 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave17; wave18 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave18-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review13-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review13-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave18-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/920`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE18-A705` merged via PR `#921` (`6b31951f`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change response assembly from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`build_branch_change_response`), extracted generic provisioning branch mutation submit-flow from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-branch-actions.ts` (`submitBranchMutation`), and revalidated deterministic lane (`wc 24390/4323`, `py_compile`, `pytest 35 passed`, `pytest branch_change 27 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`). Follow-up closure contract locked via `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review14-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave18-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave18-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review14-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/921`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW14-A705` merged via PR `#923` (`41fd52bd`) on merged-main wave18 evidence (`console.py=24390`, `ProvisioningWizard.tsx=4323`, deterministic lane retained: `35 passed` + `branch_change 27 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave18; wave19 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave19-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review14-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review14-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave19-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/923`.
- DONE (local+tests+docs, pending PR merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE19-A705` executed as next bounded structural split for `UX-11/UX-12`: extracted branch-change context resolver `get_branch_for_change_context` into `truffles-api/app/services/console_branch_changes.py` and rewired `validate/publish/rollback` handlers in `truffles-api/app/routers/console.py`; extracted shared wizard reset-state helper `createProvisioningWizardResetState` into `console-web/src/components/provisioning-wizard-state.ts` and rewired `console-web/src/components/ProvisioningWizard.tsx`; deterministic lane retained (`wc 24396/4296`, `py_compile`, `pytest 35 passed`, `pytest branch_change 29 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`). Follow-up closure contract locked via `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review15-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave19-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave19-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review15-a705.md`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE4-A705` merged via PR `#891` (`7ad5dc3d`) as next bounded structural split for `UX-11/UX-12`: extracted onboarding readiness hard-gate slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_onboarding_readiness.py` with deterministic tests `truffles-api/tests/test_console_onboarding_readiness.py` (`4 passed`), extracted provisioning readiness timeline/scorecard view from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-readiness-panel.tsx`, and revalidated UVC contract lane (`console-web build`, targeted e2e `26 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave4-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave4-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/891`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-FINAL-CLOSE-A705` merged via PR `#892` (`be222b9d`) after wave4 with explicit residual decision and follow-up contract: deterministic merged-main baseline confirmed (`console.py=24888`, `ProvisioningWizard.tsx=4742`, `pytest 7 passed`, targeted lint clean), final-close artifact published, and `UX-11/UX-12` kept fail-closed as `Open (Mitigated wave4; wave5 required)`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-final-close-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-final-close-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/892`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE5-A705` merged via PR `#893` (`94ee1152`) as next bounded structural split for `UX-11/UX-12`: extracted reusable query/limit/uuid/bool validation helpers into `truffles-api/app/services/console_router_utils.py` with extended deterministic tests `truffles-api/tests/test_console_router_utils.py`, extracted wizard shell panels from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-shell-panels.tsx`, and revalidated targeted contracts (`py_compile`, `pytest 16 passed`, frontend lint/build, targeted platform-admin e2e `26 passed`, `session_check`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave5-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave5-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/893`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW-A705` merged via PR `#894` (`7c1634a5`) with explicit fail-closed decision from merged-main evidence (`console.py=24897`, `ProvisioningWizard.tsx=4679`, `pytest 16 passed`): `UX-11/UX-12` remain `Open (Mitigated wave5; wave6 required)` and wave6 implementation contract is locked. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave6-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/894`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE6-A705` merged via PR `#895` (`9ae410bb`) as next bounded structural split for `UX-11/UX-12`: extracted fleet lifecycle/payment/service resolver layer into `truffles-api/app/services/console_fleet_state.py` with deterministic tests `truffles-api/tests/test_console_fleet_state.py` (`24 passed` with retained wave coverage), extracted provisioning JSON payload handlers into `console-web/src/components/provisioning-wizard-json-payloads.ts`, reduced monolith LOC (`console.py: 24897 -> 24743`, `ProvisioningWizard.tsx: 4679 -> 4617`), and revalidated targeted contracts (`py_compile`, frontend lint/build, targeted platform-admin e2e `26 passed`, `session_check`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave6-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave6-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/895`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW2-A705` merged via PR `#896` (`4c9817d8`) with explicit fail-closed decision on merged-main evidence (`console.py=24743`, `ProvisioningWizard.tsx=4617`, `pytest 24 passed`): `UX-11/UX-12` remain `Open (Mitigated wave6; wave7 required)` and wave7 contract is locked. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review2-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave7-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review2-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/896`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE7-A705` merged via PR `#897` (`4269e31a`) as next bounded structural split for `UX-11/UX-12`: extracted membership/role state orchestration helpers into `truffles-api/app/services/console_membership_state.py` with deterministic tests `truffles-api/tests/test_console_membership_state.py` (`30 passed` with retained wave coverage), extracted wizard state lifecycle/bootstrap/hydration helpers into `console-web/src/components/provisioning-wizard-state.ts`, reduced monolith LOC (`console.py: 24743 -> 24603`, `ProvisioningWizard.tsx: 4617 -> 4544`), and revalidated targeted contracts (`py_compile`, frontend lint/build, targeted platform-admin e2e `26 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave7-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave7-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/897`, `truffles-api/app/services/console_membership_state.py`, `console-web/src/components/provisioning-wizard-state.ts`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-FINAL-REVIEW3-A705` merged via PR `#898` (`14ad5f64`) as fail-closed merged-main decision block after wave7 (`console.py=24606`, `ProvisioningWizard.tsx=4544`, retained deterministic suite `30 passed`): `UX-11/UX-12` remain `Open (Mitigated wave7; wave8 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave8-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-final-review3-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-final-review3-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave8-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/898`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE8-A705` merged via PR `#899` (`fb43840a`) as next bounded structural split for `UX-11/UX-12`: extracted go-live normalization/waiver/gate slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_onboarding_readiness.py` with extended deterministic tests `truffles-api/tests/test_console_onboarding_readiness.py`, extracted provisioning billing/working-hours/booking-settings JSON sync helpers from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-state.ts`, and revalidated targeted contracts (`py_compile`, `ruff`, retained deterministic suite `35 passed`, frontend lint/build, targeted platform-admin e2e `26 passed`, CI run `22665215404` green). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave8-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave8-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/899`, `https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/22665215404`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW4-A705` merged via PR `#900` (`a1e4b54a`) after merged-main wave8 evidence (`console.py=24554`, `ProvisioningWizard.tsx=4552`, retained deterministic suite `35 passed`) with explicit fail-closed decision: `UX-11/UX-12` remained `Open (Mitigated wave8; wave9 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave9-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review4-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review4-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave9-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/900`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE9-A705` merged via PR `#901` (`cc956188`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change helper slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py`, extracted Provisioning Wizard branch action payload builders into `console-web/src/components/provisioning-wizard-branch-actions.ts`, and revalidated deterministic lane (`py_compile`, `ruff`, `pytest 35 passed`, `pytest branch_change 10 passed`, frontend lint/build, targeted e2e `2 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave9-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave9-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/901`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW5-A705` merged via PR `#902` (`da8379d5`) on merged-main wave9 evidence (`console.py=24493`, `ProvisioningWizard.tsx=4452`, deterministic suite retained: `35 passed` + `branch_change 10 passed`, frontend lane green) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave9; wave10 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave10-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review5-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review5-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave10-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/902`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE10-A705` merged via PR `#903` (`8bed2a80`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change normalization slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` and extracted provisioning account-action payload builders from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-account-actions.ts`; deterministic lane retained (`py_compile`, `ruff`, `pytest 35 passed`, `pytest branch_change 10 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave10-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave10-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/903`, `console-web/src/components/provisioning-wizard-account-actions.ts`, `truffles-api/app/services/console_branch_changes.py`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW6-A705` merged via PR `#904` (`91f9e79c`) on merged-main wave10 evidence (`console.py=24366`, `ProvisioningWizard.tsx=4423`, deterministic suite retained: `35 passed` + `branch_change 10 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave10; wave11 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave11-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review6-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review6-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave11-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/904`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE11-A705` merged via PR `#904` (`91f9e79c`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change context/rollback/status helper slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py`, extracted provisioning autopilot derived-state/payload assembly into `console-web/src/components/provisioning-wizard-autopilot.ts`, reduced monolith LOC (`console.py: 24366 -> 24365`, `ProvisioningWizard.tsx: 4423 -> 4365`), and revalidated deterministic lane (`py_compile`, `pytest 35 passed`, `pytest branch_change 12 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`); follow-up closure contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review7-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave11-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave11-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review7-a705.md`, `console-web/src/components/provisioning-wizard-autopilot.ts`, `truffles-api/app/services/console_branch_changes.py`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/904`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW7-A705` merged via PR `#905` (`7fbba1d8`) on merged-main wave11 evidence (`console.py=24365`, `ProvisioningWizard.tsx=4365`, deterministic suite retained: `35 passed` + `branch_change 12 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave11; wave12 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave12-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review7-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review7-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave12-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/905`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE12-A705` merged via PR `#906` (`96b487a4`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change payload prepare/validation orchestration from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`prepare_branch_change_payload`), unified router/service normalization kwargs for `draft/validate/publish/rollback`, extracted autopilot run-state/validation/toggle helpers into `console-web/src/components/provisioning-wizard-autopilot.ts`, and revalidated deterministic lane (`py_compile`, `pytest 35 passed`, `pytest branch_change 13 passed`, frontend lint/build, targeted e2e `2 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave12-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave12-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/906`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW8-A705` merged via PR `#907` (`f5f7ef3b`) after merged-main wave12 evidence (`console.py=24365`, `ProvisioningWizard.tsx=4365`, deterministic lane retained: `35 passed` + `branch_change 13 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave12; wave13 required)`; follow-up implementation contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave13-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review8-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closure-review8-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave13-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/907`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE13-A705` merged via PR `#908` (`332d1e3d`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change list response orchestration from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`build_branch_change_list_response`), extracted autopilot mutation success-sync orchestration from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-autopilot.ts` (`syncAutopilotMutationSuccess`), reduced monolith LOC (`console.py: 24365 -> 24354`, `ProvisioningWizard.tsx: 4365 -> 4355`), and revalidated deterministic lane (`py_compile`, `pytest 35 passed`, `pytest branch_change 15 passed`, frontend lint/build, targeted e2e 2 passed, `session_check`); follow-up closure contract locked via `TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review9-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave13-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave13-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review9-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/908`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW9-A705` merged via PR `#909` (`fbee9c40`) on merged-main wave13 evidence (`console.py=24354`, `ProvisioningWizard.tsx=4355`, deterministic lane retained: `35 passed` + `branch_change 15 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave13; wave14 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave14-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review9-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review9-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave14-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/909`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE14-A705` merged via PR `#910` (`fdc20429`) as next bounded structural split for `UX-11/UX-12`: extracted branch-change validation/publish-failed state transitions from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`apply_branch_change_validation_result`, `apply_branch_change_publish_failed_state`), extracted shared branch mutation success/error orchestration from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-branch-actions.ts` (`syncBranchMutationSuccess`, `handleBranchMutationError`), and revalidated deterministic lane (`py_compile`, `pytest 35 passed`, `pytest branch_change 18 passed`, frontend lint clean, `session_check`); follow-up closure contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review10-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave14-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave14-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review10-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/910`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW10-A705` merged via PR `#911` (`52db4a47`) on merged-main wave14 evidence (`console.py=24358`, `ProvisioningWizard.tsx=4356`, deterministic lane retained: `35 passed` + `branch_change 18 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave14; wave15 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave15-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review10-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review10-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave15-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/911`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE15-A705` merged via PR `#913` (`b02dfa6e`) as next bounded structural split for `UX-11/UX-12`: extracted publish/rollback runtime state-transition helpers from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`apply_branch_change_publish_runtime_error_state`, `apply_branch_change_rollback_failed_state`, `apply_branch_change_rolled_back_state`), extracted go-live decision payload validation from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-branch-actions.ts` (`buildGoLiveDecisionPayload`, `buildGoLiveWaiverPayload`), and revalidated deterministic lane (`py_compile`, `ruff`, `pytest 35 passed`, `pytest branch_change 21 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`); follow-up closure contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review11-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave15-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave15-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review11-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/913`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW11-A705` merged via PR `#914` (`55414e7f`) on merged-main wave15 evidence (`console.py=24376`, `ProvisioningWizard.tsx=4349`, deterministic lane retained: `35 passed` + `branch_change 21 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave15; wave16 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave16-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review11-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review11-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave16-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/914`.
- DONE (local+tests+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE16-A705` merged via PR `#916` (`6c14b68d`) as next bounded structural split for `UX-11/UX-12`: extracted publish-success state-transition helper from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_branch_changes.py` (`apply_branch_change_published_state`), extracted shared go-live mutate submit flow from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-branch-actions.ts` (`submitGoLiveMutation`), and revalidated deterministic lane (`py_compile`, `ruff`, `pytest 35 passed`, `pytest branch_change 23 passed`, frontend lint/build, targeted e2e `2 passed`, `session_check`); follow-up closure contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review12-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave16-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave16-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review12-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/916`.
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW12-A705` merged via PR `#918` (`1a899203`) after merged-main wave16 evidence (`console.py=24380`, `ProvisioningWizard.tsx=4338`, deterministic lane retained: `35 passed` + `branch_change 23 passed`) with explicit fail-closed decision: `UX-11/UX-12` remain `Open (Mitigated wave16; wave17 required)`; follow-up implementation contract locked via `TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave17-a705.md`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review12-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-closure-review12-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave17-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/918`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE3-A705` delivered and merged via PR `#889` (`feeb60e1`) as bounded behavior-preserving structural split for `UX-11/UX-12`: extracted control-tower orchestration/program composition from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_control_tower_program.py`, extracted `ProvisioningWizard` derived-state builders into `console-web/src/components/provisioning-wizard-derived.ts`, and kept UVC contract lane green (`26 passed`).
- DONE (local+checks+docs+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSEOUT-A705` executed on merged `main` and merged via PR `#890` (`9fe7e8bd`) with deterministic revalidation and explicit residual decision for `UX-11/UX-12`; merge-red `deploy` build failure was resolved by strict status typing in `console-web/src/components/provisioning-wizard-derived.ts` + `console-web/src/components/provisioning-wizard-utils.ts` before merge. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-closeout-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-closeout-a705.md`, `https://github.com/k1ddy/Truffles-AI-Employee/pull/890`, `https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/22658261238`.
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE2-A705` delivered and merged via PR `#888` (`fd848ada`) as bounded behavior-preserving structural split for `UX-11/UX-12`: extracted control-tower pure helper layer from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_control_tower_utils.py` and extracted provisioning domain lexicon/field-guide/formatter layer from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-domain.ts`; added deterministic module tests `truffles-api/tests/test_console_control_tower_utils.py` (`5 passed`) and kept platform-admin lane contract green (`26 passed`).
- DONE (local+tests+merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-A705` wave1 delivered and merged via PR `#885` (`b7717be9`) as behavior-preserving structural split for `UX-11/UX-12` + merge-red fix: extracted `ProvisioningWizard` pure helpers into `console-web/src/components/provisioning-wizard-utils.ts`, extracted shared router env/query helpers into `truffles-api/app/services/console_router_utils.py`, added deterministic tests `truffles-api/tests/test_console_router_utils.py` (`5 passed`), and stabilized deep-link fallback in `console-web/e2e/platform-admin.spec.ts`.
- DONE (local+tests+merge): `UVC-UX-DEDUP-INTENT-MAP-A705` step `1+2` delivered and merged via PR `#886` (`dbd810d0`) in existing tabs without adding new top-level routes: de-duplicated primary CTA placement in `Integrations` (single global context CTA moved into workspace-guidance block), normalized operator wording (`Ops` naming), and added unified intent-map hints (`факт -> действие -> проверка`) in `Tenants/Integrations/Workspace/Ops`.
- DONE (local+automation+ci-gate+docs): `UVC-UX-OPERATIONS-GOVERNANCE-CLOSEOUT-A705` delivered as fail-closed audit consistency layer on top of the existing UVC control-loop: added deterministic checker `scripts/check_console_audit_governance.py` + tests `truffles-api/tests/test_check_console_audit_governance.py` (`4 passed`), wired checker into `scripts/platform_admin_control_loop.sh` (`audit_governance` step + governance artifact in summary) and CI `console-contract-predeploy`, and cleaned stale canonical drift in `docs/CONSOLE_AUDIT/CANON_VS_IMPLEMENTED.md` / `docs/CONSOLE_AUDIT/UX_BACKLOG.md` (duplicate IDs removed, partial gap tags normalized). Evidence: `docs/TASK_PACKAGES/TP-2026-03-03-uvc-ux-operations-governance-closeout-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-03-uvc-operations-governance-closeout-a705.md`, `scripts/check_console_audit_governance.py`, `scripts/platform_admin_control_loop.sh`, `.github/workflows/ci.yml`, `/tmp/console_audit_governance_a705.json`, `/tmp/platform_admin_control_loop/governance-closeout-a705/summary.json`, `/tmp/platform_admin_control_loop/governance-closeout-a705/governance_audit.json`; checks: `python3 -m py_compile scripts/check_console_audit_governance.py`, `pytest -q truffles-api/tests/test_check_console_audit_governance.py` (`4 passed`), `cd console-web && npm run check:uvc-antidrift` (`UVC anti-drift check passed`), `scripts/platform_admin_control_loop.sh --run-id governance-closeout-a705 --run-e2e 0 --output-root /tmp/platform_admin_control_loop` (`overall_status=pass`), `SESSION_AGENT=a705 scripts/session_check.sh` (`Session OK`).
- DONE (local+automation+tests+docs): `UVC-UX-STEADY-STATE-OPERATIONS-A705` delivered as operator-assist layer over existing Platform Admin control-loop without adding new UI tabs: added deterministic remediation generator `ops/platform_admin_remediation_assist.py` (plan/brief/commands from KPI guard), wired assist stage into `scripts/platform_admin_control_loop.sh` (`--run-remediation-assist`, `--remediation-strict`), updated scheduled workflow input `remediation_strict`, and refreshed runbook/canon/master evidence docs. Evidence: `docs/TASK_PACKAGES/TP-2026-03-03-uvc-ux-steady-state-operations-a705.md`, `ops/platform_admin_remediation_assist.py`, `scripts/platform_admin_control_loop.sh`, `.github/workflows/platform-admin-control-loop.yml`, `truffles-api/tests/test_platform_admin_remediation_assist.py`, `docs/CONSOLE_AUDIT/artifacts/2026-03-03-uvc-steady-state-operations-a705.md`, `/tmp/platform_admin_control_loop/steady-ops-a705/summary.json`, `/tmp/platform_admin_control_loop/steady-ops-a705/remediation_plan.json`, `/tmp/platform_admin_control_loop/steady-ops-a705/remediation_brief.md`; checks: `python3 -m py_compile ops/platform_admin_remediation_assist.py`, `pytest -q truffles-api/tests/test_platform_admin_remediation_assist.py` (`3 passed`), `scripts/platform_admin_control_loop.sh --run-id steady-ops-a705 --run-e2e 0 --output-root /tmp/platform_admin_control_loop` (`overall_status=pass`).
- DONE (local+automation+post-merge+docs): `UVC-UX-PROGRAM-CLOSEOUT-A705` delivered as steady-state handoff without UI ownership drift: added single-entrypoint wrapper `scripts/platform_admin_control_loop.sh` (`kpi guard + anti-drift + optional e2e + summary artifact`), added scheduled/dispatch workflow `.github/workflows/platform-admin-control-loop.yml`, updated runbook/canon/master status, and validated on merged `main@c21ccf60` with stable runtime/e2e. Evidence: `docs/TASK_PACKAGES/TP-2026-03-03-uvc-ux-program-closeout-steady-loop-a705.md`, `scripts/platform_admin_control_loop.sh`, `.github/workflows/platform-admin-control-loop.yml`, `docs/runbooks/PLATFORM_ADMIN_CONTROL_LOOP.md`, `docs/CONSOLE_AUDIT/CANON_VS_IMPLEMENTED.md`, `docs/REPORTS/2026-02-22-universal-control-plane-v1-master-a500.md`, `/tmp/platform_admin_control_loop/local-a705/summary.json`, `/tmp/platform_admin_control_loop/local-a705/kpi_snapshot.json`, `/tmp/uvc_stage5_kpi_main_postmerge_c21_a705.json`; checks: `bash -n scripts/platform_admin_control_loop.sh`, `scripts/platform_admin_control_loop.sh --run-id local-a705 --run-e2e 0 --fail-level critical --output-root /tmp/platform_admin_control_loop` (`overall_status=pass`), `python3 ops/console_platform_admin_kpi_snapshot.py --pretty --output /tmp/uvc_stage5_kpi_main_postmerge_c21_a705.json`, `cd console-web && npm run check:uvc-antidrift` (`UVC anti-drift check passed`), `cd console-web && npm run lint -- --file e2e/platform-admin.spec.ts` (`No ESLint warnings or errors`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 npm run test:e2e -- --grep "Platform Admin Navigation|Platform Admin Tenants|Platform Admin Integrations"` (`26 passed`).
...
- DONE (local/code+tests+docs, Block E / E3.2 single-writer transition owner): full `E3.2` implementation delivered via `docs/TASK_PACKAGES/TP-2026-03-05-e3.2-single-writer-booking-profile-transition-a1.md`: introduced shared transition layer `truffles-api/app/services/booking_transition_owner.py` (phone resolution + profile materialization + booking state merge contract), integrated as single writer into `decision.py` and `tool_registry_service.py`, and added fail-closed quality reason `booking_transition_evidence_missing` in `ops/diagnose.py` when `calendar.book_slot:ok` lacks transition evidence (`transition_owner`, phone source, `profile_sync`). Added regressions in `truffles-api/tests/test_booking_appointments.py`, `truffles-api/tests/test_message_endpoint.py`, `truffles-api/tests/test_booking_quality_response_guard.py`. Checks: `python3 -m py_compile truffles-api/app/services/booking_transition_owner.py truffles-api/app/services/tool_registry_service.py truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_booking_appointments.py truffles-api/tests/test_message_endpoint.py truffles-api/tests/test_booking_quality_response_guard.py ops/diagnose.py`, `ruff check truffles-api/app/services/booking_transition_owner.py truffles-api/app/services/tool_registry_service.py truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_booking_appointments.py truffles-api/tests/test_message_endpoint.py truffles-api/tests/test_booking_quality_response_guard.py ops/diagnose.py` (`All checks passed`), `pytest -q truffles-api/tests/test_booking_appointments.py -k "book_slot_uses_remote_jid_phone_fallback or booking_transition_owner"` (`3 passed`), `pytest -q truffles-api/tests/test_booking_quality_response_guard.py -k "booking_transition_evidence_missing or stale_booking_carryover or mix_info_booking or booking_prompt_leak"` (`6 passed`), `pytest -q truffles-api/tests/test_message_endpoint.py -k "collect_with_full_slots_normalizes_to_book_slot or calendar_book_slot_ok_terminal_clear_blocks_followup_expected_reply or location_style_reference_reply_stays_booking_neutral"` (`3 passed`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local/code+tests+docs, Block E / E3.2 continuation for r27 blockers): deterministic continuity hardening extended after `booking-lock-20260304-firebreak-e2e3-a1-r27` findings: consult path now materializes `expected_reply_type=service_choice` (`reason=consult_booking_cta`) for unlocked `consult_reply` turns with explicit service hint and booking CTA; `specialist_missing` now emits canonical `info_sections=["master","specialist"]` (+ `specialist_name`) from tool layer; `calendar.list_slots:specialist_missing` followup contract switched to `name` (`calendar_list_slots_specialist_followup`) and protected in `decision.py` from generic slot-order override back to `time`; tool-level specialist info sections are authoritative over stale hints for this outcome. Evidence/code: `truffles-api/app/routers/webhook/response.py`, `truffles-api/app/services/tool_registry_service.py`, `truffles-api/app/services/expected_reply_contract.py`, `truffles-api/app/routers/webhook/decision.py`; regressions: `truffles-api/tests/test_message_endpoint.py::test_consult_reply_with_service_hint_sets_service_expected_reply_for_booking_cta`, `truffles-api/tests/test_message_endpoint.py::test_llm_policy_core_specialist_missing_prefers_tool_info_sections_and_name_followup`, `truffles-api/tests/test_expected_reply_contract.py::test_tool_contract_list_slots_specialist_missing_requests_name`; checks: `ruff check truffles-api/app/routers/webhook/response.py truffles-api/app/routers/webhook/decision.py truffles-api/app/services/tool_registry_service.py truffles-api/app/services/expected_reply_contract.py truffles-api/tests/test_message_endpoint.py truffles-api/tests/test_expected_reply_contract.py truffles-api/tests/test_booking_quality_response_guard.py truffles-api/tests/test_booking_appointments.py` (`All checks passed`), `pytest -q truffles-api/tests/test_expected_reply_contract.py -k "list_slots_prefers_time_when_service_known or list_slots_specialist_missing_requests_name"` (`2 passed`), `pytest -q truffles-api/tests/test_message_endpoint.py -k "consult_pack_writes_decision_meta or consult_reply_with_service_hint_sets_service_expected_reply_for_booking_cta or llm_policy_core_specialist_missing_prefers_tool_info_sections_and_name_followup or llm_policy_core_calendar_get_booking_not_found_normalized_to_master_info"` (`4 passed`), `pytest -q truffles-api/tests/test_booking_appointments.py -k "book_slot_uses_remote_jid_phone_fallback or booking_transition_owner_merges_slots_and_sets_appointment_id or booking_transition_owner_syncs_profile_from_remote_jid"` (`3 passed`), `pytest -q truffles-api/tests/test_booking_quality_response_guard.py -k "booking_transition_evidence_missing or stale_booking_carryover or mix_info_booking or booking_prompt_leak"` (`6 passed`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
- DONE (local/code+tests+sql-evidence, Block E / E3 deep-audit): cross-layer consultant data continuity hardening delivered via `docs/TASK_PACKAGES/TP-2026-03-04-e3-consultant-data-continuity-deep-audit-a1.md`: `calendar.book_slot` tool path now deterministically derives `customer_phone` from `user_phone -> remote_jid` fallback (`customer_phone_source` in meta/trace) and decision-path now syncs missing `user.name/user.phone` after successful `calendar.book_slot` (`profile_sync` + `user_phone_source_for_tool` observability) to prevent `appointments -> users` profile drift; evidence/code: `truffles-api/app/services/tool_registry_service.py`, `truffles-api/app/routers/webhook/decision.py`, tests `truffles-api/tests/test_booking_appointments.py::test_tool_registry_book_slot_uses_remote_jid_phone_fallback`, `truffles-api/tests/test_message_endpoint.py::test_llm_policy_core_collect_with_full_slots_normalizes_to_book_slot`; checks: `python3 -m py_compile truffles-api/app/services/tool_registry_service.py truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_booking_appointments.py truffles-api/tests/test_message_endpoint.py`, `pytest -q truffles-api/tests/test_booking_appointments.py -k "book_slot_uses_remote_jid_phone_fallback"` (`1 passed`), `pytest -q truffles-api/tests/test_message_endpoint.py -k "llm_policy_core_collect_with_full_slots_normalizes_to_book_slot"` (`1 passed`); runtime SQL deep-audit baseline (before fix) captured in `chatbot` DB: bot appointments `214` (`with_name=155`, `with_phone=100`) vs bot users `92` (`with_name=1`, `with_phone=1`) confirming contact materialization drift class is real and system-level.
- DONE (local/code+tests+docs, Block E / E2f.1 deterministic slice): semantic blocker remediation advanced in `docs/TASK_PACKAGES/TP-2026-03-05-e2f-firebreak-semantic-contract-closure-a1.md` with two concrete fixes from `r22` failure classes: (1) `ops/diagnose.py` calendar outcome derivation no longer marks `calendar.get_booking` as failure solely due generic `booking_blocked_reason` when explicit tool decision is successful (`not_found/ok/time_mismatch/...`), removing false `calendar_tool_contract_miss`; (2) discounts policy branch in `truffles-api/app/routers/webhook/decision.py` now suppresses stale booking sidecar for `expected_reply_type=intent_choice` and queue-transition turns (`pending_intent_queue` / `intent_queue_event`), preventing `Ещё был вопрос по записи...` leakage in FACT/info discounts replies. Added regressions: `truffles-api/tests/test_booking_quality_response_guard.py::test_calendar_outcome_not_found_not_forced_to_failure_by_booking_blocked_reason`, `truffles-api/tests/test_booking_quality_response_guard.py::test_calendar_outcome_without_decision_keeps_blocked_reason_failure`, `truffles-api/tests/test_message_endpoint.py::test_discounts_reply_with_expected_intent_choice_suppresses_booking_sidecar`; checks: `python3 -m py_compile ops/diagnose.py truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_booking_quality_response_guard.py truffles-api/tests/test_message_endpoint.py`, `pytest -q truffles-api/tests/test_booking_quality_response_guard.py` (`55 passed`), `pytest -q truffles-api/tests/test_message_endpoint.py -k "booking_info_interrupt or discounts_reply_with_expected_intent_choice_suppresses_booking_sidecar"` (`6 passed`), `ruff check ops/diagnose.py truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_booking_quality_response_guard.py truffles-api/tests/test_message_endpoint.py` (`All checks passed`).- DONE (local/code+e2e+kpi+docs, stage5 wave1/2): UVC UX Stage 5 rollout/efficiency groundwork delivered on current branch: rollout go/no-go matrix formalized (canary/cohort/fleet), baseline vs post KPI snapshots captured with no regression (`console_health=healthy`, `outbox_pending=0`, `outbox_failed=0`, `outbox_guard=ok`), legacy inventory executed for primary UVC path, and mixed-language `fallback/live` UX labels normalized in `Tenants/Integrations` while preserving API keys. Evidence: `docs/CONSOLE_AUDIT/artifacts/2026-03-03-uvc-stage5-rollout-report-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-03-uvc-stage5-legacy-removal-a705.md`, `console-web/src/app/integrations/page.tsx`, `console-web/src/app/tenants/tenants-page-helpers.ts`, `/tmp/uvc_stage5_kpi_snapshot_a705.json`, `/tmp/uvc_stage5_kpi_post_a705.json`, `/tmp/uvc_stage5_legacy_scan_a705.txt`; checks: `python3 ops/console_platform_admin_kpi_snapshot.py --pretty --output /tmp/uvc_stage5_kpi_snapshot_a705.json`, `python3 ops/console_platform_admin_kpi_snapshot.py --pretty --output /tmp/uvc_stage5_kpi_post_a705.json`, `cd console-web && npm run check:uvc-antidrift` (`UVC anti-drift check passed`), `cd console-web && npm run lint -- --file src/app/integrations/page.tsx --file src/app/tenants/tenants-page-helpers.ts --file e2e/platform-admin.spec.ts` (`No ESLint warnings or errors`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 npm run test:e2e -- --grep "Platform Admin Navigation|Platform Admin Tenants|Platform Admin Integrations"` (`26 passed`).
...
- DONE (local/code+tests+docs, Block E start): `E1 LLM-first firebreak` delivered via `TP-2026-03-03-e1-llm-first-firebreak-action-router-a1`: added env-gated router firebreak `LLM_POLICY_CORE_LLM_FIRST_FIREBREAK` in `truffles-api/app/routers/webhook/decision.py` to redirect deterministic semantic override candidates (`out_of_domain/escalation/rejection` branches) to `ai_response` owner path when enabled; observability added via `llm_first_firebreak_enabled/applied/reasons` in decision metadata and trace stage `llm_first_firebreak`; deterministic coverage added in `truffles-api/tests/test_message_endpoint.py` with legacy-off/feature-on assertions; program ТЗ sync updated with new forensic findings and A->F closure order in `docs/TASK_PACKAGES/TP-2026-02-19-llm-first-firebreak-program.md`; checks: `pytest -q truffles-api/tests/test_message_endpoint.py -k "llm_first_firebreak_out_of_domain_routes_to_ai_response or llm_first_firebreak_rejection_keeps_legacy_path_when_disabled"` (`2 passed, 270 deselected`), `pytest -q truffles-api/tests/test_message_endpoint.py -k "test_golden_cases or semantic_arbitration_off_keeps_master_without_location_rewrite"` (`31 passed, 241 deselected`), `ruff check truffles-api/app/routers/webhook/decision.py truffles-api/tests/test_message_endpoint.py` (`All checks passed`).
...
- DONE (local/code+tests+docs, Block E / E2d process-unblock): dedicated Task Package `docs/TASK_PACKAGES/TP-2026-03-05-e2d-acceptance-process-unblock-a1.md` delivered deterministic recovery path for acceptance lock retries without gate downgrade: added explicit `--allow-non-canonical-lock-retry` in `ops/diagnose.py` (`llm-quality` + `llm-quality-gates`) that permits unchanged lock fingerprint retry only when previous lock is non-canonical with process/preflight stop-reason allowlist; kept fail-closed guard by allowing this override in quality-constant only for lock-mode (replay remains blocked). Contract coverage added in `truffles-api/tests/test_booking_quality_status_gate.py` (`test_run_economy_allows_non_canonical_lock_retry_for_process_stop_reason`, `test_run_economy_blocks_non_canonical_lock_retry_for_non_process_stop_reason`, `test_quality_constant_acceptance_allows_non_canonical_lock_retry_for_lock_only`); checks: `pytest -q truffles-api/tests/test_booking_quality_status_gate.py -k "non_canonical_lock_retry or run_economy_blocks_lock_with_unchanged_non_canonical_fingerprint or run_economy_allows_lock_with_unchanged_canonical_fingerprint or quality_constant_acceptance_lane_requires_canonical_envelope"` (`6 passed, 82 deselected`), `ruff check ops/diagnose.py truffles-api/tests/test_booking_quality_status_gate.py` (`All checks passed`). `E2e` confirmed process unblock works (lock `r22` completed to full integrity); remaining E2 blocker is semantic.
...
### 2026-03-05 — P1 strict single-owner hard lock reinforcement (in progress closure)

**Что сделали:**
- В `truffles-api/app/routers/webhook/decision.py` закрыли post-hoc semantic rewrites в P1-контуре:
  - добавлен единый trace/meta helper `_record_semantic_override_block`,
  - hard-lock теперь fail-closes invalid action/mismatch без нормализации,
  - hard-lock precheck расширен на `action_changed`,
  - rewrite-ветки (`collect->booking/list_slots`, `service_query->location`, recovery rewrites) блокируются/не применяются при hard-lock.
- В `truffles-api/tests/test_message_endpoint.py` синхронизированы контрактные тесты с новой строгой моделью single-owner.
...
- `pytest -q truffles-api/tests/test_llm_policy_core.py` -> `12 passed`
...
**Остаток / блокер:**
- `EVAL_TIER=long pytest -q truffles-api/tests/test_demo_salon_eval.py` -> `1 failed, 15 passed`
- Open fail: `E508` (`missing must_include 'парковка'`) — residual quality/data continuity gap для parking carryover; не фиксируется P1-блоком без выхода за scope single-owner hard lock.

### 2026-03-05 — P1 strict single-owner hard lock closure (E508 resolved)

**Что сделали:**
- Закрыли residual `E508` в deterministic `info_bundle` carryover path без semantic rewrite:
  - `truffles-api/app/routers/webhook/info.py`: `force_parking_followup` расширен на capacity-followup (`сколько ... мест`) при активном `parking` carryover.
  - `truffles-api/tests/test_demo_salon_eval.py`: добавлен регресс-тест `test_info_bundle_parking_capacity_followup_keeps_parking_section`.
- P1 single-owner hard-lock инварианты сохранены: action/tool/intent post-hoc rewrite не возвращались.

**Evidence (deterministic + long eval):**
- `ruff check truffles-api/app/routers/webhook/info.py truffles-api/tests/test_demo_salon_eval.py` -> `All checks passed`
- `pytest -q truffles-api/tests/test_demo_salon_eval.py -k parking_capacity_followup` -> `1 passed, 16 deselected`
- `pytest -q truffles-api/tests/test_booking_chaos_dialogs.py` -> `1 passed`
- `pytest -q truffles-api/tests/test_booking_quality_response_guard.py` -> `57 passed`
- `pytest -q truffles-api/tests/test_message_endpoint.py` -> `282 passed, 2 warnings`
- `EVAL_TIER=long pytest -q truffles-api/tests/test_demo_salon_eval.py --maxfail=1 -vv` -> `17 passed`
