...
- CI (main@c3008da) green: https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/20675154955. Предыдущий прогон (20674885954) падал на `secret-scan` из-за `System.Text.EncoderFallbackException: Unable to translate Unicode character \\uD83E...` (job 59361181850); rerun устранил, build-push/deploy проходят.
- Прод: `/admin/version` → `{"version":"main","git_commit":"c3008da93eeac55472416e956ce3dede7846aaf9","build_time":"2026-01-03T09:04:11Z"}` (команда `docker exec -i truffles-api python - <<'PY' ...`).
