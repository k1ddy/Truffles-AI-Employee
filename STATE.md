...
- GAP: Provider Gateway media send live-check on canary (demo_salon) — webhook `/webhook/demo_salon` created handover conv_id `10049e90-5805-425f-841b-c0c9419c9c30`, topic_id `3722`; Telegram media webhook queued; outbox row `5d7fd86c-53dc-4523-8590-2ba211c0b665` status FAILED `invalid_payload:unsupported_event:whatsapp.send_media` (payload has signed_url + expires_at); outbox worker image lacks send_media support (`/app/app/routers/webhook/outbox.py` in `truffles-outbox`).
