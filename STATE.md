...
- CI (main@5b23db3) green: https://github.com/k1ddy/Truffles-AI-Employee/actions/runs/20675117581. Предыдущий прогон (20674885954) падал на `secret-scan` из-за `System.Text.EncoderFallbackException: Unable to translate Unicode character \\uD83E...` (job 59361181850); rerun устранил, build-push/deploy проходят.
- Прод: `/admin/version` → `{"version":"main","git_commit":"5b23db330bf64a8c7de6cd4404c58e01770d1941","build_time":"2026-01-03T09:01:15Z"}` (команда `docker exec -i truffles-api python - <<'PY' ...`).
- Контейнерные EVAL-тесты PASS: `docker exec -i truffles-api pytest /app/tests/test_demo_salon_eval.py -q` (265.62s); `docker exec -i truffles-api pytest /app/tests/test_demo_salon_eval.py -q -o EVAL_TIER=long` (262.02s).
