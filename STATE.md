...
**NOW (1 экран)**
<<<<<<< fix/2026-02-18-booking-handoff-merge-a88
...
- GAP: Runtime remediation remains blocked at Wave `0`: guard is still `critical` and classification proves majority of failed backlog is `unexpected_failure` (not only ChatFlow unpaid/billing), so feature rollout (visit/reminder/marketing) must stay blocked until `unexpected_failure` remediation loop reduces guard below critical — evidence: `docs/REPORTS/2026-02-18-wave0-integrity-outbox-baseline-a88.md`, `/tmp/platform_admin_kpi_wave0_t0.json`, `/tmp/owner_admin_kpi_wave0_t0.json`.
=======
...
- GAP: Owner/Admin UI acceptance remains blocked in this session because active auth state is `platform_admin` only; owner/admin-specific Playwright suite cannot be asserted without dedicated owner/admin credentials in this environment — evidence: `PLAYWRIGHT_WEB_SERVER=0 PLAYWRIGHT_BASE_URL=https://console.truffles.kz npx playwright test e2e/owner-admin-business.spec.ts --project=chromium --no-deps --reporter=list` (`nav-owner-admin-toggle` absent).
