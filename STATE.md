...
- DONE (local): Console login hotfix for Keycloak scope defaults: removed `offline_access` from default scope and kept it opt-in via `KEYCLOAK_SCOPE`; lint clean; deploy pending — evidence: `console-web/src/lib/auth.ts`, `docs/TASK_PACKAGES/TP-2026-02-16-console-login-scope-fix-a1.md`; checks: `npm --prefix console-web run lint -- --file src/lib/auth.ts`.
