...
**NOW (1 экран)**
- DONE (local+tests+docs, pending PR merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE2-A705` delivered as bounded behavior-preserving structural split for `UX-11/UX-12`: extracted control-tower pure helper layer from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_control_tower_utils.py` and extracted provisioning domain lexicon/field-guide/formatter layer from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-domain.ts`; added deterministic module tests `truffles-api/tests/test_console_control_tower_utils.py` (`5 passed`) and kept platform-admin lane contract green (`26 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave2-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave2-a705.md`, `truffles-api/app/services/console_control_tower_utils.py`, `truffles-api/tests/test_console_control_tower_utils.py`, `console-web/src/components/provisioning-wizard-domain.ts`, `console-web/src/components/ProvisioningWizard.tsx`, `truffles-api/app/routers/console.py`; checks: `python3 -m py_compile truffles-api/app/routers/console.py truffles-api/app/services/console_control_tower_utils.py truffles-api/tests/test_console_control_tower_utils.py` (`pass`), `pytest -q truffles-api/tests/test_console_control_tower_utils.py` (`5 passed`), `pytest -q truffles-api/tests/test_console_owner_business.py -k "control_tower_issue_counts or migration_wave_detail_filters_actions_and_counts or migration_program_aggregates_wave_gates"` (`3 passed, 56 deselected`), `cd console-web && npm run lint -- --file src/components/ProvisioningWizard.tsx --file src/components/provisioning-wizard-domain.ts --file e2e/platform-admin.spec.ts` (`No ESLint warnings or errors`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 E2E_USE_STORAGE_STATE=0 E2E_DETERMINISTIC_AUTH=1 npm run test:e2e -- --grep "Platform Admin Navigation|Platform Admin Tenants|Platform Admin Integrations|deep-link from Tenants action queue to Workspace execute"` (`26 passed`), `SESSION_AGENT=a705 scripts/session_check.sh` (`Session OK`).
...
- DONE (local+tests+docs, pending PR merge): `UVC-UX-DEDUP-INTENT-MAP-A705` step `1+2` delivered in existing tabs without adding new top-level routes: de-duplicated primary CTA placement in `Integrations` (single global context CTA moved into workspace-guidance block), normalized operator wording (`Ops` naming), and added unified intent-map hints (`факт -> действие -> проверка`) in `Tenants/Integrations/Workspace/Ops`. Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-dedup-intent-map-a705.md`, `console-web/src/app/tenants/tenants-page-view.tsx`, `console-web/src/app/tenants/use-tenants-action-queue.ts`, `console-web/src/app/integrations/page.tsx`, `console-web/src/app/company-workspace/page.tsx`, `console-web/src/components/OpsPage.tsx`, `console-web/e2e/platform-admin.spec.ts`; checks: `cd console-web && npm run lint -- --file src/app/tenants/tenants-page-view.tsx --file src/app/tenants/use-tenants-action-queue.ts --file src/app/integrations/page.tsx --file src/app/company-workspace/page.tsx --file src/components/OpsPage.tsx --file e2e/platform-admin.spec.ts` (`No ESLint warnings or errors`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 E2E_USE_STORAGE_STATE=0 E2E_DETERMINISTIC_AUTH=1 npm run test:e2e -- --grep "Platform Admin Navigation|Platform Admin Tenants|Platform Admin Integrations|deep-link from Tenants action queue to Workspace execute"` (`26 passed`).
