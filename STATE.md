...
- DONE (local+tests, pending PR merge): `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE8-A705` delivered as next bounded structural split for `UX-11/UX-12`: extracted go-live normalization/waiver/gate slice from `truffles-api/app/routers/console.py` into `truffles-api/app/services/console_onboarding_readiness.py` with extended deterministic tests `truffles-api/tests/test_console_onboarding_readiness.py`, extracted provisioning billing/working-hours/booking-settings JSON sync helpers from `console-web/src/components/ProvisioningWizard.tsx` into `console-web/src/components/provisioning-wizard-state.ts`, reduced monolith LOC (`console.py: 24606 -> 24554`, `ProvisioningWizard.tsx: 4544 -> 4527`), and revalidated targeted contracts (`py_compile`, `ruff`, retained deterministic suite `35 passed`, frontend lint/build, targeted platform-admin e2e `26 passed`). Evidence: `docs/TASK_PACKAGES/TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave8-a705.md`, `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave8-a705.md`, `truffles-api/app/services/console_onboarding_readiness.py`, `console-web/src/components/provisioning-wizard-state.ts`.
