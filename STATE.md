...
**NOW (1 экран)**
- NOW (Wave37 complete locally, ready for PR): `Wave37` is now green end-to-end in the active branch. Calendar booking creation follows one explicit operator path — `услуга -> мастер -> день -> время -> клиент -> подтверждение` — with visible blocked/loading/empty/error/ready slot states, plain-language next-step guidance, safe follow-up/contact-task copy, hidden technical owners, stronger phone/name guardrails, and a dedicated deterministic acceptance lane `console-web/e2e/calendar-operator.spec.ts` that proves valid, invalid, retry, case-prefill, follow-up, and medium-width flows. Visual inspection is clean at `1024px`, `1280px`, and `1440px`, including filters, booking composer, validation states, and the booking action panel. Evidence: `docs/TASK_PACKAGES/TP-2026-03-08-inbox-calendar-ux-reconstruction-wave37-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `/tmp/wave37-part-a-calendar-*.png`, `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file e2e/inspect_case.spec.ts --file e2e/calendar-operator.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/calendar-operator.spec.ts --project chromium` (`5 passed`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium` (`14 passed, 1 skipped`), `SESSION_AGENT=a1 scripts/session_check.sh` (`Session OK`).
