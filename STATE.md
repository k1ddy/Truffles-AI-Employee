...
- PLAN: DEC-018 Unified Reasoning Core (Signal Snapshot + pack-index + LLM pack-ref-only) — Task Package `docs/TASK_PACKAGES/TP-2026-01-30-unified-reasoning-core-dec.md` (doc-only; evidence pending).
...
- DONE: Golden eval trace/meta baseline + booking chaos-sim run (failures=1 action_mismatch) — evidence: `pytest -q truffles-api/tests/test_demo_salon_eval.py -k "golden_eval"` (`/tmp/pytest_golden_eval_20260131.txt`), chaos-sim `/tmp/chaos_golden_eval_booking_20260131` (summary/report/failures).
...
- **OPEN:** chaos-sim residuals in booking-only runs — out_of_domain actions on booking turns, occasional ai_response vs expected_reply_type, and rare poll timeouts. Evidence: `/tmp/chaos_booking_simtime_mgrskip_5b` (summary/report/cases/failures), `/tmp/chaos_golden_eval_booking_20260131` (summary: failures=1 action_mismatch).
