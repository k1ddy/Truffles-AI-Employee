...
- **PLAN (no evidence):** TP-2026-01-25-sim-time-override — use `simulation_time` as RELATIVE_BASE for booking signal datetime parsing (chaos-sim determinism).
- **GAP:** chaos-sim run for sim-time override timed out twice (`TimeoutError` on webhook). Artifacts: `/tmp/chaos_booking_simtime_override_3/cases.jsonl`, `/tmp/chaos_booking_simtime_override_1/cases.jsonl`. Needs rerun when webhook is stable.
