...
**NOW (1 экран)**
- NOW (local worktree, knowledge activation post-deploy automation P9): post-deploy proof is no longer runbook-only. `scripts/knowledge_activation_postdeploy.sh` now wraps the existing `release_guard` + optional tenant closeout into one machine-readable manifest/summary, and `.github/workflows/ci.yml` now uses it from the deploy job after parity checks: deploy restarts `truffles-knowledge-activation-service`, runs the wrapper on the VPS from the fresh deploy clone, downloads `/tmp/knowledge-activation-postdeploy-<run>.tar.gz` back to the runner, uploads `knowledge-activation-proof`, and writes a short workflow summary. Tenant closeout is explicit-config only (`workflow_dispatch` inputs or repo vars) and otherwise truthfully records `closeout.status=skipped` rather than faking `go`; this matches current prod reality, where no consultant-verification closeout target is configured yet. Evidence: `scripts/knowledge_activation_postdeploy.sh`, `.github/workflows/ci.yml`, `truffles-api/tests/test_knowledge_activation_postdeploy_script.py`, `docs/runbooks/KNOWLEDGE_ACTIVATION_RELEASE.md`, `TECH.md`, `SPECS/SYSTEM_REFERENCE.md`, `STRUCTURE.md`, `docs/TASK_PACKAGES/TP-2026-03-16-knowledge-activation-postdeploy-automation-p9-a30.md`; checks: `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && pytest -q truffles-api/tests/test_restart_release_scripts.py truffles-api/tests/test_knowledge_activation_postdeploy_script.py` (`7 passed`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && ruff check truffles-api/tests/test_restart_release_scripts.py truffles-api/tests/test_knowledge_activation_postdeploy_script.py` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && bash -n scripts/restart_release.sh scripts/knowledge_activation_postdeploy.sh` (`pass`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && python3 - <<'PY'\nimport pathlib, re, subprocess, tempfile, yaml\nworkflow = yaml.safe_load(pathlib.Path('.github/workflows/ci.yml').read_text())\nfor step_name in ('Deploy to VPS', 'Summarize knowledge activation proof'):\n    step = next(item for item in workflow['jobs']['deploy']['steps'] if item.get('name') == step_name)\n    script = re.sub(r\"\\$\\{\\{[^}]+\\}\\}\", 'DUMMY', step['run'])\n    with tempfile.NamedTemporaryFile('w', suffix='.sh', delete=False) as tmp:\n        tmp.write(script)\n        path = tmp.name\n    subprocess.check_call(['bash', '-n', path])\n    print(f'BASH_PARSE_OK {step_name}')\nprint('YAML_PARSE_OK')\nPY` (`BASH_PARSE_OK Deploy to VPS`, `BASH_PARSE_OK Summarize knowledge activation proof`, `YAML_PARSE_OK`), `cd /home/zhan/worktrees/2026-03-15-knowledge-release-model-stoploss-a30 && SESSION_AGENT=a30 bash scripts/session_check.sh` (`pass`).
