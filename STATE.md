...
- Night tone/timezone: Evidence — `truffles-api/app/knowledge/demo_salon/SALON_TRUTH.yaml` (hours + communication.tone) и `truffles-api/app/routers/webhook.py` (UTC now). Решение — добавить `salon.timezone` в client_pack, учитывать локальное время и тихие часы; статус: нужен logic-fix.
