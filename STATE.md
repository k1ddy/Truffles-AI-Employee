...
**NOW (1 экран)**
- NOW (Wave38 Part A/B complete locally; PR #960 updated): Calendar now has both missing operator contracts that were still causing immediate friction after merged `Wave37`: filters use an explicit `draft -> applied` boundary, and booking phone entry keeps a raw editable value instead of destructive on-change reformatting. Search/status/follow-up edits stay in panel draft until `Применить`, `Сбросить изменения` restores the current applied snapshot, prefixed partial phone numbers (`+7 ...` / `8 ...`) stay invalid until complete, and the composer now shows either a neutral hint or a normalized preview without fighting typing, deletion, or paste. Frontend/backend/mock normalization is aligned so incomplete prefixed numbers cannot silently turn into wrong `+7...` payloads. Remaining scope is now `Wave38 Part C/D`: full booking edit/reschedule/cancel lifecycle and the final operator acceptance matrix. Evidence: `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/routers/calendar.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `/tmp/wave38-part-a-filters-draft-1280.png`, `/tmp/wave38-part-a-filters-applied-1280.png`, `/tmp/wave38-part-a-filters-draft-1024.png`, `/tmp/wave38-part-b-phone-full-1280.png`, `/tmp/wave38-part-b-phone-partial-1280.png`, `/tmp/wave38-part-b-phone-empty-1024.png`, `cd console-web && npm run lint -- --file src/app/calendar/page.tsx --file e2e/calendar-operator.spec.ts --file e2e/inspect_case.spec.ts` (`pass`), `cd console-web && npm run build` (`pass`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/calendar-operator.spec.ts --project chromium` (`7 passed`), `cd console-web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3102 PLAYWRIGHT_WEB_SERVER=0 npx playwright test e2e/inspect_case.spec.ts --project chromium` (`14 passed, 1 skipped`), `cd truffles-api && pytest -q tests/test_calendar_bookings_router.py` (`10 passed`), `cd truffles-api && ruff check app/routers/calendar.py tests/test_calendar_bookings_router.py` (`pass`).
