...
- GAP (runtime/live-gate): Fail-closed Tenants a11y lane against deployed `https://console.truffles.kz` remains red (`desktop serious=1`, `mobile serious=1`, `color-contrast`), so Wave5 runtime a11y DoD can be closed only after this branch is deployed and lane is re-run on fresh live build — evidence: playwright run `A11Y_FAIL_ON_THRESHOLDS=1 ... e2e/tenants-a11y.spec.ts` and `docs/REPORTS/2026-02-20-tenants-v3-redesign-plan.md`.
...
- PLAN: Next wave for no-case outreach follow-up formalized in dedicated Task Package `docs/TASK_PACKAGES/TP-2026-02-22-outreach-auto-case-a200.md` (auto-case bootstrap + dedupe + inbox visibility + trace/audit contract); implementation/evidence pending in separate delivery PR.
- GAP: Even after retention hardening, `decision_trace` persistence for the locked inbound does not currently show `routing:human_lock_silent` in DB snapshots for the tested conversation (`10049e90-5805-425f-841b-c0c9419c9c30`) despite webhook response confirming lock path (`message=\"Human lock active, message forwarded\", bot_response=null`); this remains an observability gap for mandatory live-check evidence and requires root-cause follow-up in trace write path — evidence: `/tmp/outreach_hlock_livecheck_18150_fix2_20260220_152809/04_locked_inbound_webhook_response.json`, `/tmp/outreach_hlock_livecheck_18150_fix2_20260220_152809/15_human_lock_trace_rows.tsv`, `/tmp/outreach_hlock_livecheck_18150_fix2_20260220_152809/12_user_messages_decision_meta.tsv`.
