...
- DONE: Knowledge Gateway service (shadow container) — Task Package `docs/TASK_PACKAGES/TP-2026-01-27-knowledge-gateway-service.md`; local image `truffles-knowledge-gateway:20260127` via `scripts/restart_knowledge_gateway.sh` (port 8010); health `curl -s http://127.0.0.1:8010/health` → `{"status":"ok","service":"knowledge_gateway","snapshot_enabled":true}`; snapshot response `/tmp/knowledge_gateway_snapshot_20260127_140612.json` (tenant_context demo_salon, version_id `fae96e60-eaa3-449e-a1b2-068ff76dd9a1`).
