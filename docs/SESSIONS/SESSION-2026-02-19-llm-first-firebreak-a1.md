...
- note: `P1.6o199` is truthfully closed as a bounded Track A lexical-variant scenario-governance re-closure, and `P1.6o200` is the next admissible proof lane.
- done:
  - Completed `P1.6o199`; bounded scenario-governance closure now recognizes `у вас есть время ...` occupancy phrasing inside the existing partial-date availability normalization path, so surfaced turns like `У вас есть время на завтра?` canonicalize to `slot_constraint(time)` in both sanitize and post-coverage repair flows while explicit-alternative compare turns remain `slot_compare`; targeted generator checks are green, and `P1.6o200` is published as the next admissible proof lane.
  - Completed `P1.6o198`; fresh guarded `dev L2` `p1.6o198-l2-dev-20260314-a1-r1` strict-audited with `artifact_integrity.valid=true`, `infra_valid=true`, `semantic_valid=false`, moved reopened `M19` off first fail, and truthfully surfaced a Track A generator lexical-variant drift at `dialog 1 / turn 3 / "У вас есть время на завтра?"`, where scenario still required stale `slot_compare(time)` while runtime canonically emitted `slot_constraint(time)` under timeout owner-boundary collect.
  - Completed `P1.6o197`; bounded runtime re-closure now lets unique first-name branch-catalog matches recover surfaced invalid-schema named-specialist booking requests even when the real branch stores full names like `Айгерим Болатова`; targeted endpoint/helper regressions and response-guard checks are green, and `P1.6o198` is published as the next admissible proof lane.
  - Completed `P1.6o196`; fresh guarded `dev L2` `p1.6o196-l2-dev-20260314-a1-r1` moved the scenario-generation blocker off the path, strict-audited `artifact_integrity.valid=true` / `infra_valid=true`, and truthfully reopened existing `M19` on `А можно записаться к Айгерим?`, where branch context existed but branch-catalog matching still missed real full-name data.
  - Completed `P1.6o195`; bounded proof-tooling closure now reuses the scenario generator's inner retry path by default for generated LLM proof runs, targeted tooling checks are green, and `P1.6o196` was published as the next admissible proof lane.
  - Completed `P1.6o194`; fresh guarded `dev L2` `p1.6o194-l2-dev-20260314-a1-r1` stayed infra-invalid before scenario emission, but timeout normalization now truthfully surfaces `scenario_generation_timeout (batch=2 attempt=1 event=batch_attempt_error error="The read operation timed out")`, so `P1.6o195` became the next admissible deterministic blocker.
  - Completed `P1.6o193`; bounded proof-tooling observability now preserves the last `batch_attempt_error.error` detail inside normalized scenario-generation timeout reasons; targeted tooling checks are green, and `P1.6o194` was published as the next admissible proof lane.
