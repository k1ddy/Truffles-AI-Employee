# TP-2026-03-22 — Consultant Core Demo Salon Main Canary Turn 8 Booking Interrupt Exact-Time Progression Canary Replay A922

## Block identity
- `BLOCK_ID`: `CONSULTANT-CORE-DEMO-SALON-MAIN-CANARY-TURN8-BOOKING-INTERRUPT-EXACT-TIME-PROGRESSION-CANARY-REPLAY-A922`
- `PARENT_BLOCK_ID`: `CONSULTANT-CORE-DEMO-SALON-MAIN-CANARY-TURN8-BOOKING-INTERRUPT-EXACT-TIME-PROGRESSION-RUNTIME-IMPLEMENTATION-A922`
- `DEPENDS_ON`: `docs/TASK_PACKAGES/TP-2026-03-22-consultant-core-demo-salon-main-canary-turn8-booking-interrupt-exact-time-progression-runtime-implementation-a922.md`
- `UNLOCKS`: `CONSULTANT-CORE-DEMO-SALON-MAIN-CANARY-TURN8-BOOKING-INTERRUPT-EXACT-TIME-PROGRESSION-POST-REPLAY-CLASSIFICATION-A922`
...
import json
from pathlib import Path
path = Path('/tmp/booking_quality/a922-check-booking-proof-r17/run_manifest.json')
data = json.loads(path.read_text(encoding='utf-8'))
for key in ['run_id', 'status', 'mode', 'command', 'output_dir']:
    print(f'{key}: {data.get(key)}')
PY`
- `FACT findings`:
  - `/tmp/booking_quality/a922-check-booking-proof-r17` is the current truthful pre-fix replay artifact for this family: `infra_valid=true`, `semantic_valid=false`, `turns_strict_failed=0`, and the first surviving runtime family is turn `8` while judge/hq1 conflicts on turns `6`, `9`, and `11` remain advisory proof debt only.
  ...
  - `r17` already consumed the same locked scenario file `/tmp/booking_quality/a922-weekend-slot-constraint-dialog-sanitized-r10.json`; comparison stays truthful only if the new run reuses that exact file.
  - the guarded wrapper still requires one explicit guarded dev-lane continuation reason because the replay lineage already contains audited non-canonical artifacts in this family, so this block must document the continuation instead of silently bypassing the ledger/index contract.
- `Detected drift (docs vs runtime)`:
  - repo truth already says replay is the next move, but there is still no fresh post-fix artifact proving the landed turn-8 repair on the real canary path.
...
- **Reuse rule for this block:** reuse the exact search already recorded in the turn-8 implementation family; this replay block does not open a new query.
...
- **Decision:** `reuse/integrate`
  - reuse the same locked scenario file and comparable replay shape from `r17`
...
- **Symptom:** the repo still lacks fresh post-fix canary evidence for the turn-8 family; only deterministic fix evidence and the pre-fix truthful artifact `a922-check-booking-proof-r17` exist.
- **Minimal reproduction:**
  1. inspect `docs/REPORTS/artifacts/2026-03-22-consultant-core-demo-salon-main-canary-turn8-booking-interrupt-exact-time-progression-runtime-implementation-a922.md` and confirm the fix is landed with deterministic evidence only
  2. inspect `/tmp/booking_quality/a922-check-booking-proof-r17/responses.jsonl` and confirm turn `8` still fails on the pre-fix replay artifact
  3. inspect `/tmp/booking_quality/a922-check-booking-proof-r17/run_manifest.json` and confirm the exact comparable replay command and scenario file already exist
...
- **Evidence:**
  ...
  - `/tmp/booking_quality/a922-check-booking-proof-r17/responses.jsonl`
  - `/tmp/booking_quality/a922-check-booking-proof-r17/manual_audit.json`
  - `/tmp/booking_quality/a922-check-booking-proof-r17/run_manifest.json`
  - `ss -ltnp | rg ':18186'`
- **Five Whys:**
  1. Why is turn `8` not truthfully closed yet? Because there is no fresh replay artifact after the landed runtime fix.
  ...
  5. Why is a guarded continuation reason required? Because the replay lineage for this family already contains audited non-canonical artifacts, so a new comparable replay must acknowledge and document that indexed state rather than silently bypass it.
...
- **Internal reuse:**
  - `r17` replay command shape from `/tmp/booking_quality/a922-check-booking-proof-r17/run_manifest.json`
  - locked scenario file `/tmp/booking_quality/a922-weekend-slot-constraint-dialog-sanitized-r10.json`
  - focused deterministic booking-prompt/check-booking owner suite already published in the implementation block
...
- `Family handled in this block`: `turn-8 booking-interrupt exact-time progression post-fix closure replay`
...
- `Why this profile fits`:
  - the block is replay/closure by intent, but the worktree already carries the approved turn-8 runtime diff; keeping `implementation` mode avoids false governance failures on the existing code delta while canon is synced to fresh replay truth.
...
- execute one guarded comparable replay as `/tmp/booking_quality/a922-check-booking-proof-r18`
...
4. Run one guarded comparable replay as `a922-check-booking-proof-r18` on the locked scenario file with an explicit continuation reason for the prior audited invalid replay state.
...
## DoD
- fresh run `/tmp/booking_quality/a922-check-booking-proof-r18` exists with `summary.json`, `responses.jsonl`, `brief.md`, and strict-audit artifacts
- the replay uses the exact locked scenario file from `r17`
- the report truthfully classifies whether turn `8` is repaired on the real canary path and whether any downstream blocker survives independently
- mandatory packet / guard / architecture / session checks pass after canon sync
- next non-negotiable move is updated from fresh evidence rather than stale `r17` assumptions
...
- `/tmp/booking_quality/a922-check-booking-proof-r17/summary.json`
- `/tmp/booking_quality/a922-check-booking-proof-r17/responses.jsonl`
- `/tmp/booking_quality/a922-check-booking-proof-r18/brief.md`
- `/tmp/booking_quality/a922-check-booking-proof-r17/manual_audit.json`
- `/tmp/booking_quality/a922-check-booking-proof-r18/manual_audit.md`
...
- the repaired turn-8 family sits close to the already landed exact-time progression logic, so replay classification must verify both closure and any downstream surfacing without mixing oracle debt into runtime prematurely
...
- `Why not in this block:`
  - this block is the truthful replay/evidence bridge after the landed turn-8 runtime fix; it classifies from fresh evidence but does not reopen runtime or oracle code itself
- `Risk if deferred:`
  - without a fresh replay classification, the team would either reopen already-landed turn-8 code or patch oracle debt before classifying whether any runtime family survives
- `Linked follow-up Task Package(s):`
  - `classify_consultant_core_demo_salon_turn8_replay_and_promote_next_family_if_any`
- `Expiry/trigger to stop deferral:`
  - stop deferral before any new oracle/judge tuning or additional replay

## Next-block contract (mandatory)
- `Next block objective:`
  - classify the fresh turn-8 replay artifact and promote the next honest runtime or proof block from fresh evidence only
...
for idx in [8, 9, 11, 13]:
    row = next(r for r in rows if r.get('turn_index') == idx)
    print(idx, row['turn_text'], row['outbox_text'], row.get('expected_reply_type'), row.get('booking_slots'), row.get('booking_progressed'), row.get('evaluation'))
PY`
- `Blocked-by conditions:`
  - replay preflight is invalid or the local runtime is stale
