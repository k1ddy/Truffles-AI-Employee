# TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave11-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE11-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW6-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW7-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review6, чтобы дальше снизить blast-radius `UX-11/UX-12` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Date/time (local):** `2026-03-04 17:00 +0500`
- **Sources opened (from this query):**
  - `https://dora.dev/guides/dora-metrics/`
- **Found reusable solution:** keep decomposition in small bounded waves with stable deterministic regression lane.
- **Decision:** continue `reuse -> integrate -> configure -> build` with fail-closed decision blocks after each wave.
- **Rejected options:** one-shot large rewrite of router/wizard parent files.

## Root cause (mandatory)
- **Symptom:** `UX-11/UX-12` remain open after wave10.
- **Minimal reproduction:** merged-main baseline after wave10 still shows large parent files (`console.py=24366`, `ProvisioningWizard.tsx=4423`).
- **Evidence:** closure-review6 artifact and merged-main check outputs.
- **Five Whys:**
  ...
  5. why wave11 now: closure-review6 kept residual open by contract.
...
- **Fix mechanism:** extract one backend orchestration/normalization slice and one frontend action/state slice with parity-preserving contracts.

## Reuse-first plan (mandatory)
- **Internal reuse:** existing extracted modules from wave1-10 and deterministic lanes.
...
## Scope
- Backend: extract next bounded branch/provisioning orchestration slice from `console.py` into dedicated service module.
- Frontend: extract next bounded mutation/state orchestration slice from `ProvisioningWizard.tsx` into dedicated helper/hook module.
- Publish wave11 artifact and sync canon/session docs.
...
- canon/session docs touched by wave11 evidence

## Plan (1..N)
1. Capture closure-review6 merged-main baseline and pick minimal backend/frontend slices for wave11.
...
4. Publish wave11 artifact, sync canon/session docs, open PR.

## DoD
- Wave11 extraction committed with deterministic checks green.
...
- `cd console-web && npm run lint -- --file src/components/ProvisioningWizard.tsx --file src/components/provisioning-wizard-account-actions.ts --file src/components/provisioning-wizard-branch-actions.ts --file e2e/platform-admin.spec.ts`
- `cd console-web && npm run lint -- --file src/components/ProvisioningWizard.tsx --file src/components/provisioning-wizard-account-actions.ts --file src/components/provisioning-wizard-branch-actions.ts --file src/components/provisioning-wizard-autopilot.ts --file e2e/platform-admin.spec.ts`
...
## Evidence
- Wave11 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave11 merge commit.
- **Post-release monitoring window:** run closure-review7 on merged-main evidence before any wave12 work.
...
- `Current residuals accepted in this block`: parent files may remain above target maintainability threshold after wave11.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review7-a705.md`.
- `Expiry/trigger to stop deferral`: if closure-review7 still reports `Open`, next wave contract must be created in same block.

## Next-block contract (mandatory)
- `Next block objective`: closure-review7 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave11 PR not merged or required checks red.
