# TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave14-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE14-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW9-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW10-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review9, чтобы продолжить снижение оркестрационной концентрации в `console.py` и `ProvisioningWizard.tsx` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Date/time (local):** `2026-03-05 06:44 +0500`
...
- **Symptom:** `UX-11/UX-12` remained open after closure-review9.
- **Minimal reproduction:** merged-main baseline after wave13 is still `console.py=24354`, `ProvisioningWizard.tsx=4355`.
- **Evidence:** closure-review9 artifact and deterministic merged-main checks.
- **Five Whys:**
  ...
  5. why wave14 now: closure-review9 explicitly kept backlog open by contract.
- **Root cause statement:** orchestration concentration in parent router/wizard files remains above maintainability threshold despite wave13 extraction.
...
- Publish wave14 artifact and sync canon/session docs.
...
- canon/session docs touched by wave14 evidence

## Plan (1..N)
1. Capture closure-review9 merged-main baseline and define minimal backend/frontend extraction slices.
...
4. Publish wave14 artifact, sync canon/session docs, open PR.

## DoD
- Wave14 extraction committed with deterministic checks green.
...
## Evidence
- Wave14 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave14 merge commit.
- **Post-release monitoring window:** run closure-review10 on merged-main evidence before any wave15 work.
...
- `Current residuals accepted in this block`: parent files can remain above closure threshold after wave14.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review10-a705.md`.
- `Expiry/trigger to stop deferral`: if closure-review10 remains `Open`, next wave contract must be created in the same block.

## Next-block contract (mandatory)
- `Next block objective`: closure-review10 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave14 PR not merged or required checks red.
