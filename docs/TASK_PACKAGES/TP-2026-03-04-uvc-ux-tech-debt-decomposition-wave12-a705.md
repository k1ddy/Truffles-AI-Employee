# TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave12-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE12-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW7-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW8-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review7, чтобы дальше снизить blast-radius `UX-11/UX-12` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Date/time (local):** `2026-03-04 19:05 +0500`
...
- **Symptom:** `UX-11/UX-12` remain open after wave11.
- **Minimal reproduction:** merged-main baseline after wave11 still shows large parent files (`console.py=24365`, `ProvisioningWizard.tsx=4365`).
- **Evidence:** closure-review7 artifact and merged-main check outputs.
- **Five Whys:**
  ...
  5. why wave12 now: closure-review7 kept residual open by contract.
...
- **Internal reuse:** existing extracted modules from wave1-11 and deterministic lanes.
...
- Publish wave12 artifact and sync canon/session docs.
...
- canon/session docs touched by wave12 evidence

## Plan (1..N)
1. Capture closure-review7 merged-main baseline and pick minimal backend/frontend slices for wave12.
...
4. Publish wave12 artifact, sync canon/session docs, open PR.

## DoD
- Wave12 extraction committed with deterministic checks green.
...
## Evidence
- Wave12 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave12 merge commit.
- **Post-release monitoring window:** run closure-review8 on merged-main evidence before any wave13 work.
...
- `Current residuals accepted in this block`: parent files may remain above target maintainability threshold after wave12.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review8-a705.md` (to be created after wave12).
- `Expiry/trigger to stop deferral`: if closure-review8 still reports `Open`, next wave contract must be created in same block.

## Next-block contract (mandatory)
- `Next block objective`: closure-review8 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave12 PR not merged or required checks red.
