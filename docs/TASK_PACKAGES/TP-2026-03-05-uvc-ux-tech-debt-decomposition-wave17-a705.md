# TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave17-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE17-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW12-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW13-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review12, чтобы продолжить снижение оркестрационной концентрации в `console.py` и `ProvisioningWizard.tsx` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Query (exact):** `Refactoring catalog Move Function Martin Fowler`
- **Date/time (local):** `2026-03-05 08:50 +0500`
- **Sources opened (from this query):**
  - `https://refactoring.com/catalog/moveFunction.html`
- **Found reusable solution:** continue incremental ownership transfer by moving cohesive behavior to stable helper/module boundaries with parity checks.
...
- **Symptom:** `UX-11/UX-12` remain open after closure-review12.
- **Minimal reproduction:** merged-main baseline after wave16 is still `console.py=24380`, `ProvisioningWizard.tsx=4338`.
- **Evidence:** closure-review12 artifact and deterministic merged-main checks.
- **Five Whys:**
  ...
  5. why wave17 now: closure-review12 explicitly kept backlog open by contract.
- **Root cause statement:** orchestration concentration in parent router/wizard files remains above maintainability threshold despite wave16 extraction.
...
- **External reuse:** extraction pattern from Refactoring catalog (`Move Function`) to keep cohesive responsibilities isolated.
...
- Publish wave17 artifact and sync canon/session docs.
...
- canon/session docs touched by wave17 evidence

## Plan (1..N)
1. Capture closure-review12 merged-main baseline and define minimal backend/frontend extraction slices.
...
4. Publish wave17 artifact, sync canon/session docs, open PR.

## DoD
- Wave17 extraction committed with deterministic checks green.
...
## Evidence
- Wave17 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave17 merge commit.
- **Post-release monitoring window:** run closure-review13 on merged-main evidence before any wave18 work.
...
- `Current residuals accepted in this block`: parent files can remain above closure threshold after wave17.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review13-a705.md`.
- `Expiry/trigger to stop deferral`: if wave17 closes neither `UX-11` nor `UX-12`, closure-review13 must open next bounded wave immediately.

## Next-block contract (mandatory)
- `Next block objective`: closure-review13 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave17 PR not merged or required checks red.
