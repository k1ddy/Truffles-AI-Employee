...
| Wave20 | `TP-2026-03-07-inbox-calendar-ux-reconstruction-wave20-a1.md` | Split allowed and expected: `Part A operator mode contract`, `Part B history/archive rail` | Пересобрать IA панели `Заявки`: first-screen `Открытые / Закрытые / Все`, queue views only for open-mode, явный filter drawer и history/archive surface. | In progress |
| Wave21 | `TP-2026-03-07-inbox-calendar-ux-reconstruction-wave21-a1.md` | Split allowed and expected: `Part A semantic context`, `Part B booking-state propagation` | Довести бесшовную semantic chain между ботом, заявкой, действиями менеджера и календарём `Записи`, чтобы case/business state и booking state объясняли друг друга. | Planned |
| Wave22 | `TP-2026-03-07-inbox-calendar-ux-reconstruction-wave22-a1.md` | Validation/closeout block; split only if deterministic and live evidence must land separately | Закрыть программу forbidden-state matrix, deterministic acceptance и live no-mocks validation без fake-pass. | Planned |
