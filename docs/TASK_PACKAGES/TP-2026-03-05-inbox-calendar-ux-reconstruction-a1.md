...
| Wave21 | `TP-2026-03-07-inbox-calendar-ux-reconstruction-wave21-a1.md` | Split allowed and expected: `Part A semantic context`, `Part B booking-state propagation` | Довести бесшовную semantic chain между ботом, заявкой, действиями менеджера и календарём `Записи`, чтобы case/business state и booking state объясняли друг друга. | In progress (`Part A` merged via `PR #942`; `Part B` local and PR-ready) |
