...
| Wave38 | `TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md` | Split mandatory: `Part A filter-state contract`, `Part B phone/composer hardening`, `Part C booking lifecycle completion`, `Part D operator proof + visual acceptance` | Довести `Записи` до полного post-merge operator-grade состояния: deterministic filters, natural phone input, and safe edit/reschedule/cancel lifecycle with full valid/invalid proof. | Part A complete locally; `Part B`/`Part C`/`Part D` pending |
...
- `Next block objective`: `Wave38 Part A` is now green locally; execute `Part B` next (phone/composer hardening), then `Part C` and `Part D`, and only after that decide whether a bounded Calendar API follow-up is needed or whether work can return to `UX-08` / `UX-20` / `UX-26` without reopening routing v2.
