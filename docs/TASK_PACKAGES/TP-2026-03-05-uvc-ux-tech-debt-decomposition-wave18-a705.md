# TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave18-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE18-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW13-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW14-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review13, чтобы продолжить снижение оркестрационной концентрации в `console.py` и `ProvisioningWizard.tsx` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Date/time (local):** `2026-03-05 11:20 +0500`
...
- **Symptom:** `UX-11/UX-12` remain open after closure-review13.
- **Minimal reproduction:** merged-main baseline after wave17 is still `console.py=24381`, `ProvisioningWizard.tsx=4325`.
- **Evidence:** closure-review13 artifact and deterministic merged-main checks.
- **Five Whys:**
  ...
  5. why wave18 now: closure-review13 explicitly kept backlog open by contract.
- **Root cause statement:** orchestration concentration in parent router/wizard files remains above maintainability threshold despite wave17 extraction.
...
- Publish wave18 artifact and sync canon/session docs.
...
- canon/session docs touched by wave18 evidence

## Plan (1..N)
1. Capture closure-review13 merged-main baseline and define minimal backend/frontend extraction slices.
...
4. Publish wave18 artifact, sync canon/session docs, open PR.

## DoD
- Wave18 extraction committed with deterministic checks green.
...
## Evidence
- Wave18 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave18 merge commit.
- **Post-release monitoring window:** run closure-review14 on merged-main evidence before any wave19 work.
...
- `Current residuals accepted in this block`: parent files can remain above closure threshold after wave18.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review14-a705.md`.
- `Expiry/trigger to stop deferral`: if wave18 closes neither `UX-11` nor `UX-12`, closure-review14 must open next bounded wave immediately.

## Next-block contract (mandatory)
- `Next block objective`: closure-review14 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave18 PR not merged or required checks red.
