...
| Wave 3 Data contract | `done/partial` | Typed weekly snapshot schema + table/fallback (`truffles-api/app/schemas/console.py:249`, `truffles-api/app/routers/console.py:13434`); в `/tenants` добавлены decomposition hooks `useTenantsDataQueries` и `useTenantsActions` (query/context/intent/quick-create/company-client-save orchestration) | Модель аналитики и fleet-агрегации не рассчитана на очень большой объём (`F5`); часть action pipelines (lifecycle/branch-change + editor bootstrap) ещё остаётся в `page.tsx` |
...
Evidence:
- `console-web/src/app/tenants/page.tsx` = ~2214 LOC (после выделения `useTenantsDataQueries` + `useTenantsActions`), но всё ещё частичный оркестрационный монолит.
- Query/context/intent/quick-create/company-client-save orchestration уже вынесены в hooks (`console-web/src/app/tenants/use-tenants-data-queries.ts`, `console-web/src/app/tenants/use-tenants-actions.ts`), но lifecycle/branch-change pipelines и editor bootstrap остаются в странице.
Impact:
