...
| Wave 3 Data contract | `done/partial` | Typed weekly snapshot schema + table/fallback (`truffles-api/app/schemas/console.py:249`, `truffles-api/app/routers/console.py:13434`); в `/tenants` добавлены decomposition hooks `useTenantsDataQueries` и `useTenantsActions` (query/context/intent/quick-create orchestration) | Модель аналитики и fleet-агрегации не рассчитана на очень большой объём (`F5`); часть action pipelines (CRUD/lifecycle/branch-change) ещё остаётся в `page.tsx` |
...
Evidence:
- `console-web/src/app/tenants/page.tsx` = ~2301 LOC (после выделения `useTenantsDataQueries` + `useTenantsActions`), но всё ещё частичный оркестрационный монолит.
- Query/context/intent/quick-create orchestration уже вынесены в hooks (`console-web/src/app/tenants/use-tenants-data-queries.ts`, `console-web/src/app/tenants/use-tenants-actions.ts`), но крупные action pipelines CRUD/lifecycle/branch-change остаются в странице.
Impact:
