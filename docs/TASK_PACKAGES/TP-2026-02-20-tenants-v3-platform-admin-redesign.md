...
| Wave 3 Data contract | `done/partial` | Typed weekly snapshot schema + table/fallback (`truffles-api/app/schemas/console.py:249`, `truffles-api/app/routers/console.py:13434`); в `/tenants` добавлены decomposition hooks `useTenantsDataQueries`, `useTenantsActions`, `useTenantsScopeDerivedState`, `useTenantsActionQueue`, helper module `tenants-page-helpers.ts` (lifecycle audit / branch patch / formatters), `tenants/page.tsx` reduced `1883 -> 1268` | Модель аналитики и fleet-агрегации не рассчитана на очень большой объём (`F5`); часть compose wiring и UI orchestration ещё в `page.tsx` |
| Wave 4 Decomposition/perf | `done/partial` | Вынесены `OperationalKpi`, `FleetAttention`, `PortfolioCompanies`, `Clients`, `ChangeManagement`, `Decommission`, `ClientLifecycleModal` секции; page-filter race для Scenario C устранён в `use-tenants-page-filters.ts`; backend perf-track запущен через `console_tenants_endpoint_latency{endpoint=portfolio|company_cockpit}` + router instrumentation; внедрён read-model cache (`truffles-api/migrations/038_add_tenants_fleet_cache.sql`, `truffles-api/app/models/tenants_fleet_cache.py`, `truffles-api/app/routers/console.py`) + cache tests (`truffles-api/tests/test_console_tenants_list.py`); добавлен async stale-while-refresh для cache-hit near-expiry (`Thread` background refresh + inflight dedupe) для summary/attention; write-path cache invalidation переведён на scope-aware режим по `scope_company_id` (`migration 039`); post-commit prewarm покрывает affected company scopes + global default portfolio scope (`summary + attention`) и дополнен event metadata queue (`_TENANTS_FLEET_CACHE_PREWARM_EVENTS_INFO_KEY`) с `after_commit` consumer и rollback cleanup; добавлена dispatch queue коалесинга prewarm scheduling (`_TENANTS_FLEET_PREWARM_DISPATCH_QUEUE_MAX`, `_TENANTS_FLEET_PREWARM_DISPATCH_BATCH_MAX`) для более устойчивого burst handling; `/admin/branches` hot path переведён на `Client` join scope filtering + perf indexes (`migration 040`) с расширенными contract tests; perf snapshot gate дополнен minimum sample-size checks и runtime snapshot после controlled load | Полный durable incremental precompute pipeline (event queue -> persistent worker strategy + long-run reproducible perf evidence) остаётся в бэклоге; текущий runtime SLO snapshot pass |
...
- для очень большого флота всё ещё нужен durable incremental precompute worker strategy и проверка p95/p99 под длительной нагрузкой.
