...
Evidence:
- `console-web/src/app/tenants/page.tsx` = ~1883 LOC (после выделения `useTenantsDataQueries` + `useTenantsActions`), но всё ещё частичный оркестрационный монолит.
- Query/context/intent/quick-create/company-client-save/editor bootstrap/lifecycle/branch-change orchestration вынесены в hooks (`console-web/src/app/tenants/use-tenants-data-queries.ts`, `console-web/src/app/tenants/use-tenants-actions.ts`); в странице остаются helper/derived-state блоки и compose wiring.
Impact:
