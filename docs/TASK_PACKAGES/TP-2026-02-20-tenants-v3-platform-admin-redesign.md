...
Evidence:
- `console-web/src/app/tenants/page.tsx` = ~1376 LOC (после выделения `useTenantsDataQueries` + `useTenantsActions` + `useTenantsScopeDerivedState` + `tenants-page-helpers.ts`), но всё ещё частичный оркестрационный монолит.
- Query/context/intent/quick-create/company-client-save/editor bootstrap/lifecycle/branch-change + scope-derived names/maps/options вынесены в hooks (`console-web/src/app/tenants/use-tenants-data-queries.ts`, `console-web/src/app/tenants/use-tenants-actions.ts`, `console-web/src/app/tenants/use-tenants-scope-derived-state.ts`), helper/formatting и audit pipelines вынесены в `console-web/src/app/tenants/tenants-page-helpers.ts`; в странице остаются compose wiring и UI orchestration.
Impact:
