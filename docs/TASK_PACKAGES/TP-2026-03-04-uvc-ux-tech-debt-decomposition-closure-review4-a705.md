# TP-2026-03-04-uvc-ux-tech-debt-decomposition-closure-review4-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW4-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: merge of `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE8-A705`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE9-A705` (only if residual remains)

## Название/цель
Выполнить closure-review после wave8 и принять fail-closed merged-main решение по `UX-11/UX-12`: `Fixed` или `Open + wave9`.
...
- `REQ-1` no disconnected/duplicate UX actions:
  - solution: closure-review4 is evidence/governance only; runtime untouched.
- `REQ-2` no shortcuts:
  - solution: decision based on merged-main deterministic evidence, not wave count.
- `REQ-3` optimize existing tabs first:
  - solution: if residual remains, next block is internal wave9 decomposition only.
...
- **Date/time (local):** `2026-03-04 17:20 +0500`
- **Sources opened (from this query):**
  - `https://docs.sonarsource.com/sonarqube-server/10.6/user-guide/code-metrics/metrics-definition`
- **Found reusable solution:** closure status should use objective maintainability signals and deterministic checks.
...
- **Symptom:** `UX-11/UX-12` may remain open after wave8 despite extractions.
- **Minimal reproduction:** compare merged-main LOC + deterministic suite + residual hotspots in parent files.
- **Evidence:** wave8 artifact and merged-main check outputs.
- **Five Whys:**
  1. why uncertainty remains: extraction is intentionally bounded;
  ...
  5. why now: wave8 completion requires explicit merged-main decision.
- **Root cause statement:** structural debt may still remain after wave8 and must be judged by objective evidence.
- **Fix mechanism:** evidence-based closure decision with immediate next-wave contract if open.

## Reuse-first plan (mandatory)
- **Internal reuse:** reuse wave1-8 artifacts, extracted modules, deterministic test lane, and canon sync pattern.
- **External reuse:** Sonar maintainability reference for objective closure criteria.
- **Why not build from scratch:** closure-review4 is governance-only, not runtime implementation.
...
## Scope
- Revalidate merged-main wave8 baseline.
- Publish closure-review4 artifact with explicit `UX-11/UX-12` decision.
- Sync canon/session docs.

## Out of scope
- Runtime wave9 implementation.
...
## Plan (1..N)
1. Capture merged-main wave8 deterministic baseline.
2. Publish closure-review4 artifact with `Fixed/Open` decision.
3. Sync canon/session docs and run `session_check`.
4. Open PR.

## DoD
- closure-review4 artifact published.
...
## Evidence
- wave8 merged PR URL + commit SHA.
- merged-main deterministic outputs.
- closure-review4 artifact + canon sync diff.

## Token / run budget (mandatory for expensive suites)
- Max full runs: `0` (docs/evidence decision block).
- E2E policy: reuse wave8 green acceptance evidence because runtime unchanged.
- Stop condition: if runtime edits are required, stop and switch to wave9 implementation block.
...
- **Rollback:** revert closure-review4 commit.
- **Post-release monitoring window:** wave9 block must rerun targeted acceptance lane if opened.
...
- Mixing runtime refactor into closure-review4 docs block.
...
- `Current residuals accepted in this block`: to be decided from merged-main wave8 evidence.
- `Why not in this block`: closure-review4 is governance-only.
- `Risk if deferred`: repeated high-context edits in monoliths.
- `Linked follow-up Task Package(s)`: `TP-2026-03-04-uvc-ux-tech-debt-decomposition-wave9-a705.md` (if needed).
- `Expiry/trigger to stop deferral`: if closure-review4 is `Open`, wave9 starts immediately as next block.

## Next-block contract (mandatory)
- `Next block objective`: either mark `UX-11/UX-12` as `Fixed` or execute wave9 bounded extraction.
...
- `Blocked-by conditions`: wave8 not merged or merged-main checks red.
