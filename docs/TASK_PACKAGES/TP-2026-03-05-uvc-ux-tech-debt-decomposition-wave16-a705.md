# TP-2026-03-05-uvc-ux-tech-debt-decomposition-wave16-a705

## Block identity
- `BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-WAVE16-A705`
- `PARENT_BLOCK_ID`: `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`
- `DEPENDS_ON`: closure decision `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW11-A705 = Open`
- `UNLOCKS`: `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW12-A705`

## Название/цель
Выполнить следующий bounded decomposition wave после closure-review11, чтобы продолжить снижение оркестрационной концентрации в `console.py` и `ProvisioningWizard.tsx` без изменения продуктового поведения и без добавления новых вкладок.
...
- **Date/time (local):** `2026-03-05 13:20 +0500`
- **Sources opened (from this query):**
  - `https://refactoring.com/catalog/extractFunction.html`
- **Found reusable solution:** continue incremental extraction by moving cohesive logic slices behind stable interfaces with behavior parity checks.
...
- **Symptom:** `UX-11/UX-12` remain open after closure-review11.
- **Minimal reproduction:** merged-main baseline after wave15 is still `console.py=24376`, `ProvisioningWizard.tsx=4349`.
- **Evidence:** closure-review11 artifact and deterministic merged-main checks.
- **Five Whys:**
  ...
  5. why wave16 now: closure-review11 explicitly kept backlog open by contract.
- **Root cause statement:** orchestration concentration in parent router/wizard files remains above maintainability threshold despite wave15 extraction.
...
- Publish wave16 artifact and sync canon/session docs.
...
- canon/session docs touched by wave16 evidence

## Plan (1..N)
1. Capture closure-review11 merged-main baseline and define minimal backend/frontend extraction slices.
...
4. Publish wave16 artifact, sync canon/session docs, open PR.

## DoD
- Wave16 extraction committed with deterministic checks green.
...
## Evidence
- Wave16 artifact with deterministic command outputs and moved-responsibility map.
- PR URL + CI run URL.
...
- **Rollback:** revert wave16 merge commit.
- **Post-release monitoring window:** run closure-review12 on merged-main evidence before any wave17 work.
...
- `Current residuals accepted in this block`: parent files can remain above closure threshold after wave16.
...
- `Linked follow-up Task Package(s)`: `TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review12-a705.md`.
- `Expiry/trigger to stop deferral`: if wave16 closes neither `UX-11` nor `UX-12`, closure-review12 must open next bounded wave immediately.

## Next-block contract (mandatory)
- `Next block objective`: closure-review12 fail-closed merged-main decision for `UX-11/UX-12`.
...
- `Blocked-by conditions`: wave16 PR not merged or required checks red.
