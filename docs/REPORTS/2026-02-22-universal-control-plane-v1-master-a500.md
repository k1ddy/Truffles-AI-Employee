...
- `UVC-UX-TECH-DEBT-DECOMPOSITION-A705`: in_progress (wave1 merged via PR `#885`; wave2 merged via PR `#888`; wave3 merged via PR `#889`; closeout merged via PR `#890` with explicit residual decision; wave4 extraction delivered on branch with deterministic checks (`py_compile`, `pytest 7 passed`, `console-web build`, platform-admin e2e `26 passed`) and updates `UX-11/UX-12` to `Open (Mitigated wave4, pending final-close)`).
