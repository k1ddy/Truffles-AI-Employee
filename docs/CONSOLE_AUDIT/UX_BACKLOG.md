...
| UX-11 | P1 | API maintainability | `console.py` remains an overgrown router with mixed concerns. | Slow onboarding and high regression probability for Platform Admin APIs. | Current branch LOC after wave9 (`truffles-api/app/routers/console.py`: `24493`); wave9 extraction delivered in `truffles-api/app/services/console_branch_changes.py` with retained deterministic evidence (`35 passed` + `branch_change 10 passed`) and wave artifact `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave9-a705.md` (follow-up closeout required). | Open (Mitigated wave9; closure-review5 required) |
| UX-12 | P1 | Provisioning UX complexity | `ProvisioningWizard.tsx` remains a large multi-domain component. | Hard to ship safe improvements fast across onboarding paths. | Current branch LOC after wave9 (`console-web/src/components/ProvisioningWizard.tsx`: `4452`); wave9 extraction delivered in `console-web/src/components/provisioning-wizard-branch-actions.ts` with retained acceptance evidence (`build + targeted e2e 2 passed`) and wave artifact `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave9-a705.md` (follow-up closeout required). | Open (Mitigated wave9; closure-review5 required) |
