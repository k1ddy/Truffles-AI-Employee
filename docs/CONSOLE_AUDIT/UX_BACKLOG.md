...
| UX-38 | P0 | Calendar action safety envelope | `Wave39 Part A` through `Part E` are now completed locally: Calendar has a canonical action registry, backend-owned `allowed_actions` / blocked reasons, explicit booking `version` conflict handling, extracted local machines, historical blocked-state action surfaces, consultant-bot case-link gating, deterministic double-submit protection proof, and failure-family observability for denied/version-conflict/double-submit/filter/follow-up actions. | The remaining risk is now limited to merge closeout: `Wave39` is open as `PR #961`; merge it and run the bounded post-merge replay on `main` before switching away from Calendar work. | `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave39-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/calendar-bookings.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/logging_config.py`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/calendar_action_contract.py`, `truffles-api/tests/test_calendar_bookings_router.py`, `contracts/console_api/openapi.v1.yaml`, `/tmp/wave39-part-e-operator-captures/wave39-queue-default-1280.png`, `/tmp/wave39-part-e-operator-captures/wave39-cancel-panel-1280.png`, `/tmp/wave39-part-e-operator-captures/wave39-medium-width-1024.png`, `PR #960`, `PR #961`. | Open (`PR #961`) |
