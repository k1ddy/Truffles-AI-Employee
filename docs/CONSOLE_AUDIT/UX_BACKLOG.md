...
| UX-35 | P0 | Calendar operator surface density | `Wave38 Part A/B` now hardens the two post-merge friction points that were still making Calendar feel inconsistent: filters use one explicit `draft -> applied` contract, and the phone field preserves raw typing/deletion/paste while showing a normalized preview only when the number is coherent. | Primary booking entry is now predictable during filter use and phone entry, but lifecycle recovery for an already created booking is still unfinished until `Wave38 Part C/D` land. | `docs/CONSOLE_AUDIT/artifacts/2026-03-08-inbox-calendar-ux-logic-audit-a1.md`, `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/queue-state.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `truffles-api/app/routers/calendar.py`, `PR #960`. | Reopened under `Wave38`; `Part A/B` complete locally, `Part C/D` still required before closure |
| UX-36 | P0 | Operator workflow coverage gap | `Wave38 Part A/B` closes two missing proof areas (`draft -> applied` filter semantics and phone typing/deletion/paste ergonomics), but the lifecycle matrix is still incomplete until edit/reschedule/cancel flows for existing bookings are covered. | Create-flow proof can still go green while real operators remain blocked on correction and recovery flows if lifecycle coverage is not finished. | `docs/CONSOLE_AUDIT/artifacts/2026-03-08-inbox-calendar-ux-logic-audit-a1.md`, `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `console-web/src/app/calendar/page.tsx`, `PR #960`. | Reopened under `Wave38`; filter/phone proof green locally, lifecycle matrix still open |
| UX-37 | P0 | Booking lifecycle completeness | Created bookings still cannot be edited, rescheduled, or cancelled from the full operator path even though correction and cancellation are part of the normal job and cancel capability already exists in the backend. | Operators cannot safely recover from wrong service/time/contact data and are forced into broken manual workarounds or stale records. | `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/components/CaseBookingsPanel.tsx`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `PR #959`. | Open (`Wave38` active) |
