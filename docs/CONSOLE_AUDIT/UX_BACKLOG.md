...
| UX-11 | P1 | API maintainability | `console.py` remains an overgrown router with mixed concerns. | Slow onboarding and high regression probability for Platform Admin APIs. | Current branch LOC after wave8 extraction (`truffles-api/app/routers/console.py`: `24554`); wave1 extraction delivered in `truffles-api/app/services/console_router_utils.py` + tests `truffles-api/tests/test_console_router_utils.py`; wave2 extraction delivered in `truffles-api/app/services/console_control_tower_utils.py` + tests `truffles-api/tests/test_console_control_tower_utils.py`; wave3 orchestration extraction delivered in `truffles-api/app/services/console_control_tower_program.py` + tests `truffles-api/tests/test_console_control_tower_program.py`; wave4 readiness extraction delivered in `truffles-api/app/services/console_onboarding_readiness.py` + tests `truffles-api/tests/test_console_onboarding_readiness.py`; wave5 validation extraction delivered in `truffles-api/app/services/console_router_utils.py`; wave6 fleet-state extraction delivered in `truffles-api/app/services/console_fleet_state.py`; wave7 membership-state extraction delivered in `truffles-api/app/services/console_membership_state.py`; wave8 go-live governance extraction delivered in `truffles-api/app/services/console_onboarding_readiness.py` with deterministic evidence `35 passed` in retained lane: `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave8-a705.md`. | Open (Mitigated wave8; closure-review4 required) |
| UX-12 | P1 | Provisioning UX complexity | `ProvisioningWizard.tsx` remains a large multi-domain component. | Hard to ship safe improvements fast across onboarding paths. | Current branch LOC after wave8 extraction (`console-web/src/components/ProvisioningWizard.tsx`: `4527`); wave1 helper extraction delivered in `console-web/src/components/provisioning-wizard-utils.ts`; wave2 domain extraction delivered in `console-web/src/components/provisioning-wizard-domain.ts`; wave3 derived-state extraction delivered in `console-web/src/components/provisioning-wizard-derived.ts`; wave4 readiness view extraction delivered in `console-web/src/components/provisioning-wizard-readiness-panel.tsx`; wave5 shell extraction delivered in `console-web/src/components/provisioning-wizard-shell-panels.tsx`; wave6 JSON payload extraction delivered in `console-web/src/components/provisioning-wizard-json-payloads.ts`; wave7 state lifecycle extraction delivered in `console-web/src/components/provisioning-wizard-state.ts`; wave8 JSON sync extraction delivered in `console-web/src/components/provisioning-wizard-state.ts` with retained acceptance evidence (`build + e2e 26 passed`): `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave8-a705.md`. | Open (Mitigated wave8; closure-review4 required) |
