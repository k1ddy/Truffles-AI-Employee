...
| UX-35 | P0 | Calendar operator surface density | `Wave38` completes the hard Calendar recovery path: filters now use one explicit `draft -> applied` contract, phone entry keeps raw editing natural, and booking correction no longer hides behind dead-end panels. | Booking creation and recovery now stay understandable and usable for operators, but this remains pending final PR merge because the closeout is still local to `PR #960`. | `docs/CONSOLE_AUDIT/artifacts/2026-03-08-inbox-calendar-ux-logic-audit-a1.md`, `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/queue-state.ts`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `/tmp/wave38-operator-captures/wave38-filters-applied-1280.png`, `/tmp/wave38-operator-captures/wave38-phone-valid-1280.png`, `/tmp/wave38-operator-captures/wave38-edit-open-1280.png`, `PR #960`. | Reopened under `Wave38`; local closeout complete, pending PR merge |
| UX-36 | P0 | Operator workflow coverage gap | `Wave38` now proves the full Calendar operator loop instead of only the create path: deterministic coverage includes `draft/apply/reset`, phone typing/deletion/paste, edit/reschedule, cancel, blocked no-show lifecycle, follow-up safety, and medium-width layout. | The acceptance gap is locally closed, but evidence still needs to land through `PR #960` before the backlog item can be fully closed in canon/main. | `docs/CONSOLE_AUDIT/artifacts/2026-03-08-inbox-calendar-ux-logic-audit-a1.md`, `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/e2e/calendar-operator.spec.ts`, `console-web/e2e/inspect_case.spec.ts`, `/tmp/wave38-operator-captures/wave38-cancel-panel-1280.png`, `/tmp/wave38-operator-captures/wave38-no-show-disabled-1280.png`, `/tmp/wave38-operator-captures/wave38-medium-width-1024.png`, `PR #960`. | Reopened under `Wave38`; local proof complete, pending PR merge |
| UX-37 | P0 | Booking lifecycle completeness | Existing bookings now have bounded operator lifecycle controls: `PATCH /calendar/bookings/{booking_id}` for edit/reschedule, body-based cancel, edit-mode composer prefill, deterministic slot resets, and explicit blocked states for non-active statuses. | Operators can now correct or cancel wrong bookings without manual workarounds, but the backlog item stays open until `PR #960` merges the local closeout. | `docs/TASK_PACKAGES/TP-2026-03-09-inbox-calendar-ux-reconstruction-wave38-a1.md`, `console-web/src/app/calendar/page.tsx`, `console-web/src/lib/calendar-bookings.ts`, `truffles-api/app/routers/calendar.py`, `truffles-api/app/services/appointment_service.py`, `contracts/console_api/openapi.v1.yaml`, `PR #960`. | Reopened under `Wave38`; local lifecycle closeout complete, pending PR merge |
