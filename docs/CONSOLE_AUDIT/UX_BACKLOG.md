...
| UX-11 | P1 | API maintainability | `console.py` remains an overgrown router with mixed concerns. | Slow onboarding and high regression probability for Platform Admin APIs. | Current branch LOC snapshot after wave7 extraction (`truffles-api/app/routers/console.py`: `24603`); wave1 extraction delivered in `truffles-api/app/services/console_router_utils.py` + tests `truffles-api/tests/test_console_router_utils.py`; wave2 extraction delivered in `truffles-api/app/services/console_control_tower_utils.py` + tests `truffles-api/tests/test_console_control_tower_utils.py`; wave3 orchestration extraction delivered in `truffles-api/app/services/console_control_tower_program.py` + tests `truffles-api/tests/test_console_control_tower_program.py`; wave4 readiness extraction delivered in `truffles-api/app/services/console_onboarding_readiness.py` + tests `truffles-api/tests/test_console_onboarding_readiness.py`; wave5 validation extraction delivered in `truffles-api/app/services/console_router_utils.py`; wave6 fleet-state extraction delivered in `truffles-api/app/services/console_fleet_state.py`; wave7 membership-state extraction delivered in `truffles-api/app/services/console_membership_state.py` + deterministic tests `truffles-api/tests/test_console_membership_state.py` (`30 passed` in retained lane) with evidence: `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave7-a705.md`. | Open (Mitigated wave7; final-review3 required) |
| UX-12 | P1 | Provisioning UX complexity | `ProvisioningWizard.tsx` remains a large multi-domain component. | Hard to ship safe improvements fast across onboarding paths. | Current branch LOC snapshot after wave7 extraction (`console-web/src/components/ProvisioningWizard.tsx`: `4544`); wave1 helper extraction delivered in `console-web/src/components/provisioning-wizard-utils.ts`; wave2 domain extraction delivered in `console-web/src/components/provisioning-wizard-domain.ts`; wave3 derived-state extraction delivered in `console-web/src/components/provisioning-wizard-derived.ts`; wave4 readiness view extraction delivered in `console-web/src/components/provisioning-wizard-readiness-panel.tsx`; wave5 shell extraction delivered in `console-web/src/components/provisioning-wizard-shell-panels.tsx`; wave6 JSON payload extraction delivered in `console-web/src/components/provisioning-wizard-json-payloads.ts`; wave7 state lifecycle extraction delivered in `console-web/src/components/provisioning-wizard-state.ts` with evidence: `docs/CONSOLE_AUDIT/artifacts/2026-03-04-uvc-tech-debt-decomposition-wave7-a705.md`. | Open (Mitigated wave7; final-review3 required) |
