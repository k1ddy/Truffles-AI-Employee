...
| UX-11 | P1 | API maintainability | `console.py` remains an overgrown router with mixed concerns. | Slow onboarding and high regression probability for Platform Admin APIs. | Wave17 delegated rollback payload normalization into `console_branch_changes` helper (`prepare_branch_change_rollback_payload`) and updated rollback handler wiring; bounded deterministic lane remained green (`console.py=24381`, `ProvisioningWizard.tsx=4325`, `35 passed` + `branch_change 25 passed`) and fail-closed status remains pending merged-main closure-review13 decision. References: `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave17-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review13-a705.md`. | Open (Mitigated wave17; closure-review13 required) |
| UX-12 | P1 | Provisioning UX complexity | `ProvisioningWizard.tsx` remains a large multi-domain component. | Hard to ship safe improvements fast across onboarding paths. | Wave17 delegated go-live decision/waiver submit orchestration into `provisioning-wizard-branch-actions` (`submitGoLiveDecisionMutation`, `submitGoLiveWaiverMutation`) and reduced wizard size (`4338 -> 4325`) with deterministic lane retained (`2 passed` targeted e2e); final status is fail-closed pending merged-main closure-review13 decision. References: `docs/CONSOLE_AUDIT/artifacts/2026-03-05-uvc-tech-debt-decomposition-wave17-a705.md`, `docs/TASK_PACKAGES/TP-2026-03-05-uvc-ux-tech-debt-decomposition-closure-review13-a705.md`. | Open (Mitigated wave17; closure-review13 required) |
...
| UX-29 | P0 | Cases <-> Calendar context switch | `Заявки` and `Записи` are weakly connected; managers lose case context when moving to bookings. | Extra clicks and slower response to booking actions. | `console-web/src/components/CaseConversation.tsx`, `console-web/src/app/calendar/page.tsx`, `truffles-api/app/routers/calendar.py`. | Open |
| UX-30 | P0 | Inbox first-screen readability | Large context panels before chat force vertical scrolling in active case handling. | Managers spend time scrolling before seeing the operative chat exchange. | `console-web/src/components/CaseConversation.tsx`. | Open |
| UX-31 | P1 | SLA semantics clarity | SLA labels prioritize ambiguous elapsed time phrases over action guidance. | Operators misread urgency and choose slower or wrong next action. | `console-web/src/utils/labels.ts`, `console-web/src/components/CaseList.tsx`. | Open |
| UX-32 | P0 | Calendar as operator queue | `Записи` page is form-centric and lacks case-linked queue ergonomics for managers. | Poor booking queue triage and weak handoff between chat and appointment operations. | `console-web/src/app/calendar/page.tsx`, `truffles-api/app/services/appointment_service.py`. | Open |
| UX-33 | P1 | Terminology simplification and hinting | Technical terms and status blocks in Inbox/Calendar are not consistently business-readable. | Increased onboarding time and misinterpretation risk for non-technical admins. | `console-web/src/components/CaseConversation.tsx`, `console-web/src/app/calendar/page.tsx`, `console-web/src/utils/labels.ts`. | Open |
