# UVC Tech Debt Decomposition Wave16 (A705)
...
Goal
- Continue bounded decomposition after closure-review11 without runtime behavior drift.
- Reduce orchestration concentration for `UX-11/UX-12` by delegating repeated publish/go-live mutation flows into service/helper modules.

What changed
- Backend (`UX-11`): extracted publish-success branch-change state transition helper to `truffles-api/app/services/console_branch_changes.py`:
  - `apply_branch_change_published_state`
- Backend wiring:
  - `truffles-api/app/routers/console.py` (`publish_branch_change`) now delegates publish-success state mutation to service helper.
- Backend tests:
  - Added deterministic service-level coverage in `truffles-api/tests/test_console_branch_changes.py`:
    - `test_apply_branch_change_published_state_sets_snapshot_and_actor`
    - `test_apply_branch_change_published_state_preserves_none_snapshot`
- Frontend (`UX-12`): extracted shared go-live mutation submit flow into `console-web/src/components/provisioning-wizard-branch-actions.ts`:
  - `submitGoLiveMutation`
- Frontend wiring:
  - `ProvisioningWizard` now uses `submitGoLiveMutation` for `approve/reject/waive` actions and preserves existing validation texts/copy.

LOC impact
- `truffles-api/app/routers/console.py`: `24376 -> 24380` (`+4`)
- `console-web/src/components/ProvisioningWizard.tsx`: `4349 -> 4338` (`-11`)
- Note: wave16 keeps fail-closed quality focus on ownership transfer and deterministic parity; closure decision remains in closure-review12.
...
Residual
- `UX-11`/`UX-12` remain fail-closed until merged-main closure decision in `UVC-UX-TECH-DEBT-DECOMPOSITION-CLOSURE-REVIEW12-A705`.
