...
- Consultant verification Wave1/Wave2/Wave3 contract:
  ...
  - Overview now returns stable workspace blocker codes (`workspace_disabled`, `branch_required`, `preview_source_missing`) alongside owner-safe blocker copy, so tenant acceptance can diagnose the tab deterministically without reintroducing ops jargon into the primary owner path.
  - `feature_enabled` remains in the response only as a backward-compatible alias of `workspace_enabled`; new logic should treat `workspace_enabled` as the real workspace gate and `team_tools_enabled` as the separate governance gate.
  - `python3 ops/consultant_verification_acceptance.py --client-slug <client> --branch-slug <branch> --pretty` now captures one tenant-level acceptance artifact from the real overview + session/message path, so the remaining owner proof is machine-readable instead of browser-only.
